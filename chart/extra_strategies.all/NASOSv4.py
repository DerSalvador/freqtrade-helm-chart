...
class NASOSv4(IStrategy):
    ...
    rsi_entry = IntParameter(50, 100, default=entry_params['rsi_entry'], space='entry', optimize=True)
    # trailing stoploss hyperopt parameters
    # hard stoploss profit
    pHSL = DecimalParameter(-0.2, -0.04, default=-0.15, decimals=3, space='exit', optimize=True, load=True)
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(0.008, 0.02, default=0.016, decimals=3, space='exit', optimize=True, load=True)
    pSL_1 = DecimalParameter(0.008, 0.02, default=0.014, decimals=3, space='exit', optimize=True, load=True)
    # profit threshold 2, SL_2 is used
    pPF_2 = DecimalParameter(0.04, 0.1, default=0.024, decimals=3, space='exit', optimize=True, load=True)
    pSL_2 = DecimalParameter(0.02, 0.07, default=0.022, decimals=3, space='exit', optimize=True, load=True)
    # Trailing stop:
    trailing_stop = True
