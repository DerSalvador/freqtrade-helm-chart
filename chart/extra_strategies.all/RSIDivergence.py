...
def valuewhen(dataframe, condition, source, occurrence):
    ...
    copy.loc[(rangeLower <= copy['barrsince']) & (copy['barrsince'] <= rangeUpper), 'in_range'] = 1
...
class RSIDivTirail(IStrategy):
    ...
    entry_params = {'use_bull': True, 'use_hidden_bull': False, 'ewo_high': 5.835, 'rsi_entry': 55}
    # Sell hyperspace params:
    exit_params = {'use_bear': True, 'use_hidden_bear': True}
    # ROI table:
    minimal_roi = {'0': 0.05}
    # Stoploss:
    stoploss = -0.05
    ...
    use_bull = BooleanParameter(default=entry_params['use_bull'], space='entry', optimize=True)
    use_hidden_bull = BooleanParameter(default=entry_params['use_hidden_bull'], space='entry', optimize=True)
    use_bear = BooleanParameter(default=exit_params['use_bear'], space='exit', optimize=True)
    use_hidden_bear = BooleanParameter(default=exit_params['use_hidden_bear'], space='exit', optimize=True)
    # Protection
    fast_ewo = 50
    slow_ewo = 200
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['osc'] > dataframe['valuewhen_plFound_osc']) & (dataframe['inrange_plFound_osc'] == 1), 'oscHL'] = 1
        # Price: Lower Low
        # priceLL = low[lbR] < valuewhen(plFound, low[lbR], 1)
        dataframe['valuewhen_plFound_low'], dataframe['inrange_plFound_low'] = valuewhen(dataframe, 'plFound', 'low', 1)
        dataframe.loc[dataframe['low'] < dataframe['valuewhen_plFound_low'], 'priceLL'] = 1
        #bullCond = plotBull and priceLL and oscHL and plFound
        dataframe.loc[(dataframe['priceLL'] == 1) & (dataframe['oscHL'] == 1) & (dataframe['plFound'] == 1), 'bullCond'] = 1
        # plot(
        #      plFound ? osc[lbR] : na,
        #      offset=-lbR,
        ...
        dataframe['valuewhen_plFound_osc'], dataframe['inrange_plFound_osc'] = valuewhen(dataframe, 'plFound', 'osc', 1)
        dataframe.loc[(dataframe['osc'] < dataframe['valuewhen_plFound_osc']) & (dataframe['inrange_plFound_osc'] == 1), 'oscLL'] = 1
        #
        # // Price: Higher Low
        #
        # priceHL = low[lbR] > valuewhen(plFound, low[lbR], 1)
        dataframe['valuewhen_plFound_low'], dataframe['inrange_plFound_low'] = valuewhen(dataframe, 'plFound', 'low', 1)
        dataframe.loc[dataframe['low'] > dataframe['valuewhen_plFound_low'], 'priceHL'] = 1
        # hiddenBullCond = plotHiddenBull and priceHL and oscLL and plFound
        dataframe.loc[(dataframe['priceHL'] == 1) & (dataframe['oscLL'] == 1) & (dataframe['plFound'] == 1), 'hiddenBullCond'] = 1
        #
        # plot(
        #      plFound ? osc[lbR] : na,
        #      offset=-lbR,
        ...
        dataframe.loc[(dataframe['osc'] < dataframe['valuewhen_phFound_osc']) & (dataframe['inrange_phFound_osc'] == 1), 'oscLH'] = 1
        #
        # // Price: Higher High
        #
        # priceHH = high[lbR] > valuewhen(phFound, high[lbR], 1)
        dataframe['valuewhen_phFound_high'], dataframe['inrange_phFound_high'] = valuewhen(dataframe, 'phFound', 'high', 1)
        dataframe.loc[dataframe['high'] > dataframe['valuewhen_phFound_high'], 'priceHH'] = 1
        #
        # bearCond = plotBear and priceHH and oscLH and phFound
        dataframe.loc[(dataframe['priceHH'] == 1) & (dataframe['oscLH'] == 1) & (dataframe['phFound'] == 1), 'bearCond'] = 1
        #
        # plot(
        #      phFound ? osc[lbR] : na,
        #      offset=-lbR,
        ...
        dataframe['valuewhen_phFound_osc'], dataframe['inrange_phFound_osc'] = valuewhen(dataframe, 'phFound', 'osc', 1)
        dataframe.loc[(dataframe['osc'] > dataframe['valuewhen_phFound_osc']) & (dataframe['inrange_phFound_osc'] == 1), 'oscHH'] = 1
        #
        # // Price: Lower High
        #
        # priceLH = high[lbR] < valuewhen(phFound, high[lbR], 1)
        dataframe['valuewhen_phFound_high'], dataframe['inrange_phFound_high'] = valuewhen(dataframe, 'phFound', 'high', 1)
        dataframe.loc[dataframe['high'] < dataframe['valuewhen_phFound_high'], 'priceLH'] = 1
        #
        # hiddenBearCond = plotHiddenBear and priceLH and oscHH and phFound
        dataframe.loc[(dataframe['priceLH'] == 1) & (dataframe['oscHH'] == 1) & (dataframe['phFound'] == 1), 'hiddenBearCond'] = 1
        #
        # plot(
        #      phFound ? osc[lbR] : na,
        #      offset=-lbR,
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.use_bull.value:
            ...
            #(dataframe['osc'] < self.rsi_entry.value) &
            conditions.append((dataframe['bullCond'] > 0) & (dataframe['volume'] > 0))
        if self.use_hidden_bull.value:
            #(dataframe['EWO'] > self.ewo_high.value) &
            #(dataframe['osc'] < self.rsi_entry.value) &
            conditions.append((dataframe['hiddenBullCond'] > 0) & (dataframe['volume'] > 0))
        if conditions:
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.use_hidden_bear.value:
            conditions.append((dataframe['hiddenBearCond'] > 0) & (dataframe['volume'] > 0))
        if conditions:
