...
class HLHB(IStrategy):
    ...
    plot_config = {'main_plot': {'ema5': {}, 'ema10': {}}, 'subplots': {'RSI': {'rsi': {'color': 'red'}}, 'ADX': {'adx': {}}}}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # Make sure Volume is not 0
        dataframe.loc[qtpylib.crossed_above(dataframe['rsi'], 50) & qtpylib.crossed_above(dataframe['ema5'], dataframe['ema10']) & (dataframe['adx'] > 25) & (dataframe['volume'] > 0), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # Make sure Volume is not 0
        dataframe.loc[qtpylib.crossed_below(dataframe['rsi'], 50) & qtpylib.crossed_below(dataframe['ema5'], dataframe['ema10']) & (dataframe['adx'] > 25) & (dataframe['volume'] > 0), 'exit_long'] = 1
        return dataframe