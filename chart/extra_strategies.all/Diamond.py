...
# of entry/exit signals, But you can add your indicators and add
...
# freqtrade hyperopt --hyperopt-loss ShortTradeDurHyperOptLoss --spaces entry exit roi trailing stoploss --strategy Diamond -j 2 -e 10
...
# freqtrade hyperopt --hyperopt-loss OnlyProfitHyperOptLoss --spaces entry exit roi trailing stoploss --strategy Diamond -j 2 -e 10
...
# freqtrade hyperopt --hyperopt-loss SharpeHyperOptLoss --spaces entry exit roi trailing stoploss --strategy Diamond -j 2 -e 10
...
# freqtrade hyperopt --hyperopt-loss SharpeHyperOptLossDaily --spaces entry exit roi trailing stoploss --strategy Diamond -j 2 -e 10
...
# freqtrade hyperopt --hyperopt-loss SortinoHyperOptLoss --spaces entry exit roi trailing stoploss --strategy Diamond -j 2 -e 10
...
# freqtrade hyperopt --hyperopt-loss SortinoHyperOptLossDaily --spaces entry exit roi trailing stoploss --strategy Diamond -j 2 -e 10
...
class Diamond(IStrategy):
    ...
    entry_params = {'entry_fast_key': 'high', 'entry_horizontal_push': 7, 'entry_slow_key': 'volume', 'entry_vertical_push': 0.942}
    # Sell hyperspace params:
    exit_params = {'exit_fast_key': 'high', 'exit_horizontal_push': 10, 'exit_slow_key': 'low', 'exit_vertical_push': 1.184}
    # ROI table:
    ...
    timeframe = '5m'
    # #################### END OF RESULT PLACE ####################
    entry_vertical_push = DecimalParameter(0.5, 1.5, decimals=3, default=1, space='entry')
    entry_horizontal_push = IntParameter(0, 10, default=0, space='entry')
    #  you can not enable this lines befour you
    #  populate an indicator for them and set
    #  the same key name for it
    #  'ma_fast', 'ma_slow', {...}
    entry_fast_key = CategoricalParameter(['open', 'high', 'low', 'close', 'volume'], default='ma_fast', space='entry')
    #  'ma_fast', 'ma_slow', {...}
    entry_slow_key = CategoricalParameter(['open', 'high', 'low', 'close', 'volume'], default='ma_slow', space='entry')
    exit_vertical_push = DecimalParameter(0.5, 1.5, decimals=3, default=1, space='exit')
    exit_horizontal_push = IntParameter(0, 10, default=0, space='exit')
    #  'ma_fast', 'ma_slow', {...}
    exit_fast_key = CategoricalParameter(['open', 'high', 'low', 'close', 'volume'], default='ma_fast', space='exit')
    #  'ma_fast', 'ma_slow', {...}
    exit_slow_key = CategoricalParameter(['open', 'high', 'low', 'close', 'volume'], default='ma_slow', space='exit')
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(qtpylib.crossed_above(dataframe[self.entry_fast_key.value].shift(self.entry_horizontal_push.value), dataframe[self.entry_slow_key.value] * self.entry_vertical_push.value))
        if conditions:
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(qtpylib.crossed_below(dataframe[self.exit_fast_key.value].shift(self.exit_horizontal_push.value), dataframe[self.exit_slow_key.value] * self.exit_vertical_push.value))
        if conditions:
