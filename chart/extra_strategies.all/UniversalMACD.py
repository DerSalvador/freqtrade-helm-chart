...
class UniversalMACD(IStrategy):
    ...
    # Strategy parameters
    entry_umacd_max = DecimalParameter(-0.05, 0.05, decimals=5, default=-0.01176, space='entry')
    entry_umacd_min = DecimalParameter(-0.05, 0.05, decimals=5, default=-0.01416, space='entry')
    exit_umacd_max = DecimalParameter(-0.05, 0.05, decimals=5, default=-0.02323, space='exit')
    exit_umacd_min = DecimalParameter(-0.05, 0.05, decimals=5, default=-0.00707, space='exit')
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[dataframe['umacd'].between(self.entry_umacd_min.value, self.entry_umacd_max.value), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[dataframe['umacd'].between(self.exit_umacd_min.value, self.exit_umacd_max.value), 'exit_long'] = 1
        return dataframe