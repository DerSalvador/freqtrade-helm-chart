...
class Obelisk_3EMA_StochRSI_ATR(IStrategy):
    ...
    def do_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['ema8'] > dataframe['ema14']) & (dataframe['ema14'] > dataframe['ema50']) & qtpylib.crossed_above(dataframe['srsi_k'], dataframe['srsi_d']), 'go_long'] = 1
        dataframe['go_long'].fillna(0, inplace=True)
        dataframe.loc[qtpylib.crossed_above(dataframe['go_long'], 0), 'take_profit'] = dataframe['close'] + dataframe['atr'] * 2
        dataframe['take_profit'].fillna(method='ffill', inplace=True)
        dataframe.loc[qtpylib.crossed_above(dataframe['go_long'], 0), 'stop_loss'] = dataframe['close'] - dataframe['atr'] * 3
        dataframe['stop_loss'].fillna(method='ffill', inplace=True)
        dataframe.loc[qtpylib.crossed_above(dataframe['go_long'], 0), 'stop_pct'] = dataframe['atr'] * 3 / dataframe['close']
        ...
        if custom_info_pair is not None:
            ...
            if take_profit is not None:
                if current_rate > take_profit:
                    logger.debug('take_profit={}, current={}'.format(take_profit, current_rate))
                    return 0.001
            ...
            if stop_pct is not None:
                ...
                logger.debug('open={}, current={}, profit={}, stop_pct={}, stop={}'.format(current_rate / (1 + current_profit), current_rate, current_profit, stop_pct, current_rate * (1 - new_stop)))
                if new_stop > 0:
                    result = new_stop
        return result
    plot_config = {'main_plot': {'ema50': {'color': 'orange'}, 'ema14': {'color': 'blue'}, 'ema8': {'color': 'purple'}, 'take_profit': {'color': 'green'}, 'stop_loss': {'color': 'red'}}, 'subplots': {'SRSI': {'srsi_k': {'color': 'blue'}, 'srsi_d': {'color': 'red'}}, 'ATR': {'atr': {'color': 'blue'}}}}