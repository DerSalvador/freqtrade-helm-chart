...
import talib.abstract as ta
from technical.util import resample_to_interval, resampled_merge
from freqtrade.strategy import BooleanParameter, CategoricalParameter, DecimalParameter, IStrategy, IntParameter, RealParameter, merge_informative_pair, stoploss_from_open
...
class turbov8(IStrategy):
    ...
    # entry optizimation
    max_epa = CategoricalParameter([0, 1, 2, 3], default=3, space='entry', optimize=True)
    # protections
    ...
    ### BTC 
    fast_1h = IntParameter(3, 8, default=5, space='entry', optimize=True)
    slow_1h = IntParameter(25, 40, default=30, space='entry', optimize=True)
    # dca level optimization
    dca1 = DecimalParameter(low=0.01, high=0.03, decimals=2, default=0.02, space='entry', optimize=True, load=True)
    dca2 = DecimalParameter(low=0.03, high=0.05, decimals=2, default=0.04, space='entry', optimize=True, load=True)
    dca3 = DecimalParameter(low=0.05, high=0.07, decimals=2, default=0.06, space='entry', optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        elif (informative[f'sma_{fast}'].iloc[-1] < informative[f'sma_{slow}'].iloc[-1] and informative[f'sma_{fast}'].iloc[-1] < informative[f'sma_{fast}'].iloc[-2]).all():
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['rsi_fast'] < 35) & (dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['Run_From_Bear'] == 0) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'EWO above high')
        dataframe.loc[(dataframe['rsi_fast'] < 35) & (dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['close'] < dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset.value) & (dataframe['Run_From_Bear'] == 0) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'EWO below low')
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['Run_From_Bear'] == 1) & (dataframe['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'fucking bearzzz')
        return dataframe