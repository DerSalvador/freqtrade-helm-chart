...
#
# The point of this strategy is to entry and hold an up trend as long as possible.
...
# WARNING 
#
# Do not use stoploss_on_exchange or the bot may trigger emergencyexit when it 
# fails to place the stoploss.
#
# WARNING
#
# This strat will entry into ongoing trends, so pay attention to timing when you launch it. 
# If the market is peaking then you may end up entrying into trends that are just about to end.
#
#
# Contributions:
...
class Obelisk_Ichimoku_Slow_v1_3(IStrategy):
    ...
    minimal_roi = {'0': 0.1, '60': 0.072, '120': 0.049, '240': 0.02, '360': 0}
    # Trailing stoploss (not used)
    trailing_stop = True
    trailing_only_offset_is_reached = True
    trailing_stop_positive = 0.005
    trailing_stop_positive_offset = 0.04
    # WARNING setting a stoploss for this strategy doesn't make much sense, as it will entry
    # back into the trend at the next available opportunity, unless the trend has ended,
    # in which case it would exit anyway.
    # Stoploss:
    stoploss = -0.99
    ...
    def slow_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['ema_ok'] = ((dataframe['close'] > dataframe['ema50']) & (dataframe['ema50'] > dataframe['ema200'])).astype('int') * 2
        ...
        dataframe['entry_ok'] = ((dataframe['efi_ok'] > 0) & (dataframe['open'] < dataframe['ssl_up']) & (dataframe['close'] < dataframe['ssl_up'])).astype('int') * 1
        dataframe['trend_pulse'] = ((dataframe['ichimoku_ok'] > 0) & (dataframe['ssl_ok'] > 0) & (dataframe['ema_ok'] > 0)).astype('int') * 2
        dataframe['trend_over'] = (dataframe['ssl_ok'] == 0).astype('int') * 1
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # when backtesting at 5m/1m only set signal on the hour
        dataframe.loc[(dataframe['trending'] > 0) & (dataframe['entry_ok'] > 0) & (dataframe['date'].dt.minute == 0), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # when backtesting at 5m/1m only set signal on the hour
        dataframe.loc[(dataframe['trending'] == 0) & (dataframe['date'].dt.minute == 0), 'exit_long'] = 1
        return dataframe
    ...
    plot_config = {'main_plot': {'senkou_a': {'color': 'green', 'fill_to': 'senkou_b', 'fill_label': 'Ichimoku Cloud', 'fill_color': 'rgba(0,0,0,0.2)'}, 'senkou_b': {'color': 'red'}, 'tenkan_sen': {'color': 'blue'}, 'kijun_sen': {'color': 'orange'}}, 'subplots': {'Trend': {'trend_pulse': {'color': 'blue'}, 'trending': {'color': 'orange'}, 'trend_over': {'color': 'red'}}, 'Signals': {'ichimoku_ok': {'color': 'green'}, 'ssl_ok': {'color': 'red'}, 'ema_ok': {'color': 'orange'}, 'entry_ok': {'color': 'blue'}}}}