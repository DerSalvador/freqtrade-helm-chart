...
class MultiMA_TSL3(IStrategy):
    ...
    SELLMA = 1
    SELL_TRIGGER = 2
    entry_params = {'base_nb_candles_entry_trima': 15, 'base_nb_candles_entry_trima2': 38, 'low_offset_trima': 0.959, 'low_offset_trima2': 0.949, 'base_nb_candles_entry_ema': 9, 'base_nb_candles_entry_ema2': 75, 'low_offset_ema': 1.067, 'low_offset_ema2': 0.973, 'base_nb_candles_entry_zema': 25, 'base_nb_candles_entry_zema2': 53, 'low_offset_zema': 0.958, 'low_offset_zema2': 0.961, 'base_nb_candles_entry_hma': 70, 'base_nb_candles_entry_hma2': 12, 'low_offset_hma': 0.948, 'low_offset_hma2': 0.941, 'ewo_high': 2.615, 'ewo_high2': 2.188, 'ewo_low': -19.632, 'ewo_low2': -19.955, 'rsi_entry': 60, 'rsi_entry2': 45}
    exit_params = {'base_nb_candles_ema_exit': 5, 'high_offset_exit_ema': 0.994}
    # ROI table:
    minimal_roi = {'0': 100}
    stoploss = -0.15
    ...
    base_nb_candles_ema_exit2 = IntParameter(5, 80, default=20, space='exit', optimize=False)
    # Multi Offset
    ...
    entry_condition_enable_optimize = False
    ...
    ewo_high = DecimalParameter(2.0, 12.0, default=6.0, space='entry', optimize=ewo_check_optimize)
    ...
    rsi_entry_optimize = False
    rsi_entry = IntParameter(30, 70, default=50, space='entry', optimize=rsi_entry_optimize)
    rsi_entry2 = IntParameter(30, 70, default=50, space='entry', optimize=rsi_entry_optimize)
    entry_rsi_fast = IntParameter(0, 50, default=35, space='entry', optimize=False)
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if self.custom_info[pair][self.DATESTAMP] != last_candle['date']:
            ...
            if (last_candle['close'] > exit_ema * self.high_offset_exit_ema.value) & (last_candle['entry_copy'] == 0):
                ...
                return f'New Sell Signal ({enter_tag})'
        return False
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.entry_condition_trima_enable.value:
            ...
            entry_offset_trima = (dataframe['close'] < dataframe['trima_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['trima_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh'])
            dataframe.loc[entry_offset_trima, 'enter_tag'] += 'trima '
            conditions.append(entry_offset_trima)
        if self.entry_condition_zema_enable.value:
            ...
            entry_offset_zema = (dataframe['close'] < dataframe['zema_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['zema_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh'])
            dataframe.loc[entry_offset_zema, 'enter_tag'] += 'zema '
        ...
        if self.entry_condition_hma_enable.value:
            ...
            entry_offset_hma = ((dataframe['close'] < dataframe['hma_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & (dataframe['rsi'] < 35) | (dataframe['close'] < dataframe['hma_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['rsi'] < 30)) & (dataframe['rsi_fast'] < 30)
            dataframe.loc[entry_offset_hma, 'enter_tag'] += 'hma '
            conditions.append(entry_offset_hma)
        add_check = dataframe['live_data_ok'] & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_exit'] * self.high_offset_exit_ema.value) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < self.entry_rsi_fast.value) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & ((dataframe['close'] < dataframe['ema_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry.value)) | (dataframe['close'] < dataframe['ema_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low2.value) | (dataframe['ewo'] > self.ewo_high2.value) & (dataframe['rsi'] < self.rsi_entry2.value))) & (dataframe['volume'] > 0)
        if conditions:
            dataframe.loc[add_check & reduce(lambda x, y: x | y, conditions), ['entry_copy', 'enter_long']] = (1, 1)
        return dataframe
...
class MultiMA_TSL3a(MultiMA_TSL3):
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if self.custom_info[pair][self.DATESTAMP] != last_candle['date']:
            ...
            self.custom_info[pair][self.DATESTAMP] = last_candle['date']
            exit_tag = ''
            if (last_candle['close'] > exit_ema * self.high_offset_exit_ema.value) & (last_candle['entry_copy'] == 0) & (last_candle['rsi'] > self.min_rsi_exit):
                if self.config['runmode'].value in ('live', 'dry_run'):
                    self.custom_info[pair][self.SELL_TRIGGER] = 1
                    return False
                exit_tag = 'Decaying EMA'
            if (last_candle['rsi_fast_15m'] > self.min_rsi_exit_15m) & (last_candle['entry_copy'] == 0) & (last_candle['rsi'] > self.min_rsi_exit):
                if self.config['runmode'].value in ('live', 'dry_run'):
                    self.custom_info[pair][self.SELL_TRIGGER] = 1
                    return False
                exit_tag = 'RSI 15m Overbought'
            if not exit_tag == '':
                enter_tag = 'empty'
                if hasattr(trade, 'enter_tag') and trade.entry_tag is not None:
                    enter_tag = trade.entry_tag
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.entry_condition_hma_enable.value:
            ...
            dataframe.loc[entry_offset_hma, 'enter_tag'] += 'hma '
            conditions.append(entry_offset_hma)
        add_check = dataframe['live_data_ok'] & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_exit'] * self.high_offset_exit_ema.value) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < self.entry_rsi_fast.value) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & (dataframe['pump'].rolling(20).max() < 1) & ((dataframe['close'] < dataframe['ema_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry.value)) | (dataframe['close'] < dataframe['ema_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low2.value) | (dataframe['ewo'] > self.ewo_high2.value) & (dataframe['rsi'] < self.rsi_entry2.value))) & (dataframe['volume'] > 0)
        if conditions:
            dataframe.loc[add_check & reduce(lambda x, y: x | y, conditions), ['entry_copy', 'enter_long']] = (1, 1)
        return dataframe
