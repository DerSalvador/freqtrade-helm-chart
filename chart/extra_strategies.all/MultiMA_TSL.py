...
class MultiMA_TSL(IStrategy):
    INTERFACE_VERSION = 3
    entry_params = {'base_nb_candles_entry_ema': 50, 'low_offset_ema': 1.061, 'base_nb_candles_entry_zema': 30, 'low_offset_zema': 0.963, 'rsi_entry_zema': 50, 'base_nb_candles_entry_trima': 14, 'low_offset_trima': 0.963, 'rsi_entry_trima': 50, 'entry_roc_max': 45, 'entry_condition_trima_enable': True, 'entry_condition_zema_enable': True}
    exit_params = {'base_nb_candles_exit': 32, 'high_offset_ema': 1.002, 'base_nb_candles_exit_trima': 48, 'high_offset_trima': 1.085}
    # ROI table:
    minimal_roi = {'0': 100}
    stoploss = -0.15
    # Multi Offset
    base_nb_candles_exit = IntParameter(5, 80, default=20, space='exit', optimize=False)
    base_nb_candles_exit_trima = IntParameter(5, 80, default=20, space='exit', optimize=False)
    high_offset_trima = DecimalParameter(0.99, 1.1, default=1.012, space='exit', optimize=False)
    base_nb_candles_entry_ema = IntParameter(5, 80, default=20, space='entry', optimize=False)
    ...
    rsi_entry_ema = IntParameter(30, 70, default=61, space='entry', optimize=False)
    base_nb_candles_entry_trima = IntParameter(5, 80, default=20, space='entry', optimize=False)
    low_offset_trima = DecimalParameter(0.9, 0.99, default=0.958, space='entry', optimize=False)
    rsi_entry_trima = IntParameter(30, 70, default=61, space='entry', optimize=False)
    base_nb_candles_entry_zema = IntParameter(5, 80, default=20, space='entry', optimize=False)
    low_offset_zema = DecimalParameter(0.9, 0.99, default=0.958, space='entry', optimize=False)
    rsi_entry_zema = IntParameter(30, 70, default=61, space='entry', optimize=False)
    entry_condition_enable_optimize = True
    ...
    entry_roc_max = DecimalParameter(20, 70, default=55, space='entry', optimize=False)
    entry_peak_max = DecimalParameter(1, 1.1, default=1.03, decimals=3, space='entry', optimize=False)
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float, current_profit: float, **kwargs) -> float:
        ...
        if not self.config['runmode'].value in ('backtest', 'hyperopt'):
            ...
            if len(dataframe) >= 1:
                ...
                if (last_candle['exit_copy'] == 1) & (last_candle['entry_copy'] == 0):
                    sl_new = 0.001
        return sl_new
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        entry_offset_trima = self.entry_condition_trima_enable.value & (dataframe['close'] < dataframe['trima_offset_entry']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry_trima.value))
        dataframe.loc[entry_offset_trima, 'enter_tag'] += 'trima '
        conditions.append(entry_offset_trima)
        entry_offset_zema = self.entry_condition_zema_enable.value & (dataframe['close'] < dataframe['zema_offset_entry']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry_zema.value))
        dataframe.loc[entry_offset_zema, 'enter_tag'] += 'zema '
        conditions.append(entry_offset_zema)
        add_check = (dataframe['rsi_fast'] < self.entry_rsi_fast.value) & (dataframe['close'] < dataframe['ema_offset_entry']) & (dataframe['volume'] > 0)
        if conditions:
            dataframe.loc[add_check & reduce(lambda x, y: x | y, conditions), ['entry_copy', 'enter_long']] = (1, 1)
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[:, 'exit_copy'] = 0
        dataframe['ema_offset_exit'] = ta.EMA(dataframe, int(self.base_nb_candles_exit.value)) * self.high_offset_ema.value
        dataframe['trima_offset_exit'] = ta.TRIMA(dataframe, int(self.base_nb_candles_exit_trima.value)) * self.high_offset_trima.value
        conditions = []
        conditions.append((dataframe['close'] > dataframe['ema_offset_exit']) & (dataframe['volume'] > 0))
        conditions.append((dataframe['close'] > dataframe['trima_offset_exit']) & (dataframe['volume'] > 0))
        if conditions:
            dataframe.loc[reduce(lambda x, y: x | y, conditions), ['exit_copy', 'exit_long']] = (1, 1)
        if not self.config['runmode'].value in ('backtest', 'hyperopt'):
