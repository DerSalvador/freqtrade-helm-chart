...
class ConsensusShort(IStrategy):
    ...
    max_allowed_drawdown = DecimalParameter(0.1, 0.5, default=0.2, decimals=2, space='protection', optimize=protect_optimize)
    ...
    # Protection hyperspace params:
    protection_params = {'cooldown_lookback': 5, 'max_drawdown_lookback': 12, 'max_drawdown_trade_limit': 5, 'max_drawdown_stop_duration': 12, 'max_allowed_drawdown': 0.2, 'stoploss_guard_lookback': 12, 'stoploss_guard_trade_limit': 3, 'stoploss_guard_stop_duration': 12}

    @property
    def protections(self):
        return [{'method': 'CooldownPeriod', 'stop_duration_candles': self.cooldown_lookback.value}, {'method': 'MaxDrawdown', 'lookback_period_candles': self.max_drawdown_lookback.value, 'trade_limit': self.max_drawdown_trade_limit.value, 'stop_duration_candles': self.max_drawdown_stop_duration.value, 'max_allowed_drawdown': self.max_allowed_drawdown.value}, {'method': 'StoplossGuard', 'lookback_period_candles': self.stoploss_guard_lookback.value, 'trade_limit': self.stoploss_guard_trade_limit.value, 'stop_duration_candles': self.stoploss_guard_stop_duration.value, 'only_per_pair': False}]
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[dataframe['consensus_buy'] < self.buy_score_short.value, 'enter_short'] = 1
