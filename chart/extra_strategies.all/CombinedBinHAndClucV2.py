...
# The main idea is to entry only when overall uptrend in higher informative
...
class CombinedBinHAndClucV2(IStrategy):
    ...
    protections = [{'method': 'StoplossGuard', 'lookback_period_candles': 24, 'trade_limit': 2, 'stop_duration_candles': 12, 'only_per_pair': True}]
    ...
    def get_informative_indicators(dataframe: DataFrame, metadata: dict):
        ...
        dataframe['go_long'] = ((dataframe['ssl_high'] > 0) & (dataframe['mfi'].shift().rolling(3).mean() > dataframe['mfi']) & (dataframe['srsi_fk'].shift().rolling(3).mean() > dataframe['srsi_fk'])).astype('int') * 4
        return dataframe
