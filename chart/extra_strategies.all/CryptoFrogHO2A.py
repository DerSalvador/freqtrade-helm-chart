...
class CryptoFrogHO2A(IStrategy):
    ...
    exit_params = {'cstp_bail_how': 'roc', 'cstp_bail_roc': -0.01, 'cstp_bail_time': 1432, 'cstp_threshold': 0.0, 'droi_pullback': True, 'droi_pullback_amount': 0.014, 'droi_pullback_respect_table': False, 'droi_trend_type': 'rmi'}
    # ROI table:
    minimal_roi = {'0': 0.09, '39': 0.028, '49': 0.011, '105': 0}
    # Stoploss:
    stoploss = -0.13
    # Trailing stop:
    trailing_stop = True
    ...
    # -0.085,     # starting value: should be the same or smaller than initial stoploss - default -0.30
    ...
    custom_stop = {'decay-time': 105, 'decay-delay': 0, 'decay-start': -0.13, 'decay-end': -0.02, 'cur-min-diff': 0.03, 'cur-threshold': -0.02, 'roc-bail': -0.03, 'rmi-trend': 50, 'bail-how': 'immediate', 'pos-trail': True, 'pos-threshold': 0.005, 'pos-trail-dist': 0.015}
    # Dynamic ROI
