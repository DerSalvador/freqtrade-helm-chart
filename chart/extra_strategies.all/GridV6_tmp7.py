...
class GridV6_tmp7(IStrategy):
    ...
    grid_down_spacing_pct = DecimalParameter(3.0, 5.0, default=0.4, decimals=1, load=True, space='exit', optimize=False)
    grid_up_spacing_pct = DecimalParameter(1.4, 3.0, default=1.6, decimals=1, load=True, space='exit', optimize=True)
    ...
    def calculate_state(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        df_entry_1 = pd.DataFrame(Buy_1, columns=['entry_1'])
        df_entry_2 = pd.DataFrame(Buy_2, columns=['entry_2'])
        df_exit_1 = pd.DataFrame(Sell_1, columns=['exit_1'])
        ...
        # merge
        dataframe = pd.concat([dataframe,df_entry_1,df_entry_2,df_exit_1,df_grid_up,df_grid_down,df_bot_state],axis=1)
