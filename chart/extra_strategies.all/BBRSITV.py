...
class BBRSITV(IStrategy):
    ...
    entry_params = {'ewo_high': 4.86, 'for_ma_length': 22, 'for_sigma': 1.74}
    # Sell hyperspace params:
    exit_params = {'for_ma_length_exit': 65, 'for_sigma_exit': 1.895, 'rsi_high': 72}
    ...
    ewo_high = DecimalParameter(0, 7.0, default=entry_params['ewo_high'], space='entry', optimize=True)
    for_sigma = DecimalParameter(0, 10.0, default=entry_params['for_sigma'], space='entry', optimize=True)
    for_sigma_exit = DecimalParameter(0, 10.0, default=exit_params['for_sigma_exit'], space='exit', optimize=True)
    rsi_high = IntParameter(60, 100, default=exit_params['rsi_high'], space='exit', optimize=True)
    for_ma_length = IntParameter(5, 80, default=entry_params['for_ma_length'], space='entry', optimize=True)
    for_ma_length_exit = IntParameter(5, 80, default=exit_params['for_ma_length_exit'], space='exit', optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.config['runmode'].value == 'hyperopt':
            ...
            dataframe[f'basis_{self.for_ma_length_exit.value}'] = ta.EMA(dataframe['rsi'], self.for_ma_length_exit.value)
            # dev = for_mult * stdev(current_rsi, for_ma)
            dataframe[f'dev_{self.for_ma_length.value}'] = ta.STDDEV(dataframe['rsi'], self.for_ma_length.value)
            dataframe[f'dev_{self.for_ma_length_exit.value}'] = ta.STDDEV(dataframe['rsi'], self.for_ma_length_exit.value)
        #
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['rsi'] < dataframe[f'basis_{self.for_ma_length.value}'] - dataframe[f'dev_{self.for_ma_length.value}'] * self.for_sigma.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['volume'] > 0), 'enter_long'] = 1
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[((dataframe['rsi'] > self.rsi_high.value) | (dataframe['rsi'] > dataframe[f'basis_{self.for_ma_length_exit.value}'] + dataframe[f'dev_{self.for_ma_length_exit.value}'] * self.for_sigma_exit.value)) & (dataframe['volume'] > 0), 'exit_long'] = 1
        return dataframe
...
class BBRSITV4(BBRSITV):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['rsi'] < dataframe[f'basis_{self.for_ma_length.value}'] - dataframe[f'dev_{self.for_ma_length.value}'] * self.for_sigma.value) & ((dataframe['EWO'] > self.ewo_high.value) & (dataframe['EWO'] < 10) | (dataframe['EWO'] >= 10) & (dataframe['rsi'] < 40)) & (dataframe['rsi_4'] < 25) & (dataframe['volume'] > 0), 'enter_long'] = 1
        return dataframe
...
============================================================================================================================================================================
    ...
    INTERFACE_VERSION = 2
    # Buy hyperspace params:
    entry_params = {'ewo_high': 4.964, 'for_ma_length': 12, 'for_sigma': 2.313}
    # Sell hyperspace params:
    exit_params = {'for_ma_length_exit': 78, 'for_sigma_exit': 1.67, 'rsi_high': 60}
    # ROI table:  # value loaded from strategy
    minimal_roi = {'0': 0.1}
    # Stoploss:
    stoploss = -0.25  # value loaded from strategy
...
============================================================================================================================================================================
    """
    # Buy hyperspace params:
    entry_params = {'ewo_high': 4.85, 'for_ma_length': 11, 'for_sigma': 2.066}
    # Sell hyperspace params:
    exit_params = {'for_ma_length_exit': 61, 'for_sigma_exit': 1.612, 'rsi_high': 87}
    # ROI table:  # value loaded from strategy
    minimal_roi = {'0': 0.1}
    # Stoploss:
    stoploss = -0.25  # value loaded from strategy
...
class BBRSITV5(BBRSITV):
    ...
    stoploss = -0.3  # value loaded from strategy
    ##
    exit_params = {'pHSL': -0.178, 'pPF_1': 0.01, 'pPF_2': 0.048, 'pSL_1': 0.009, 'pSL_2': 0.043}
