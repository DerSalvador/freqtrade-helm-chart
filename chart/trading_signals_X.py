...
def calculate_bollinger_bands(df, period=20, num_sd=2):
    df['SMA'] = df['CLOSE'].rolling(window=period).mean()
    df['STD'] = df['CLOSE'].rolling(window=period).std()
    df['Upper_BB'] = df['SMA'] + (df['STD'] * num_sd)
    df['Lower_BB'] = df['SMA'] - (df['STD'] * num_sd)
    return df
...
def calculate_cci(df, period=20):
    ...
    df['MAD'] = df['TP'].rolling(window=period).apply(lambda x: np.fabs(x - x.mean()).mean())
...
def generate_trading_signals(df):
    conditions = [
        ...
        (df['%K'] < 20) & (df['%D'] < 20),
        (df['%K'] > 80) & (df['%D'] > 80),
        (df['ADX'] > 20) & (df['CCI'] < -100),
        (df['ADX'] > 20) & (df['CCI'] > 100)
    ]
    choices = ['Long', 'Short', 'Long', 'Short', 'Long', 'Short', 'Long', 'Short']
...
def analyze_groups(df):
    ...
    for name, group in grouped:
        print(f"\nAnalysis for {name}:\n")
        print(group[['TIMESTAMP', 'OPEN_RATE', 'CURRENT_RATE', 'ADX', 'SMI', 'Upper_BB', 'Lower_BB', '%K', '%D', 'CCI', 'Signal']].describe())
