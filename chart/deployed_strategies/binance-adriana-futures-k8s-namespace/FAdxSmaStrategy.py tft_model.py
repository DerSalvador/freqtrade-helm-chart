kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n binance-adriana-futures exec -it pod/freqtrade-binance-adriana-futures-669f5576b5-2wnwd -c freqtrade -- cat /freqtrade/user_data/strategies/FAdxSmaStrategy.py tft_model.py
...
class FAdxSmaStrategy(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions_short.append(dataframe[f'adx_{self.adx_period.value}'] > self.pos_entry_adx.value)
        conditions_long.append(qtpylib.crossed_above(dataframe[f'sma_short_{self.sma_short_period.value}'], dataframe[f'sma_long_{self.sma_long_period.value}']))
        conditions_short.append(qtpylib.crossed_below(dataframe[f'sma_short_{self.sma_short_period.value}'], dataframe[f'sma_long_{self.sma_long_period.value}']))
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        return dataframecat: tft_model.py: No such file or directory
