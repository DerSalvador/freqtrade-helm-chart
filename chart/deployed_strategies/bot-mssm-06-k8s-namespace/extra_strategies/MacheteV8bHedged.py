kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-mssm-06 exec -it pod/freqtrade-bot-mssm-06-696c8d58c4-fkj6j -c freqtrade -- cat /extra_strategies/MacheteV8bHedged.py
...
class MacheteV8bHedged(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def sendMessageToTelegram(msg: str):
        if MacheteV8bHedged.rpc is not None: 
            msg = {
                'type': RPCMessageType.STARTUP,
                'status': f"{msg}"
            }
            MacheteV8bHedged.rpc.send_msg(msg)
        else:
    ...
    def bot_start(self, **kwargs) -> None:
        ...
        if self.config['dersalvador']['hedging'] is not None:
            ...
            log.info(msg)
            MacheteV8bHedged.sendMessageToTelegram(msg)
        else:
            msg="MSSM: No Hedging section found in config file"
            log.info(msg)
            MacheteV8bHedged.sendMessageToTelegram(msg)
    ...
    def get_market_condition_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ssl =  MacheteV8bHedged.SSLChannels_ATR(dataframe, 10)
    ...
    def get_custom_stoploss_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['rmi'] = MacheteV8bHedged.RMI(dataframe, length=24, mom=5)
        ssldown, sslup = MacheteV8bHedged.SSLChannels_ATR(dataframe, length=21)
        dataframe['sroc'] = MacheteV8bHedged.SROC(dataframe, roclen=21, emalen=13, smooth=21)
    ...
    def trend_hedge(self, pair: str, trade: Trade, current_time , current_rate, current_profit, **kwargs) -> float:
        ...
        if is_bullish:
            dsHedging.hedge_me(self, trade, pair, MacheteV8bHedged.existing_position_on_exchange )
        elif is_bearish:
            dsHedging.hedge_me(self, trade, pair, MacheteV8bHedged.existing_position_on_exchange )
        else:
            log.info("MSSM: Trend is not bullish or bearish..")
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
        ...
        MacheteV8bHedged.existing_position_on_exchange = positionFetcher.get_futures_position_information(symbol)
        # if MacheteV8bHedged.existing_position_on_exchange is not None and MacheteV8bHedged.existing_position_on_exchange != "":              
        MacheteV8bHedged.trend_hedge(self, pair, trade, current_time, current_rate, current_profit, **kwargs) # type: ignore
        return None