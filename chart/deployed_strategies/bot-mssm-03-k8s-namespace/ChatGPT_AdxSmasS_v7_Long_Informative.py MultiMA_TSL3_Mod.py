kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-mssm-03 exec -it pod/freqtrade-bot-mssm-03-7bcbf65bf7-hjf5x -c freqtrade -- cat /freqtrade/user_data/strategies/ChatGPT_AdxSmasS_v7_Long_Informative.py MultiMA_TSL3_Mod.py
...
class ChatGPT_AdxSmasS_v7_Long_Informative(IStrategy):
    INTERFACE_VERSION = 3
    minimal_roi = {"0": 0.10}  # Targeting a 10% ROI
    stoploss = -0.10  # Adjust stoploss for long futures trades
    timeframe = '1h'
    
    informative_timeframe = '4h'  # New informative timeframe

    def informative_pairs(self):
        return [("ETH/USDT", self.informative_timeframe)]
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative = self.dp.get_pair_dataframe(pair="ETH/USDT", timeframe=self.informative_timeframe)
...
cat: MultiMA_TSL3_Mod.py: No such file or directory
