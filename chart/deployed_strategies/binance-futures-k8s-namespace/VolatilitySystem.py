...
class VolatilitySystem(IStrategy):
    ...
    plot_config = {'main_plot': {}, 'subplots': {'Volatility system': {'atr': {'color': 'white'}, 'abs_close_change': {'color': 'red'}}}}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Based on TA indicators, populates the entry and exit signals for the given dataframe
        :param dataframe: DataFrame
        :return: DataFrame with entry and exit columns
        """
        ...
        dataframe.loc[dataframe['close_change'] * 1 > dataframe['atr'].shift(1), 'enter_long'] = 1
        # qtpylib.crossed_above(dataframe['close_change'] * -1, dataframe['atr']),
        dataframe.loc[dataframe['close_change'] * -1 > dataframe['atr'].shift(1), 'enter_short'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        use exit/entry signals as long/short indicators
        """
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float, proposed_leverage: float, max_leverage: float, side: str, **kwargs) -> float:
        ...
        :param entry_tag: Optional entry_tag (entry_tag) if provided with the entry signal.
