...
class StarRise(IStrategy):
    ...
    # 1.1% TTP
    exit_params = {'pHSL': -0.998, 'pPF_1': 0.011, 'pPF_2': 0.065, 'pSL_1': 0.011, 'pSL_2': 0.062}
    # Max Deviation -0.349
    stoploss = -0.998
    # Custom stoploss
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        is_crash_1 = (dataframe['tpct_change_1'] < 0.08) & (dataframe['tpct_change_2'] < 0.08) & (dataframe['tpct_change_4'] < 0.1)
        # Dip check
        # Bull confirm
        # Overpump check
        dataframe.loc[(dataframe['close'] < dataframe['mama']) & (dataframe['r_14'] < -30) & (dataframe['cti'] < 3.0) & (dataframe['adx'] > 26) & (dataframe['mama_diff_1h'] > 0.003) & (dataframe['mama'] > dataframe['fama']) & (dataframe['sma_50'] > dataframe['sma_200'] * 1.01) & (dataframe['mama_1h'] > dataframe['fama_1h'] * 1.01) & (dataframe['rsi_84'] < 55) & (dataframe['rsi_112'] < 55) & (dataframe['cti_40_1h'] < 0.73) & (dataframe['r_96_1h'] < -6) & (dataframe['mama_diff_1h'] < 0.027) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.03), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['close'] > dataframe['bb_upperband'] * 0.999) & (dataframe['rsi'] > 76), 'exit_long'] = 0
        return dataframe
