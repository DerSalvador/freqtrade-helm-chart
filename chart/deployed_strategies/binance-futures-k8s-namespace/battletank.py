...
from freqtrade.strategy import BooleanParameter, CategoricalParameter, DecimalParameter, IStrategy, IntParameter, RealParameter, merge_informative_pair, informative
...
class thetank2(IStrategy):
    ...
    # Filter Wave
    esa_length = IntParameter(3, 20, default=10, space='entry', optimize=True)
    d_length = IntParameter(3, 20, default=10, space='entry', optimize=True)
    ci_mult = DecimalParameter(0.001, 0.05, default=0.015, space='entry', optimize=True)
    wt1_length = IntParameter(6, 50, default=21, space='entry', optimize=True)
    wt2_length = IntParameter(3, 15, default=4, space='exit', optimize=True)
    # trading
    ...
    def populate_indicators_1h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # SMA
        for entry_ma in self.entry_ma_length.range:
            dataframe[f'entry_ma{entry_ma}'] = ta.SMA(dataframe['close'], timeperiod=entry_ma)
        for exit_ma in self.exit_ma_length.range:
            dataframe[f'exit_ma{exit_ma}'] = ta.SMA(dataframe['close'], timeperiod=exit_ma)
        # Parabolic SAR
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Signal: RSI crosses above 30
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bear.value) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['squeeze_ON'] == 1) & (df['200_SMA_1h'] < df['200_SMA_1h'].shift(1)) & (df['wave_t1_1h'] < self.entry_wt_bear.value) & (df['wave_t1_1h'] > df['wave_t1_1h'].shift(1)) & qtpylib.crossed_above(df['wave_t1_1h'], df['wave_t2_1h']) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'ttm_Squeeze/WT - bear')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bear.value) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['squeeze_ON'] == 1) & (df['200_SMA_1h'] < df['200_SMA_1h'].shift(1)) & (df['wave_t1_1h'] < self.entry_wt_bear.value) & (df['wave_t1_1h'] > df['wave_t1_1h'].shift(1)) & (df['wave_t1_1h'].shift(2) > df['wave_t1_1h'].shift(1)) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'ttm_Squeeze/WTT - bear')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bear.value) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['200_SMA_1h'] < df['200_SMA_1h'].shift(1)) & (df['wave_t1_1h'] < self.entry_wt_bear.value) & (df['wave_t1_1h'] > df['wave_t1_1h'].shift(1)) & (df['wave_t1_1h'].shift(2) > df['wave_t1_1h'].shift(1)) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'WT transition - bear')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bull.value) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['squeeze_ON'] == 1) & (df['200_SMA_1h'] > df['200_SMA_1h'].shift(1)) & (df['wave_t1_1h'] > df['wave_t1_1h'].shift(1)) & qtpylib.crossed_above(df['wave_t1_1h'], df['wave_t2_1h']) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'ttm_Squeeze/WT - bull')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bull.value) &
        # Make sure Volume is not 0
        df.loc[(df['squeeze_ON'] == 1) & (df['200_SMA_1h'] > df['200_SMA_1h'].shift(1)) & (df['wave_t1_1h'] > df['wave_t1_1h'].shift(1)) & (df['wave_t1_1h'] < self.entry_wt_bull.value) & (df['wave_t1_1h'].shift(2) > df['wave_t1_1h'].shift(1)) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'ttm_Squeeze/WTT - bull')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bull.value) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['200_SMA_1h'] > df['200_SMA_1h'].shift(1)) & (df['wave_t1_1h'] > df['wave_t1_1h'].shift(1)) & (df['wave_t1_1h'] < self.entry_wt_bull.value) & (df['wave_t1_1h'].shift(2) > df['wave_t1_1h'].shift(1)) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'WT transition - bull')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # (df['rsi'] >  self.entry_rsi.value) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bear.value) &
        # Make sure Volume is not 0
        df.loc[(df['200_SMA_1h'] < df['200_SMA_1h'].shift(1)) & qtpylib.crossed_above(df['rsi'], df['rsi_ma']) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'RSI-XO bear')
        # (df['close'] > df[f'entry_ma{self.entry_ma_length.value}']) &
        # # Signal: RSI crosses above 30
        # (df['rsi'] >  self.entry_rsi.value) &
        # (df['rsi_ma'] > df['rsi_ma'].shift(1)) &
        # (df['rsi'] < self.entry_rsi_bull.value) &
        # Make sure Volume is not 0
        df.loc[(df['200_SMA_1h'] > df['200_SMA_1h'].shift(1)) & qtpylib.crossed_above(df['rsi'], df['rsi_ma']) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'RSI-XO bull')
        return df
    ### EXIT CONDITIONS ###
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Signal: RSI crosses above 30
        # (df['close'] < df[f'exit_ma{self.exit_ma_length.value}']) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['squeeze_ON'] == 1) & (df['wave_t1_1h'] < df['wave_t1_1h'].shift(1)) & (df['wave_t1_1h'].shift(2) < df['wave_t1_1h'].shift(1)) & (df['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'ttm_Squeeze/WT transition')
        # Signal: RSI crosses above 30
        # (df['close'] < df[f'exit_ma{self.exit_ma_length.value}']) &
        # (df['rsi'] < df['rsi_ma']) &
        # (df['tf'] > df['close']) &
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['wave_t1_1h'] < df['wave_t1_1h'].shift(1)) & qtpylib.crossed_above(df['wave_t2_1h'], df['wave_t1_1h']) & (df['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'WT/RSI-XO')
        # (df['close'] < df[f'exit_ma{self.exit_ma_length.value}']) &
        # (df['tf'] > df['close']) &
        # Make sure Volume is not 0
...
# 2023-02-10 10:20:38,361 - freqtrade.resolvers.strategy_resolver - INFO - Strategy using ignore_entrying_expired_candle_after: 0
...
# 2023-02-10 10:20:39,264 - freqtrade.strategy.hyper - INFO - Strategy Parameter: entry_rsi = 27
# 2023-02-10 10:20:39,264 - freqtrade.strategy.hyper - INFO - Strategy Parameter: entry_rsi_bear = 45
# 2023-02-10 10:20:39,264 - freqtrade.strategy.hyper - INFO - Strategy Parameter: entry_rsi_bull = 61
...
# 2023-02-10 10:20:39,264 - freqtrade.strategy.hyper - INFO - Strategy Parameter: exit_rsi = 72
