...
class BB_RPB_3c(IStrategy):
    ...
    ##
    ##
    entry_params = {'entry_adx': 20, 'entry_fastd': 20, 'entry_fastk': 22, 'entry_ema_cofi': 0.98, 'entry_ewo_high': 4.179, 'entry_clucha_bbdelta_close': 0.01965, 'entry_clucha_bbdelta_tail': 0.95089, 'entry_clucha_closedelta_close': 0.00556, 'entry_clucha_rocr_1h': 0.54904, 'entry_gumbo_ema': 1.121, 'entry_gumbo_ewo_low': -9.442, 'entry_gumbo_cti': -0.374, 'entry_gumbo_r14': -51.971, 'entry_vwap_closedelta': 19.108, 'entry_vwap_cti': -0.022, 'entry_vwap_width': 0.392, 'entry_lambo2_ema': 0.983, 'entry_lambo2_rsi14': 44, 'entry_lambo2_rsi4': 44, 'entry_V_bb_width': 0.067, 'entry_V_cti': -0.672, 'entry_V_mfi': 38.796, 'entry_V_r14': -53.601}
    # exit space
    # disable
    exit_params = {'pHSL': -1, 'pPF_1': 0.014, 'pPF_2': 0.072, 'pSL_1': 0.011, 'pSL_2': 0.063, 'base_nb_candles_exit': 23, 'high_offset': 1.051, 'high_offset_2': 1.02}
    # ROI
    minimal_roi = {'0': 0.1}
    ...
    entry_lambo2_ema = DecimalParameter(0.85, 1.15, default=0.942, optimize=is_optimize_lambo2)
    entry_lambo2_rsi4 = IntParameter(15, 45, default=45, optimize=is_optimize_lambo2)
    entry_lambo2_rsi14 = IntParameter(15, 45, default=45, optimize=is_optimize_lambo2)
    is_optimize_V = False
    entry_V_bb_width = DecimalParameter(0.04, 0.1, default=0.01, optimize=is_optimize_V)
    ...
    pHSL = DecimalParameter(-0.35, -0.04, default=-0.08, decimals=3, space='exit', load=True, optimize=False)
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(0.008, 0.02, default=0.016, decimals=3, space='exit', load=True, optimize=False)
    pSL_1 = DecimalParameter(0.008, 0.02, default=0.011, decimals=3, space='exit', load=True, optimize=False)
    # profit threshold 2, SL_2 is used
    pPF_2 = DecimalParameter(0.04, 0.1, default=0.08, decimals=3, space='exit', load=True, optimize=False)
    pSL_2 = DecimalParameter(0.02, 0.07, default=0.04, decimals=3, space='exit', load=True, optimize=False)
    ############################################################################
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_clucHA = (dataframe['rocr_1h'] > self.entry_clucha_rocr_1h.value) & ((dataframe['bb_lowerband2_40'].shift() > 0) & (dataframe['bb_delta_cluc'] > dataframe['ha_close'] * self.entry_clucha_bbdelta_close.value) & (dataframe['ha_closedelta'] > dataframe['ha_close'] * self.entry_clucha_closedelta_close.value) & (dataframe['tail'] < dataframe['bb_delta_cluc'] * self.entry_clucha_bbdelta_tail.value) & (dataframe['ha_close'] < dataframe['bb_lowerband2_40'].shift()) & (dataframe['ha_close'] < dataframe['ha_close'].shift()))  # Modified from gumbo1, creadit goes to original author @raph92
        is_gumbo = (dataframe['EWO'] < self.entry_gumbo_ewo_low.value) & (dataframe['bb_middleband2_1h'] >= dataframe['T3_1h']) & (dataframe['T3'] <= dataframe['ema_8'] * self.entry_gumbo_ema.value) & (dataframe['cti'] < self.entry_gumbo_cti.value) & (dataframe['r_14'] < self.entry_gumbo_r14.value)
        is_vwap = (dataframe['close'] < dataframe['vwap_lowerband']) & (dataframe['vwap_width'] > self.entry_vwap_width.value) & (dataframe['closedelta'] > dataframe['close'] * self.entry_vwap_closedelta.value / 1000) & (dataframe['cti'] < self.entry_vwap_cti.value)
        is_lambo_2 = (dataframe['close'] < dataframe['ema_14'] * self.entry_lambo2_ema.value) & (dataframe['rsi_fast'] < self.entry_lambo2_rsi4.value) & (dataframe['rsi'] < self.entry_lambo2_rsi14.value)
        is_V = (dataframe['bb_width'] > self.entry_V_bb_width.value) & (dataframe['cti'] < self.entry_V_cti.value) & (dataframe['r_14'] < self.entry_V_r14.value) & (dataframe['mfi'] < self.entry_V_mfi.value)
        is_additional_check = (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & (dataframe['rsi_42_1h'] < 56) & (dataframe['volume'] > 0)
        # condition append
        conditions.append(is_cofi)  # ~3.21 90.8%
        dataframe.loc[is_cofi, 'enter_tag'] += 'cofi '
        conditions.append(is_clucHA)  # ~68.2%
        dataframe.loc[is_clucHA, 'enter_tag'] += 'cluc '
        ...
        dataframe.loc[is_lambo_2, 'enter_tag'] += 'lambo2 '
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if conditions:
            dataframe.loc[reduce(lambda x, y: x | y, conditions), 'exit_long'] = 0  #disable
        return dataframe
