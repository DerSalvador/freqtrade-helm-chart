...
class PRICEFOLLOWING2(IStrategy):
    ...
    minimal_roi = {'60': 0.025, '30': 0.03, '0': 0.04}
    ...
    rsi_value = IntParameter(low=1, high=50, default=30, space='entry', optimize=True, load=True)
    rsi_enabled = BooleanParameter(default=False, space='entry', optimize=True, load=True)
    ema_pct = DecimalParameter(0.0001, 0.1, decimals=4, default=0.004, space='entry', optimize=True)
    ema_exit_pct = DecimalParameter(0.0001, 0.1, decimals=4, default=0.003, space='exit', optimize=True, load=True)
    exit_rsi_value = IntParameter(low=25, high=100, default=70, space='exit', optimize=True, load=True)
    ...
    plot_config = {'main_plot': {'tema': {}, 'ema7': {}, 'ha_open': {}, 'ha_close': {}}, 'subplots': {'MACD': {'macd': {'color': 'blue'}, 'macdsignal': {'color': 'orange'}}, 'RSI': {'rsi': {'color': 'red'}}}}
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.exit_rsi_enabled.value:
            conditions.append(dataframe['rsi'] < self.exit_rsi_value.value)
            ...
            conditions.append((last_tema - last_ema7) / last_ema7 < self.ema_exit_pct.value)
        if conditions:
