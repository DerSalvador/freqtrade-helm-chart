...
##   the value you need to prevent your strat to entry.                                                    ##
...
##   - add a line to each of your entry conditions to check if the coins was resently pump and didn't      ##
##     recovered yet                                                                                     ##
##                                                                                                       ##
##   - add a confirm_trade_entry condition (easier for a strat like NFI with 50 entry conditions           ##
###########################################################################################################
...
class YourStratName(IStrategy):
    ...
    pump_period = IntParameter(
        5, 24, default=entry_params['pump_period'], space='entry', optimize=False)
    pump_limit = IntParameter(
        100,10000, default=entry_params['pump_limit'], space='entry', optimize=True)
    pump_recorver_price = DecimalParameter(
        1.0, 1.3, default=entry_params['pump_recorver_price'], space='entry', optimize=True)
    pump_pause_duration = IntParameter(
        6, 500, default=entry_params['pump_pause_duration'], space='entry', optimize=True)
...
########## Your Strat parameters, has no effect on pump detection ##########
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(
            (
                (Condition_11 & (Condition_12 | Conditon_13)) & # you first entry condition
                dataframe['entry_ok']
            )
        )
        ...
        conditions.append(
            (
                (Condition_21 & (Condition_22 | Conditon_23) & # you second entry condition
                dataframe['entry_ok']
            )
        )
        # etc & etc
        ...
        conditions.append(
            (
                (Condition_n1 & (Condition_n2 | Conditon_n3) & # you n entry condition
                dataframe['entry_ok']
            )
        )
        if conditions:
...
### Confirm trade entry is to use only if the dataframe['entry_ok'] test was not added in populate_entry_trend (in case of too many entry conditions like for NFI)
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
                            time_in_force: str, current_time: datetime, **kwargs) -> bool:
        dataframe, _ = self.dp.get_analyzed_dataframe(pair=pair, timeframe=self.timeframe)
        current_candle = dataframe.iloc[-1].squeeze()
        
        if current_candle['entry_ok'] :
            return True
        else:
            return False
