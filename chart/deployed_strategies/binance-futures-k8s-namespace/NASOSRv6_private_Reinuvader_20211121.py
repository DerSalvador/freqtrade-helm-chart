...
entry_params = {'base_nb_candles_entry': 17, 'ewo_high': 3.33, 'ewo_high_2': -0.444, 'ewo_low': -12.788, 'lookback_candles': 1, 'low_offset': 0.985, 'low_offset_2': 0.935, 'profit_threshold': 1.016, 'clucha_rocr_1h': 0.47782, 'clucha_bbdelta_close': 0.02206, 'clucha_bbdelta_tail': 1.02515, 'clucha_close_bblower': 0.03669, 'clucha_closedelta_close': 0.04401, 'base_nb_candles_entry_zema': 25, 'base_nb_candles_entry_zema2': 53, 'low_offset_zema': 0.958, 'low_offset_zema2': 0.961, 'base_nb_candles_entry_ema': 9, 'base_nb_candles_entry_ema2': 75, 'low_offset_ema': 1.067, 'low_offset_ema2': 0.973, 'high_offset_exit_ema': 0.994, 'rsi_entry': 71}
# Sell hyperspace params:
exit_params = {'base_nb_candles_exit': 8, 'high_offset': 1.01, 'high_offset_2': 1.406, 'ProfitLoss1': 0.005, 'ProfitLoss2': 0.021, 'ProfitMargin1': 0.018, 'ProfitMargin2': 0.051, 'pHSL': -0.15}
...
class NASOSRv6_private_Reinuvader_20211121(IStrategy):
    ...
    stoploss = -0.15
    # SMAOffset
    base_nb_candles_entry = IntParameter(2, 26, default=entry_params['base_nb_candles_entry'], space='entry', optimize=False)
    base_nb_candles_exit = IntParameter(2, 20, default=exit_params['base_nb_candles_exit'], space='exit', optimize=False)
    low_offset = DecimalParameter(0.85, 0.99, default=entry_params['low_offset'], space='entry', optimize=False)
    ...
    profit_threshold = DecimalParameter(1.0, 1.08, default=entry_params['profit_threshold'], space='entry', optimize=False)
    ...
    rsi_entry = IntParameter(60, 82, default=entry_params['rsi_entry'], space='entry', optimize=False)
    # trailing stoploss hyperopt parameters
    pHSL = DecimalParameter(-0.15, -0.08, default=exit_params['pHSL'], decimals=3, space='exit', optimize=True)
    ProfitMargin1 = DecimalParameter(0.009, 0.019, default=exit_params['ProfitMargin1'], decimals=3, space='exit', optimize=True)
    ProfitLoss1 = DecimalParameter(0.005, 0.012, default=exit_params['ProfitLoss1'], decimals=3, space='exit', optimize=True)
    ProfitMargin2 = DecimalParameter(0.033, 0.099, default=exit_params['ProfitMargin2'], decimals=3, space='exit', optimize=True)
    ProfitLoss2 = DecimalParameter(0.01, 0.025, default=exit_params['ProfitLoss2'], decimals=3, space='exit', optimize=True)
    # ClucHA
    clucha_bbdelta_close = DecimalParameter(0.01, 0.05, default=entry_params['clucha_bbdelta_close'], decimals=5, space='entry', optimize=False)
    clucha_bbdelta_tail = DecimalParameter(0.7, 1.2, default=entry_params['clucha_bbdelta_tail'], decimals=5, space='entry', optimize=False)
    clucha_close_bblower = DecimalParameter(0.001, 0.05, default=entry_params['clucha_close_bblower'], decimals=5, space='entry', optimize=False)
    clucha_closedelta_close = DecimalParameter(0.001, 0.05, default=entry_params['clucha_closedelta_close'], decimals=5, space='entry', optimize=False)
    clucha_rocr_1h = DecimalParameter(0.1, 1.0, default=entry_params['clucha_rocr_1h'], decimals=5, space='entry', optimize=False)
    # Zema
    base_nb_candles_entry_zema = IntParameter(5, 80, default=entry_params['base_nb_candles_entry_zema'], space='entry', optimize=False)
    low_offset_zema = DecimalParameter(0.9, 0.99, default=entry_params['low_offset_zema'], space='entry', optimize=False)
    base_nb_candles_entry_zema2 = IntParameter(5, 80, default=entry_params['base_nb_candles_entry_zema2'], space='entry', optimize=False)
    low_offset_zema2 = DecimalParameter(0.9, 0.99, default=entry_params['low_offset_zema2'], space='entry', optimize=False)
    high_offset_exit_ema = DecimalParameter(0.99, 1.1, default=entry_params['high_offset_exit_ema'], space='entry', optimize=False)
    base_nb_candles_entry_ema = IntParameter(5, 80, default=entry_params['base_nb_candles_entry_ema'], space='entry', optimize=False)
    low_offset_ema = DecimalParameter(0.9, 1.1, default=entry_params['low_offset_ema'], space='entry', optimize=False)
    base_nb_candles_entry_ema2 = IntParameter(5, 80, default=entry_params['base_nb_candles_entry_ema2'], space='entry', optimize=False)
    low_offset_ema2 = DecimalParameter(0.9, 1.1, default=entry_params['low_offset_ema2'], space='entry', optimize=False)
    # Trailing stop:
    trailing_stop = False
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float, time_in_force: str, exit_reason: str, current_time: datetime, **kwargs) -> bool:
        ...
        if slippage < self.slippage_protection['max_slippage']:
            ...
            if pair_retries < self.slippage_protection['retries']:
                ...
                return False
        state[pair] = 0
        if hasattr(last_candle, 'exit_tag') and str(last_candle.exit_tag) != 'nan':
            if exit_reason == 'exit_signal':
                exit_reason = last_candle.exit_tag
            else:
                exit_reason += last_candle.exit_tag
        trade.exit_reason = f'{trade.entry_tag}->{exit_reason}'
        return True
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['ema_exit'] = ta.EMA(dataframe, 5)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # don't entry if there isn't some profit to be made
        ...
        dont_entry_conditions.append(dataframe['trendline'] < 0.995)
        dont_entry_conditions.append(dataframe['relative_price'] > 0.51)
        dataframe.loc[(dataframe['rsi_fast'] < 35) & (dataframe['rsi_36'] < 0.45) & (dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset.value), ['entry', 'entry_tag']] = (1, 'ewo1')
        ...
        #         (dataframe['uptrend'] < 1)
        #     ),
        #     ['entry', 'entry_tag']] = (1, 'MacD')
        if dont_entry_conditions:
            for condition in dont_entry_conditions:
                dataframe.loc[condition, 'entry'] = 0
        # This does not fit well the protections for EWO entrys.
        dataframe.loc[(dataframe['volume'] > 0) & dataframe['rocr_1h'].gt(self.clucha_rocr_1h.value) & (dataframe['lower'].shift().gt(0) & dataframe['bbdelta'].gt(dataframe['ha_close'] * self.clucha_bbdelta_close.value) & dataframe['ha_closedelta'].gt(dataframe['ha_close'] * self.clucha_closedelta_close.value) & dataframe['tail'].lt(dataframe['bbdelta'] * self.clucha_bbdelta_tail.value) & dataframe['ha_close'].lt(dataframe['lower'].shift()) & dataframe['ha_close'].le(dataframe['ha_close'].shift()) | (dataframe['ha_close'] < dataframe['hema_slow']) & (dataframe['ha_close'] < self.clucha_close_bblower.value * dataframe['lower'])), ['entry', 'entry_tag']] = (1, 'clucHA')
        # This does not fit well the protections for EWO entrys.
        dataframe.loc[((dataframe['close'] < dataframe['zema_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['zema_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh'])) & (dataframe['volume'] > 0) & (dataframe['long_term_price_warning'] < 1) & (dataframe['trendline'] > 0.996) & (dataframe['btc_rsi'] > 0.49) & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_exit'] * self.high_offset_exit_ema.value) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < 35) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & ((dataframe['close'] < dataframe['ema_offset_entry']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['EWO'] < -19.632) | (dataframe['EWO'] > 2.615) & (dataframe['rsi'] < 60)) | (dataframe['close'] < dataframe['ema_offset_entry2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['EWO'] < -19.955) | (dataframe['EWO'] > 2.188) & (dataframe['rsi'] < 45))), ['entry', 'entry_tag']] = (1, 'zema')
        return dataframe
