...
class cryptotankV2(IStrategy):
    ...
    entry_rsi_ma_length = IntParameter(10, 14, default=14, space='entry', optimize=True)
    reference_ma_length = IntParameter(180, 200, default=200, space='entry', optimize=True)
    smoothing_length = IntParameter(25, 50, default=35, space='entry', optimize=True)
    filterlength = IntParameter(low=8, high=40, default=35, space='exit', optimize=True)
    entry_offset1 = DecimalParameter(low=0.9, high=0.99, decimals=2, default=0.96, space='entry', optimize=True, load=True)
    entry_offset2 = DecimalParameter(low=0.9, high=0.96, decimals=2, default=0.94, space='entry', optimize=True, load=True)
    exit_offset = DecimalParameter(low=1.01, high=1.1, decimals=2, default=1.08, space='exit', optimize=True, load=True)
    #trailing stop loss optimiziation
    tsl_target5 = DecimalParameter(low=0.15, high=0.2, decimals=2, default=0.3, space='exit', optimize=True, load=True)
    ts5 = DecimalParameter(low=0.04, high=0.06, default=0.05, decimals=2, space='exit', optimize=True, load=True)
    tsl_target4 = DecimalParameter(low=0.1, high=0.15, default=0.2, decimals=2, space='exit', optimize=True, load=True)
    ts4 = DecimalParameter(low=0.03, high=0.05, default=0.045, decimals=2, space='exit', optimize=True, load=True)
    tsl_target3 = DecimalParameter(low=0.08, high=0.1, default=0.15, decimals=2, space='exit', optimize=True, load=True)
    ts3 = DecimalParameter(low=0.025, high=0.04, default=0.035, decimals=3, space='exit', optimize=True, load=True)
    tsl_target2 = DecimalParameter(low=0.06, high=0.8, default=0.1, decimals=3, space='exit', optimize=True, load=True)
    ts2 = DecimalParameter(low=0.015, high=0.03, default=0.02, decimals=3, space='exit', optimize=True, load=True)
    tsl_target1 = DecimalParameter(low=0.04, high=0.06, default=0.06, decimals=3, space='exit', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.01, high=0.016, default=0.013, decimals=3, space='exit', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.02, high=0.04, default=0.03, decimals=3, space='exit', optimize=True, load=True)
    ts0 = DecimalParameter(low=0.005, high=0.015, default=0.013, decimals=3, space='exit', optimize=True, load=True)
    ### entrying 
    # rsi
    entry_rsi_upper = IntParameter(55, 65, default=55, space='entry', optimize=True)
    entry_rsi_lower = IntParameter(20, 35, default=35, space='entry', optimize=True)
    entry_rsi_ma_slope = DecimalParameter(-0.01, 0.1, default=0, decimals=2, space='entry', optimize=True)
    entry_ma_slope = DecimalParameter(-0.1, 0.1, default=0, decimals=2, space='entry', optimize=True)
    ...
    entry_dip_x = DecimalParameter(0.5, 2.0, default=1.5, decimals=1, space='entry', optimize=True)
    ### exiting
    # # rsi
    # exit_rsi_upper = IntParameter(80, 100, default=80, space="exit", optimize=True)
    # exit_rsi_lower = IntParameter(50, 79, default=55, space="exit", optimize=True)
    # exit_ma_slope = IntParameter(-10, 10, default=0, space="exit", optimize=True)
    # {
    #   "main_plot": {
    #     "enter_tag": {
    #       "color": "#6fc96b",
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        #         (df[f'entry_rsi_{self.entry_rsi_length.value}'] < self.entry_rsi_upper.value) &
        #         (df['volume'] > 0)  # Make sure Volume is not 0
        #     ),
        #     ['enter_long', 'enter_tag']] = (1, 'XO ')
        # Make sure Volume is not 0
        df.loc[qtpylib.crossed_above(df['change'], df[f'smooth_change_{self.smoothing_length.value}']) & (df['smooth_ma_slope'] > self.entry_ma_slope.value) & (df['close'] < df[f'reference_ma_{self.reference_ma_length.value}'] * self.exit_offset.value) & (df[f'smooth_change_{self.smoothing_length.value}'] > 0) & (df[f'rsi_ma_slope'] > self.entry_rsi_ma_slope.value) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'XO Above 0')
        #WIP
        # Make sure Volume is not 0
        df.loc[qtpylib.crossed_above(df['change'], df[f'smooth_change_{self.smoothing_length.value}']) & (df['close'] < df[f'reference_ma_{self.reference_ma_length.value}']) & (df[f'smooth_change_{self.smoothing_length.value}'] < self.entry_lower.value * 2) & (df[f'entry_rsi_{self.entry_rsi_length.value}'] < self.entry_rsi_dip.value) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'XO Below Lower Entry * 2')
        # df.loc[
        #     (
        #         (qtpylib.crossed_above(df['change'], df[f'smooth_change_{self.smoothing_length.value}'])) &
        #         (df['smooth_ma_slope'] > self.entry_ma_slope.value) &
        #         (df[f'smooth_change_{self.smoothing_length.value}'] < self.entry_lower.value) &
        #         (df['close'] < df[f'reference_ma_{self.reference_ma_length.value}'] * self.entry_offset1.value) &
        #         (df['volume'] > 0)  # Make sure Volume is not 0
        #     ),
        ...
        #         (df['smooth_ma_slope'] > self.entry_ma_slope.value) &
        #         (df[f'smooth_change_{self.smoothing_length.value}'] < self.entry_lower.value) &
        #         (df['close'] < df[f'reference_ma_{self.reference_ma_length.value}'] * self.entry_offset2.value) &
        #         (df['volume'] > 0)  # Make sure Volume is not 0
        #     ),
        #     ['enter_long', 'enter_tag']] = (1, 'XO Below Lower and OFFSET2')
        # Make sure Volume is not 0
        df.loc[qtpylib.crossed_above(df[f'entry_rsi_{self.entry_rsi_length.value}'], df[f'entry_rsi_ma_{self.entry_rsi_ma_length.value}']) & (df[f'entry_rsi_{self.entry_rsi_length.value}'] < self.entry_rsi_dip.value) & (df['change'] < df[f'smooth_change_{self.smoothing_length.value}'] - self.entry_dip_x.value) & (df['change'] < self.entry_lower.value) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'RSI XO | Change < Smooth + Dip < 0')  # r3..
        # Make sure Volume is not 0
        df.loc[qtpylib.crossed_above(df['close'], df['r3']) & (df['change'] > df[f'smooth_change_{self.smoothing_length.value}']) & (df[f'entry_rsi_{self.entry_rsi_length.value}'] < self.entry_rsi_upper.value) & (df['close'] > df[f'reference_ma_{self.reference_ma_length.value}']) & (df['change'] > 0) & (df['change'] > df['change'].shift(1)) & (df[f'entry_rsi_{self.entry_rsi_length.value}'] > df[f'entry_rsi_{self.entry_rsi_length.value}'].shift(1)) & (df[f'entry_rsi_ma_{self.entry_rsi_ma_length.value}'] > df[f'entry_rsi_ma_{self.entry_rsi_ma_length.value}'].shift(1)) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'MEGA PUMP')
        # df.loc[
        #     (
        #         (df[f'entry_rsi_{self.entry_rsi_length.value}'] < self.entry_rsi_dip.value) &
        #         (df['close'] < (df[f'reference_ma_{self.reference_ma_length.value}'] * self.entry_offset2.value)) &
        #         (df['change'] < self.entry_lower.value) &
        #         (df['volume'] > 0)  # Make sure Volume is not 0
        #     ),
