...
class BB_RPB_TSL_SMA_Tranz_TB_1_1_1(IStrategy):
    ...
    ##
    ##DIAMOND
    entry_params = {'max_slip': 0.983, 'entry_bb_width_1h': 0.954, 'entry_roc_1h': 86, 'entry_threshold': 0.003, 'entry_bb_factor': 0.999, 'entry_bb_delta': 0.025, 'entry_bb_width': 0.095, 'entry_cci': -116, 'entry_cci_length': 25, 'entry_rmi': 49, 'entry_rmi_length': 17, 'entry_srsi_fk': 32, 'entry_closedelta': 17.922, 'entry_ema_diff': 0.026, 'entry_ema_high': 0.968, 'entry_ema_low': 0.935, 'entry_ewo': -5.001, 'entry_rsi': 23, 'entry_rsi_fast': 44, 'entry_closedelta_local_dip': 12.044, 'entry_ema_diff_local_dip': 0.024, 'entry_ema_high_local_dip': 1.014, 'entry_rsi_local_dip': 21, 'entry_r_deadfish_bb_factor': 1.014, 'entry_r_deadfish_bb_width': 0.299, 'entry_r_deadfish_ema': 1.054, 'entry_r_deadfish_volume_factor': 1.59, 'entry_r_deadfish_cti': -0.115, 'entry_r_deadfish_r14': -44.34, 'entry_clucha_bbdelta_close': 0.04796, 'entry_clucha_bbdelta_tail': 0.93112, 'entry_clucha_close_bblower': 0.01645, 'entry_clucha_closedelta_close': 0.00931, 'entry_clucha_rocr_1h': 0.41663, 'entry_ema_high_2': 1.04116, 'entry_ema_low_2': 0.97463, 'entry_ewo_high_2': 5.249, 'entry_rsi_ewo_2': 35, 'entry_rsi_fast_ewo_2': 45, 'entry_adx': 13, 'entry_cofi_r14': -85.016, 'entry_cofi_cti': -0.892, 'entry_ema_cofi': 1.147, 'entry_ewo_high': 8.594, 'entry_fastd': 28, 'entry_fastk': 39, 'entry_gumbo_ema': 1.121, 'entry_gumbo_ewo_low': -9.442, 'entry_gumbo_cti': -0.374, 'entry_gumbo_r14': -51.971, 'entry_sqzmom_ema': 0.981, 'entry_sqzmom_ewo': -3.966, 'entry_sqzmom_r14': -45.068, 'entry_nfix_49_cti': -0.105, 'entry_nfix_49_r14': -81.827, 'entry_fast': 31, 'entry_push': 0.72, 'entry_shift': -7, 'entry_slow': 2}
    #############################################################
    ##
    ##
    ##
    ...
    minimal_roi = {'0': 0.205, '81': 0.038, '292': 0.005}
    ...
    entry_push = DecimalParameter(0, 2, decimals=3, default=1, optimize=is_diamond_protection)
    entry_shift = IntParameter(-10, 0, default=-6, optimize=is_diamond_protection)
    entry_fast = IntParameter(2, 50, default=9, optimize=is_diamond_protection)
    entry_slow = IntParameter(2, 50, default=18, optimize=is_diamond_protection)
    #NFI 7 SMA
    ...
    exit_push = DecimalParameter(0, 2, decimals=3, default=1, optimize=is_optimize_diamondprofit)
    exit_shift = IntParameter(-10, 0, default=-6, optimize=is_optimize_diamondprofit)
    exit_fast = IntParameter(2, 50, default=9, optimize=is_optimize_diamondprofit)
    exit_slow = IntParameter(2, 50, default=18, optimize=is_optimize_diamondprofit)
    # Sell
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.entry_12_protection__ema_fast.value:
            entry_12_protections.append(dataframe[f'ema_{self.entry_12_protection__ema_fast_len.value}'] > dataframe['ema_200']) & entry_12_protections.append(dataframe[f'ema_{self.entry_12_protection__ema_slow_len.value}_1h'] > dataframe['ema_200_1h']) & entry_12_protections.append(dataframe['close'] > dataframe[f'ema_{self.entry_12_protection__close_above_ema_fast_len.value}']) & entry_12_protections.append(dataframe['close'] > dataframe[f'ema_{self.entry_12_protection__close_above_ema_slow_len.value}_1h']) & entry_12_protections.append(dataframe['sma_200'] > dataframe['sma_200'].shift(int(self.entry_12_protection__sma200_rising_val.value))) & entry_12_protections.append(dataframe['sma_200_1h'] > dataframe['sma_200_1h'].shift(int(self.entry_12_protection__sma200_1h_rising_val.value))) & entry_12_protections.append(dataframe[f'safe_dips_{self.entry_12_protection__safe_dips_type.value}']) & entry_12_protections.append(dataframe[f'safe_pump_{self.entry_12_protection__safe_pump_period.value}_{self.entry_12_protection__safe_pump_type.value}_1h'])
        ...
        is_sqzOff = (dataframe['bb_lowerband2'] < dataframe['kc_lowerband_28_1']) & (dataframe['bb_upperband2'] > dataframe['kc_upperband_28_1'])  # from BinH
        ...
        is_nfix_12 = (dataframe['close'] < dataframe['ema_20'] * 0.932) & (dataframe['EWO'] > 0.1) & (dataframe['rsi_14'] < 40.0) & (dataframe['cti'] < -0.9) & (dataframe['r_480_1h'] < -20.0)
        ...
        is_nfi7_33 = dataframe['moderi_96'] & (dataframe['cti'] < -0.88) & (dataframe['close'] < dataframe['ema_13'] * 0.988) & (dataframe['EWO'] > 6.4) & (dataframe['rsi'] < 32.0) & (dataframe['volume'] < dataframe['volume_mean_4'] * 2.0)
        is_nfi7_37 = (dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['close'] < dataframe['sma_75'] * 0.98) & (dataframe['EWO'] > 9.8) & (dataframe['rsi'] < 56.0) & (dataframe['cti'] < -0.7) & dataframe['safe_dump_50_1h']
        ## BB MODDED
        is_nfi_ctt35 = (dataframe['pm'] <= dataframe['pmax_thresh']) & (dataframe['close'] < dataframe['sma_75'] * 0.984) & (dataframe['EWO'] > 9.6) & (dataframe['rsi_14'] < 32.0) & (dataframe['cti'] < -0.5)
        is_nfi_ctt25 = (dataframe['rsi_20'] < dataframe['rsi_20'].shift()) & (dataframe['rsi_4'] < self.entry_25_rsi_4.value) & (dataframe['ema_20_1h'] > dataframe['ema_26_1h']) & (dataframe['close'] < dataframe['sma_20'] * self.entry_25_ma_offset.value) & (dataframe['open'] > dataframe['sma_20'] * self.entry_25_ma_offset.value) & ((dataframe['open'] < dataframe['ema_20_1h']) & (dataframe['low'] < dataframe['ema_20_1h']) | (dataframe['open'] > dataframe['ema_20_1h']) & (dataframe['low'] > dataframe['ema_20_1h'])) & (dataframe['cti'] < self.entry_25_cti.value)
        is_nfi_ctt15 = (dataframe['close'] > dataframe['ema_200_1h'] * self.entry_ema_rel_15.value) & (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_open_mult_15.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['rsi_14'] < self.entry_rsi_15.value) & (dataframe['close'] < dataframe['ema_20'] * self.entry_ma_offset_15.value)
        is_nfix_39 = (dataframe['ema_200'] > dataframe['ema_200'].shift(12) * 1.01) & (dataframe['ema_200'] > dataframe['ema_200'].shift(48) * 1.07) & (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & dataframe['bb_lowerband2_40'].shift().gt(0) & dataframe['bb_delta_cluc'].gt(dataframe['close'] * 0.056) & dataframe['closedelta'].gt(dataframe['close'] * 0.01) & dataframe['tail'].lt(dataframe['bb_delta_cluc'] * 0.5) & dataframe['close'].lt(dataframe['bb_lowerband2_40'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['close'] > dataframe['ema_13'] * self.entry_nfix_39_ema.value)
        is_nfi_9 = (dataframe['ema_50'] > dataframe['ema_200']) & (dataframe['close'] < dataframe['ema_20'] * 0.968) & (dataframe['close'] < dataframe['bb20_2_low'] * 0.982) & (dataframe['mfi'] < 50.0) & (dataframe['cti'] < -0.85) & (dataframe['r_14'] < -94.0) & (dataframe['rsi_14_1h'] > 20.0) & (dataframe['rsi_14_1h'] < 88.0) & (dataframe['crsi_1h'] > 21.0)
        # EDIT ME MAYBE? this was ema fast = SMA now i made it ema_fast is larger then SMA
        is_diamond = (dataframe['ema_20'] > ta.SMA(dataframe, timeperiod=int(self.entry_fast.value))) & (dataframe['ema_25'] > ta.SMA(dataframe, timeperiod=int(self.entry_slow.value)))
        is_nfi_26 = (dataframe['close'] < dataframe['zema_61'] * 0.9405) & (dataframe['cti'] < -0.72) & (dataframe['cci'] < -166.0) & (dataframe['r_14'] < -98.0) & (dataframe['cti_1h'] < 0.95) & (dataframe['volume'] < dataframe['volume_mean_4'] * 2.0)
        is_nfix_54 = (dataframe['ema_200'] > dataframe['ema_200'].shift(12) * 1.01) & (dataframe['ema_200'] > dataframe['ema_200'].shift(48) * 1.07) & dataframe['bb40_2_low'].shift().gt(0) & dataframe['bb40_2_delta'].gt(dataframe['close'] * 0.056) & dataframe['closedelta'].gt(dataframe['close'] * 0.01) & dataframe['tail'].lt(dataframe['bb40_2_delta'] * 0.5) & dataframe['close'].lt(dataframe['bb40_2_low'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['close'] > dataframe['ema_50'] * 0.925)
        is_nfix_53 = (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & (dataframe['ema_200_1h'].shift(24) > dataframe['ema_200_1h'].shift(36)) & (dataframe['ema_26_15m'] > dataframe['ema_12_15m']) & (dataframe['ema_26_15m'] - dataframe['ema_12_15m'] > dataframe['open_15m'] * 0.02) & (dataframe['ema_26_15m'].shift(3) - dataframe['ema_12_15m'].shift(3) > dataframe['open_15m'] / 100) & (dataframe['close_15m'] < dataframe['bb20_2_low_15m'] * 0.99) & (dataframe['r_14'] < -90.0) & (dataframe['cti_1h'] > -0.7)
        is_nfix_52 = (dataframe['ema_26_15m'] > dataframe['ema_12_15m']) & (dataframe['ema_26_15m'] - dataframe['ema_12_15m'] > dataframe['open_15m'] * 0.032) & (dataframe['ema_26_15m'].shift(3) - dataframe['ema_12_15m'].shift(3) > dataframe['open_15m'] / 100) & (dataframe['close_15m'] < dataframe['bb20_2_low_15m'] * 0.998) & (dataframe['crsi_1h'] > 10.0)
        is_nfix_51 = (dataframe['close_15m'] < dataframe['ema_16_15m'] * 0.944) & (dataframe['ewo_15m'] < -1.0) & (dataframe['rsi_14_15m'] > 28.0) & (dataframe['cti_15m'] < -0.84) & (dataframe['r_14_15m'] < -94.0) & (dataframe['rsi_14'] > 30.0) & (dataframe['crsi_1h'] > 1.0)
        is_nfix_48 = (dataframe['close_15m'].shift(3) < dataframe['sma_15_15m'].shift(3) * 0.95) & (dataframe['close_15m'] > dataframe['open_15m'].shift(3)) & (dataframe['ewo_15m'] > 2.8) & (dataframe['cti_15m'] < -0.75) & (dataframe['r_14_15m'].shift(3) < -94.0) & (dataframe['cti'] < -0.5) & (dataframe['cti_1h'] < 0.1) & (dataframe['crsi_1h'] > 18.0)
        is_nfix_47 = (dataframe['rsi_14_15m'] < dataframe['rsi_14_15m'].shift(3)) & (dataframe['ema_20_1h'] > dataframe['ema_25_1h']) & (dataframe['close_15m'] < dataframe['sma_15_15m'] * 0.95) & ((dataframe['open_15m'] < dataframe['ema_20_1h']) & (dataframe['low_15m'] < dataframe['ema_20_1h']) | (dataframe['open_15m'] > dataframe['ema_20_1h']) & (dataframe['low_15m'] > dataframe['ema_20_1h'])) & (dataframe['cti_15m'] < -0.9) & (dataframe['r_14_15m'] < -90.0) & (dataframe['r_14'] < -97.0) & (dataframe['cti_1h'] < 0.1) & (dataframe['crsi_1h'] > 8.0)
        is_nfix_41 = (dataframe['ema_12_15m'] > dataframe['ema_200_1h']) & (dataframe['ema_26_15m'] > dataframe['ema_12_15m']) & (dataframe['ema_26_15m'] - dataframe['ema_12_15m'] > dataframe['open_15m'] * 0.03) & (dataframe['ema_26_15m'].shift(3) - dataframe['ema_12_15m'].shift(3) > dataframe['open_15m'] / 100) & (dataframe['close_15m'] < dataframe['bb20_2_low_15m'] * 0.99)
        is_nfix_38 = (dataframe['ema_200'] > dataframe['ema_200'].shift(12) * 1.01) & (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * 0.02) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['mfi'] < 34.5) & (dataframe['r_64'] < -65.0) & (dataframe['r_96'] < -50.0) & (dataframe['r_480_1h'] < -1.0)
        is_nfix_36 = (dataframe['ema_200'] > dataframe['ema_200'].shift(36) * 1.035) & (dataframe['close'] < dataframe['ema_20'] * 0.956) & (dataframe['rsi_14'] < 34.0) & (dataframe['r_64'] < -80.0) & (dataframe['cti'] < -0.5) & (dataframe['r_480_1h'] < -30.0)
        is_nfix_204 = (dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['close'] < dataframe['sma_75'] * 0.98) & (dataframe['EWO'] < -4.4) & (dataframe['cti'] < -0.95) & (dataframe['r_14'] < -97.0) & (dataframe['crsi_1h'] > 0.5)
        is_nfix_203 = (dataframe['ema_50_1h'] > dataframe['ema_100_1h']) & (dataframe['close'] < dataframe['sma_30'] * 0.99) & (dataframe['cti'] < -0.92) & (dataframe['EWO'] < -6.0) & (dataframe['cti_1h'] < -0.88) & (dataframe['crsi_1h'] > 10.0)
        is_nfix_202 = (dataframe['close'] > dataframe['ema_200_1h'] * 0.84) & (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * 0.02) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb20_2_low'] * 0.999) & (dataframe['cti'] < -0.5) & (dataframe['rsi_14'] > 25.0) & (dataframe['mfi'] > 18.0) & (dataframe['r_14'] < -94.0) & (dataframe['r_14'].shift(1) < -94.0) & (dataframe['crsi_1h'] > 12.0) & (dataframe['volume'] < dataframe['volume_mean_4'] * 1.6)
        is_nfix_201 = (dataframe['rsi_20'] < dataframe['rsi_20'].shift()) & (dataframe['rsi_4'] < 30.0) & (dataframe['ema_20_1h'] > dataframe['ema_26_1h']) & (dataframe['close'] < dataframe['sma_15'] * 0.953) & (dataframe['cti'] < -0.78) & (dataframe['cci'] < -200.0)
        is_nfix_34 = (dataframe['close'] < dataframe['ema_50']) & (dataframe['close'] < dataframe['bb20_2_low'] * 0.972) & (dataframe['cti'] < -0.8) & (dataframe['rsi_14'] < 18.0)
        is_nfix_28 = (dataframe['close'] < dataframe['sma_75'] * 0.96) & (dataframe['EWO'] < -8.0) & (dataframe['cti'] < -0.9) & (dataframe['r_14'] < -97.0) & (dataframe['crsi_1h'] > 14.0)
        is_nfix_27 = (dataframe['close'] < dataframe['sma_75'] * 0.934) & (dataframe['EWO'] > 6.4) & (dataframe['rsi_14'] < 32.0) & (dataframe['cti'] < -0.8) & (dataframe['r_14'] < -96.0)
        is_nfix_19 = (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & dataframe['bb40_2_low'].shift().gt(0) & dataframe['bb40_2_delta'].gt(dataframe['close'] * 0.045) & dataframe['closedelta'].gt(dataframe['close'] * 0.02) & dataframe['tail'].lt(dataframe['bb40_2_delta'] * 0.28) & dataframe['close'].lt(dataframe['bb40_2_low'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['cti'] < -0.9) & (dataframe['cti_1h'] > -0.75) & (dataframe['cti_1h'] < 0.25)
        is_nfix_11 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * 0.027) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['ema_20'] * 0.932) & (dataframe['rsi_14'] < 25.0)
        is_nfix_9 = (dataframe['ema_50_1h'] > dataframe['ema_100_1h']) & (dataframe['close'] < dataframe['sma_30'] * 0.99) & (dataframe['cti'] < -0.92) & (dataframe['EWO'] < -5.0) & (dataframe['cti_1h'] < -0.88) & (dataframe['crsi_1h'] > 20.0)
        is_nfix_36 = (dataframe['ema_200'] > dataframe['ema_200'].shift(36) * 1.035) & (dataframe['close'] < dataframe['ema_20'] * 0.956) & (dataframe['rsi_14'] < 34.0) & (dataframe['r_64'] < -80.0) & (dataframe['cti'] < -0.5) & (dataframe['r_480_1h'] < -30.0)
        is_nfi_sma_1 = ((dataframe['close'] - dataframe['open'].rolling(36).min()) / dataframe['open'].rolling(36).min() > self.entry_min_inc_1.value) & (dataframe['rsi_1h'] > self.entry_rsi_1h_min_1.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_max_1.value) & (dataframe['rsi'] < self.entry_rsi_1.value) & (dataframe['mfi'] < self.entry_mfi_1.value) & (dataframe['volume'] > 0)
        is_nfi_sma_2 = (dataframe['rsi'] < dataframe['rsi_1h'] - self.entry_rsi_1h_diff_2.value) & (dataframe['mfi'] < self.entry_mfi_2.value) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_2.value) & (dataframe['volume'] > 0)
        is_nfi_sma_3 = dataframe['bb40_2_low'].shift().gt(0) & dataframe['bb40_2_delta'].gt(dataframe['close'] * self.entry_bb40_bbdelta_close_3.value) & dataframe['closedelta'].gt(dataframe['close'] * self.entry_bb40_closedelta_close_3.value) & dataframe['tail'].lt(dataframe['bb40_2_delta'] * self.entry_bb40_tail_bbdelta_3.value) & dataframe['close'].lt(dataframe['bb40_2_low'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['volume'] > 0)
        is_nfi_sma_4 = (dataframe['close'] < dataframe['ema_50']) & (dataframe['close'] < self.entry_bb20_close_bblowerband_4.value * dataframe['bb20_2_low']) & (dataframe['volume'] < dataframe['volume_mean_30'].shift(1) * self.entry_bb20_volume_4.value)
        is_nfi_sma_5 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_open_mult_5.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_5.value) & (dataframe['volume'] > 0)
        is_nfi_sma_6 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_open_mult_6.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_6.value) & (dataframe['volume'] > 0)
        is_nfi_sma_7 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_open_mult_7.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['rsi'] < self.entry_rsi_7.value) & (dataframe['volume'] > 0)
        is_nfi_sma_8 = (dataframe['rsi'] < self.entry_rsi_8.value) & (dataframe['volume'] > dataframe['volume'].shift(1) * self.entry_volume_8.value) & (dataframe['close'] > dataframe['open']) & (dataframe['close'] - dataframe['low'] > (dataframe['close'] - dataframe['open']) * self.entry_tail_diff_8.value) & (dataframe['volume'] > 0)
        is_nfi_sma_9 = (dataframe['close'] < dataframe['ema_20'] * self.entry_ma_offset_9.value) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_9.value) & (dataframe['rsi_1h'] > self.entry_rsi_1h_min_9.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_max_9.value) & (dataframe['mfi'] < self.entry_mfi_9.value) & (dataframe['volume'] > 0)
        is_nfi_sma_10 = (dataframe['close'] < dataframe['sma_30'] * self.entry_ma_offset_10.value) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_10.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_10.value) & (dataframe['volume'] > 0)
        is_nfi_sma_11 = ((dataframe['close'] - dataframe['open'].rolling(36).min()) / dataframe['open'].rolling(36).min() > self.entry_min_inc_11.value) & (dataframe['close'] < dataframe['sma_30'] * self.entry_ma_offset_11.value) & (dataframe['rsi_1h'] > self.entry_rsi_1h_min_11.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_max_11.value) & (dataframe['rsi'] < self.entry_rsi_11.value) & (dataframe['mfi'] < self.entry_mfi_11.value) & (dataframe['volume'] > 0)
        is_nfi_sma_12 = (dataframe['close'] < dataframe['sma_30'] * self.entry_ma_offset_12.value) & (dataframe['EWO'] > self.entry_ewo_12.value) & (dataframe['rsi'] < self.entry_rsi_12.value) & (dataframe['volume'] > 0)
        is_nfi_sma_13 = (dataframe['close'] < dataframe['sma_30'] * self.entry_ma_offset_13.value) & (dataframe['EWO'] < self.entry_ewo_13.value) & (dataframe['volume'] > 0)
        is_nfi_sma_14 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_open_mult_14.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_14.value) & (dataframe['close'] < dataframe['ema_20'] * self.entry_ma_offset_14.value) & (dataframe['volume'] > 0)
        is_nfi_sma_15 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_open_mult_15.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['rsi'] < self.entry_rsi_15.value) & (dataframe['close'] < dataframe['ema_20'] * self.entry_ma_offset_15.value) & (dataframe['volume'] > 0)
        is_nfi_sma_16 = (dataframe['close'] < dataframe['ema_20'] * self.entry_ma_offset_16.value) & (dataframe['EWO'] > self.entry_ewo_16.value) & (dataframe['rsi'] < self.entry_rsi_16.value) & (dataframe['volume'] > 0)
        is_nfi_sma_17 = (dataframe['close'] < dataframe['ema_20'] * self.entry_ma_offset_17.value) & (dataframe['EWO'] < self.entry_ewo_17.value) & (dataframe['volume'] > 0)
        is_nfi_sma_18 = (dataframe['rsi'] < self.entry_rsi_18.value) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_18.value) & (dataframe['volume'] > 0)
        is_nfi_sma_19 = (dataframe['close'].shift(1) > dataframe['ema_100_1h']) & (dataframe['low'] < dataframe['ema_100_1h']) & (dataframe['close'] > dataframe['ema_100_1h']) & (dataframe['rsi_1h'] > self.entry_rsi_1h_min_19.value) & (dataframe['chop'] < self.entry_chop_min_19.value) & (dataframe['volume'] > 0)
        is_nfi_sma_20 = (dataframe['rsi'] < self.entry_rsi_20.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_20.value) & (dataframe['volume'] > 0)
        is_nfi_sma_21 = (dataframe['rsi'] < self.entry_rsi_21.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_21.value) & (dataframe['volume'] > 0)
        is_nfi_sma_22 = (dataframe['volume_mean_4'] * self.entry_volume_22.value > dataframe['volume']) & (dataframe['close'] < dataframe['sma_30'] * self.entry_ma_offset_22.value) & (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_22.value) & (dataframe['EWO'] > self.entry_ewo_22.value) & (dataframe['rsi'] < self.entry_rsi_22.value) & (dataframe['volume'] > 0)
        is_nfi_sma_23 = (dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_23.value) & (dataframe['EWO'] > self.entry_ewo_23.value) & (dataframe['rsi'] < self.entry_rsi_23.value) & (dataframe['rsi_1h'] < self.entry_rsi_1h_23.value) & (dataframe['volume'] > 0)
        is_nfi_sma_24 = (dataframe['ema_12_1h'].shift(12) < dataframe['ema_35_1h'].shift(12)) & (dataframe['ema_12_1h'].shift(12) < dataframe['ema_35_1h'].shift(12)) & (dataframe['ema_12_1h'] > dataframe['ema_35_1h']) & (dataframe['cmf_1h'].shift(12) < 0) & (dataframe['cmf_1h'] > 0) & (dataframe['rsi'] < self.entry_24_rsi_max.value) & (dataframe['rsi_1h'] > self.entry_24_rsi_1h_min.value) & (dataframe['volume'] > 0)
        ...
        dataframe.loc[is_ewo_2, 'entry_tag'] += 'ewo2 '
        ...
        dataframe.loc[is_nfi_38, 'entry_tag'] += 'nfi_38 '
        ...
        dataframe.loc[is_diamond, 'entry_tag'] += 'diamond '
        ...
        dataframe.loc[is_nfix_38, 'entry_tag'] += 'nfix_38 '
        ...
        dataframe.loc[is_nfix_39, 'entry_tag'] += 'nfix_39 '
        conditions.append(is_nfi_sma_1)
        dataframe.loc[is_nfi_sma_1, 'entry_tag'] += 'is_nfi_sma_1 '
        ...
        dataframe.loc[is_nfi_sma_8, 'entry_tag'] += 'is_nfi_sma_8 '
        ...
        dataframe.loc[is_nfi_sma_11, 'entry_tag'] += 'is_nfi_sma_11 '
        conditions.append(is_nfi_sma_12)
        dataframe.loc[is_nfi_sma_12, 'entry_tag'] += 'is_nfi_sma_12 '
        conditions.append(is_nfi_sma_13)
        dataframe.loc[is_nfi_sma_13, 'entry_tag'] += 'is_nfi_sma_13 '
        conditions.append(is_nfi_sma_14)
        dataframe.loc[is_nfi_sma_14, 'entry_tag'] += 'is_nfi_sma_14 '
        ...
        dataframe.loc[is_nfi_sma_18, 'entry_tag'] += 'is_nfi_sma_18 '
        conditions.append(is_nfi_sma_19)
        dataframe.loc[is_nfi_sma_19, 'entry_tag'] += 'is_nfi_sma_19 '
        conditions.append(is_nfi_sma_20)
        dataframe.loc[is_nfi_sma_20, 'entry_tag'] += 'is_nfi_sma_20 '
        conditions.append(is_nfi_sma_21)
        dataframe.loc[is_nfi_sma_21, 'entry_tag'] += 'is_nfi_sma_21 '
        ...
        dataframe.loc[is_nfi_sma_24, 'entry_tag'] += 'is_nfi_sma_24 '
        if conditions:
...
class BB_RPB_TSL_Tranz_TrailingBuy(BB_RPB_TSL_SMA_Tranz_TB_1_1_1):
    ...
    def trailing_entry_info(self, pair: str, current_price: float):
        ...
        try:
            ...
            log.info(f"pair: {pair} : start: {trailing_entry['start_trailing_price']:.4f}, duration: {duration}, current: {current_price:.4f}, uplimit: {trailing_entry['trailing_entry_order_uplimit']:.4f}, profit: {self.current_trailing_profit_ratio(pair, current_price) * 100:.2f}%, offset: {trailing_entry['offset']}")
                ...
                if len(dataframe) >= 1:
                    ...
                    if trailing_entry['allow_trailing']:
                        if not trailing_entry['trailing_entry_order_started'] and last_candle['entry'] == 1:
                            ...
                            log.info(f"start trailing entry for {pair} at {last_candle['close']}")
                            ...
                            if trailing_entry_offset == 'forceentry':
                                ...
                                log.info(f'STOP trailing entry for {pair} because "trailing entry offset" returned None')
                                ...
                                self.trailing_entry_info(pair, current_price)
                                log.info(f"update trailing entry for {pair} at {old_uplimit} -> {self.custom_info_trail_entry[pair]['trailing_entry']['trailing_entry_order_uplimit']}")
                                ...
                                ratio = '%.2f' % (self.current_trailing_profit_ratio(pair, current_price) * 100)
                                self.trailing_entry_info(pair, current_price)
                                log.info(f"current price ({current_price}) > uplimit ({trailing_entry['trailing_entry_order_uplimit']}) and lower than starting price price ({trailing_entry['start_trailing_price'] * (1 + self.trailing_entry_max_entry)}). OK for {pair} ({ratio} %), order may not be triggered if all slots are full")
                                ...
                                self.trailing_entry(pair, reinit=True)
                                self.trailing_entry_info(pair, current_price)
                                log.info(f'STOP trailing entry for {pair} because of the price is higher than starting price * {1 + self.trailing_entry_max_stop}')
                            else:
                                # uplimit > current_price > max_price, continue trailing and wait for the price to go down
                                self.trailing_entry_info(pair, current_price)
                                log.info(f'price too high for {pair} !')
                    else:
                        log.info(f'Wait for next entry signal for {pair}')
                if val == True:
                    self.trailing_entry_info(pair, rate)
                    self.trailing_entry(pair, reinit=True)
                    log.info(f'STOP trailing entry for {pair} because I entry it')
        return val
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.trailing_entry_order_enabled and self.config['runmode'].value in ('live', 'dry_run'):
            ...
            if last_candle['entry'] == 1:
                ...
                log.info(f"Continue trailing for {metadata['pair']}. Manually trigger entry signal!!")
