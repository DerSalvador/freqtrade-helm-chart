...
entry_params = {'base_nb_candles_entry': 16, 'ewo_high': 5.672, 'ewo_low': -19.931, 'low_offset': 0.973, 'rsi_entry': 59, 'ewo_high_bb': 4.86, 'for_ma_length': 22, 'for_sigma': 1.74}
# Sell hyperspace params:
exit_params = {'base_nb_candles_exit': 20, 'high_offset': 1.01, 'pHSL': -0.178, 'pPF_1': 0.01, 'pPF_2': 0.048, 'pSL_1': 0.009, 'pSL_2': 0.043, 'for_ma_length_exit': 65, 'for_sigma_exit': 1.895, 'rsi_high': 72}
...
class SMA_BBRSI(IStrategy):
    ...
    ewo_high_bb = DecimalParameter(0, 7.0, default=entry_params['ewo_high_bb'], space='entry', optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append((dataframe['rsi'] < dataframe[f'basis_{self.for_ma_length.value}'] - dataframe[f'dev_{self.for_ma_length.value}'] * self.for_sigma.value) & ((dataframe['EWO'] > self.ewo_high_bb.value) & (dataframe['EWO'] < 10) | (dataframe['EWO'] >= 10) & (dataframe['rsi'] < 40)) & (dataframe['rsi_4'] < 25) & (dataframe['volume'] > 0) & (dataframe['cci_fast'] < 100) & (dataframe['moderi_96'] == True))
        if conditions:
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append((((dataframe['rsi'] > self.rsi_high.value) | (dataframe['rsi'] > dataframe[f'basis_{self.for_ma_length_exit.value}'] + dataframe[f'dev_{self.for_ma_length_exit.value}'] * self.for_sigma_exit.value)) & (dataframe['moderi_96'] == True) | qtpylib.crossed_below(dataframe['close'], dataframe['atr_high'])) & (dataframe['volume'] > 0))
        if conditions:
