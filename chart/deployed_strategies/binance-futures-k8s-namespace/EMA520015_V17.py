...
class EMA520015_V17(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['close'].shift(1) < dataframe['ema20']) & (dataframe['close'] > dataframe['ema20']), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['close'].shift(1) > dataframe['ema20']) & (dataframe['close'] < dataframe['ema20']), 'exit'] = 1
        return dataframe
