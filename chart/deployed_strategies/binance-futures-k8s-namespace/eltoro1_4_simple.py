...
class eltoro1_4_simple(IStrategy):
    ...
    # SMAOffset
    base_nb_candles_entry = IntParameter(25, 60, default=25, space='entry', optimize=True)
    base_nb_candles_exit = IntParameter(30, 60, default=49, space='exit', optimize=True)
    low_offset = DecimalParameter(0.97, 0.99, default=0.97, decimals=2, space='entry', optimize=True)
    high_offset = DecimalParameter(1.01, 1.05, default=1.01, decimals=2, space='exit', optimize=True)
    high_offset_2 = DecimalParameter(1.05, 1.1, default=1.05, decimals=2, space='exit', optimize=True)
    max_length = CategoricalParameter([24, 48, 72, 96, 144, 192, 240], default=48, space='entry', optimize=False)
    decision_length = IntParameter(30, 60, default=25, space='entry', optimize=True)
    # Buy Parameters
    ...
    auto_entry_EWO = IntParameter(8, 15, default=10, space='entry', optimize=True)
    # Buy Parameters
    rsi_exit = IntParameter(60, 70, default=64, space='exit', optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[dataframe['close'] > dataframe['hma_50'], 'hma_entry1'] = 2
        ...
        dataframe.loc[dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value, 'base_ma_entry1'] = 2
        dataframe.loc[dataframe['close'] > dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value, 'base_ma_entry'] = 0
        ...
        dataframe['auto_entry'] = dataframe[['rsi_weight', 'willr_weight', 'hma_weight', 'base_ma_entry_weight', 'macdl_weight', '200SMA_weight', 'from_weight']].sum(axis=1)
        ### SELLING ###
        ...
        dataframe.loc[dataframe['close'] > dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset_2.value, 'base_ma_exit2'] = 2
        ...
        dataframe['auto_exit'] = dataframe[['rsi_weight_exit', 'willr_weight_exit', 'hma_weight_exit', 'base_ma_exit_weight', 'macdl_weight_exit', '200SMA_weight_exit', 'from_weight_exit']].sum(axis=1)
        dataframe['auto_entry_decision'] = ta.SMA(dataframe['auto_entry'] - dataframe['auto_exit'], timeperiod=self.decision_length.value)
        dataframe['auto_exit_decision'] = ta.SMA(dataframe['auto_exit'] - dataframe['auto_entry'], timeperiod=self.decision_length.value)
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['auto_entry_decision'] >= self.auto_entry.value) & (dataframe['auto_entry_decision'] < dataframe['auto_entry']) & (dataframe['INFEWO_1h'] >= self.bull.value) & (dataframe['INFEWO_1h'] > dataframe['INFEWO_1h'].shift(4)) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'auto entry bullzzz up')
        dataframe.loc[(dataframe['auto_entry_decision'] >= self.auto_entry.value + self.auto_entry_down.value) & (dataframe['auto_entry_decision'] < dataframe['auto_entry']) & (dataframe['INFEWO_1h'] >= self.bull.value) & (dataframe['INFEWO_1h'] <= dataframe['INFEWO_1h'].shift(4)) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'auto entry bullzzz down')
        dataframe.loc[(dataframe['auto_entry_decision'] >= self.auto_entry.value + self.auto_entry_bearzzz.value) & (dataframe['auto_entry_decision'] < dataframe['auto_entry']) & (dataframe['INFEWO_1h'] < self.bull.value) & (dataframe['INFEWO_1h'] > dataframe['INFEWO_1h'].shift(4)) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'auto entry bearzzz up')
        dataframe.loc[(dataframe['auto_entry_decision'] >= self.auto_entry.value + self.auto_entry_bearzzz.value + self.auto_entry_bearzzz_down.value) & (dataframe['auto_entry_decision'] < dataframe['auto_entry']) & (dataframe['INFEWO_1h'] < self.bull.value) & (dataframe['INFEWO_1h'] <= dataframe['INFEWO_1h'].shift(4)) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'auto entry bearzzz down')
        # (dataframe['INFEWO_1h'].shift(48) > dataframe['INFEWO_1h'].shift(32)) &
        dataframe.loc[(dataframe['auto_entry_decision'] > self.auto_entry_EWO.value) & (dataframe['auto_entry_decision'] > dataframe['auto_entry_decision'].shift(1)) & (dataframe['INFEWO_1h'] > dataframe['INFEWO_1h'].shift(4)) & (dataframe['INFEWO_1h'].shift(4) < dataframe['INFEWO_1h'].shift(8)) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, '1h EWO Transition')
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['auto_exit_decision'] >= self.auto_exit_bull.value + self.auto_exit_bear.value) & (dataframe['close'] < dataframe['hma_50']) & (dataframe['INFEWO_1h'] < self.bull.value) & (dataframe['INFEWO_1h'] < dataframe['BTC_EWO_Fast_1h']) & (dataframe['INFEWO_1h'] <= dataframe['INFEWO_1h'].shift(2)) & (dataframe['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'auto_exit_bull')
        dataframe.loc[(dataframe['auto_exit_decision'] >= self.auto_exit_bear.value) & (dataframe['close'] < dataframe['hma_50']) & (dataframe['INFEWO_1h'] > self.bull.value) & (dataframe['INFEWO_1h'] < dataframe['BTC_EWO_Fast_1h']) & (dataframe['INFEWO_1h'] <= dataframe['INFEWO_1h'].shift(4)) & (dataframe['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'auto_exit_bear')
        return dataframe