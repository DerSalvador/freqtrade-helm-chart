...
class Schism4(IStrategy):
    ...
    entry_params = {'inf-rsi': 58, 'mp': 26, 'rmi-fast': 11, 'rmi-slow': 55, 'xinf-stake-rmi': 25, 'xtf-fiat-rsi': 64, 'xtf-stake-rsi': 90}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if trade_data['active_trade']:
            ...
            conditions.append((dataframe[f'rsi_{self.inf_timeframe}'] <= params['inf-rsi']) & (dataframe['rmi-up-trend'] == 1) & (dataframe['rmi-slow'] <= params['rmi-slow']) & (dataframe['rmi-fast'] >= params['rmi-fast']) & (dataframe['mp'] >= params['mp']))
