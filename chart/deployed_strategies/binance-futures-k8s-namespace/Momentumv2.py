...
class Momentumv2(IStrategy):
    ...
    # Buy Parameters
    entry_ema = IntParameter(10, 100, default=30, space='entry', optimize=True, load=True)
    # Sell Parameters
    exit_rsi = DecimalParameter(70, 99, default=80, space='exit', optimize=True, load=True)
    # Stoploss Parameters
    atr_timeperiod = IntParameter(5, 21, default=7, space='exit')
    atr_multiplier = DecimalParameter(2.5, 3.5, default=2.5, space='exit')
    entry_params = {'entry_ema': 80}
    exit_params = {'exit_rsi': 90, 'atr_multiplier': 2.6, 'atr_timeperiod': 12}
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # EMA
        dataframe['ema'] = ta.EMA(dataframe, timeperiod=self.entry_ema.value)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(qtpylib.crossed_below(dataframe['macd'], dataframe['macdsignal']) | qtpylib.crossed_below(dataframe['rsi'], self.exit_rsi.value))
        conditions.append(dataframe['volume'] > 0)
        if conditions:
