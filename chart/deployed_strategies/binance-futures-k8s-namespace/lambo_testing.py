...
class lambotest(IStrategy):
    ...
    # ROI table:
    minimal_roi = {'0': 0.0625, '28': 0.05, '76': 0.04, '125': 0.03, '240': 0.02, '360': 0}
    ...
    # hyperopt params
    entry_williams_low = DecimalParameter(-100, -50, default=-58.201)
    entry_williams_high = DecimalParameter(-100, -50, default=-92.098)
    entry_rsi_low = DecimalParameter(0, 30, default=2.342)
    entry_rsi_high = DecimalParameter(30, 60, default=51.22)
    entry_volume = DecimalParameter(1000, 100000, default=5000)
    exit_rsi = DecimalParameter(60, 100, default=60.631)
    exit_williams = DecimalParameter(-40, -10, default=-19.308)
    exit_volume = DecimalParameter(1000, 100000, default=1000)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['volume'] > self.entry_volume.value) & (dataframe['OBVSlope'] > 0) & (dataframe['williamspercent'] <= self.entry_williams_low.value) & (dataframe['williamspercent'] > self.entry_williams_high.value) & (dataframe['rsi'] > self.entry_rsi_low.value) & (dataframe['rsi'] <= self.entry_rsi_high.value) & qtpylib.crossed_above(dataframe['fastk'], dataframe['slowd']) & (dataframe['close'] < dataframe['bb.middle']), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['volume'] > self.exit_volume.value) & (dataframe['williamspercent'] > self.exit_williams.value) & (dataframe['rsi'] > self.exit_rsi.value) | qtpylib.crossed_below(dataframe['plus.di'], dataframe['minus.di']) | qtpylib.crossed_below(dataframe['macd'], dataframe['macdsignal']) | qtpylib.crossed_below(dataframe['fastk'], dataframe['slowd']) & (dataframe['close'] > dataframe['bb.upper']) | (dataframe['vpci'] >= dataframe['bb.upper']), 'exit_long'] = 1
        return dataframe
