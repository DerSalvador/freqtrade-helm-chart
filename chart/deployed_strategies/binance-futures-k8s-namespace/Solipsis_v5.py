...
class Solipsis5(IStrategy):
    ...
    # Base Pair Params
    base_mp = IntParameter(10, 50, default=30, space='entry', load=True, optimize=True)
    base_rmi_max = IntParameter(30, 60, default=50, space='entry', load=True, optimize=True)
    base_rmi_min = IntParameter(0, 30, default=20, space='entry', load=True, optimize=True)
    base_ma_streak = IntParameter(1, 4, default=1, space='entry', load=True, optimize=True)
    base_rmi_streak = IntParameter(3, 8, default=3, space='entry', load=True, optimize=True)
    base_trigger = CategoricalParameter(['pcc', 'rmi', 'none'], default='rmi', space='entry', load=True, optimize=True)
    inf_pct_adr = DecimalParameter(0.7, 0.99, default=0.8, space='entry', load=True, optimize=True)
    # BTC Informative
    xbtc_guard = CategoricalParameter(['strict', 'lazy', 'none'], default='lazy', space='entry', optimize=True)
    xbtc_base_rmi = IntParameter(20, 70, default=40, space='entry', load=True, optimize=True)
    # BTC / ETH Stake Parameters
    xtra_base_stake_rmi = IntParameter(10, 50, default=50, space='entry', load=True, optimize=True)
    xtra_base_fiat_rmi = IntParameter(30, 70, default=50, space='entry', load=True, optimize=True)
    ## Sell Space Params are being used for both custom_stoploss and custom_exit
    # Custom Sell Profit (formerly Dynamic ROI)
    cexit_roi_type = CategoricalParameter(['static', 'decay', 'step'], default='step', space='exit', load=True, optimize=True)
    ...
    cexit_trend_type = CategoricalParameter(['rmi', 'ssl', 'candle', 'any', 'none'], default='any', space='exit', load=True, optimize=True)
    ...
    cexit_pullback_respect_roi = CategoricalParameter([True, False], default=False, space='exit', load=True, optimize=True)
    cexit_endtrend_respect_roi = CategoricalParameter([True, False], default=False, space='exit', load=True, optimize=True)
    # Custom Stoploss
    cstop_loss_threshold = DecimalParameter(-0.05, -0.01, default=-0.03, space='exit', load=True, optimize=True)
    cstop_bail_how = CategoricalParameter(['roc', 'time', 'any', 'none'], default='none', space='exit', load=True, optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(dataframe['close'] <= dataframe[f'1d-low_{self.inf_timeframe}'] + self.inf_pct_adr.value * dataframe[f'adr_{self.inf_timeframe}'])
        # Base Timeframe Guards
        conditions.append((dataframe['rmi-dn-count'] >= self.base_rmi_streak.value) & (dataframe['streak-bo-count'] >= self.base_ma_streak.value) & (dataframe['rmi'] <= self.base_rmi_max.value) & (dataframe['rmi'] >= self.base_rmi_min.value) & (dataframe['mp'] <= self.base_mp.value))
        ...
        if self.config['stake_currency'] in ('BTC', 'ETH'):
            conditions.append((dataframe[f'{self.custom_fiat}_rmi'] > self.xtra_base_fiat_rmi.value) | (dataframe[f"{self.config['stake_currency']}_rmi"] < self.xtra_base_stake_rmi.value))
        # Extra conditions for BTC/STAKE if not in whitelist
        elif self.custom_btc_inf:
            if self.xbtc_guard.value == 'strict':
                conditions.append((dataframe['BTC_rmi'] > self.xbtc_base_rmi.value) & (dataframe['BTC_close'] > dataframe['BTC_kama']))
            if self.xbtc_guard.value == 'lazy':
                conditions.append((dataframe['close'] > dataframe['kama']) | (dataframe['BTC_rmi'] > self.xbtc_base_rmi.value) & (dataframe['BTC_close'] > dataframe['BTC_kama']))
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if self.cexit_roi_type.value == 'static':
            ...
            min_roi = cta.linear_decay(self.cexit_roi_start.value, self.cexit_roi_end.value, 0, self.cexit_roi_time.value, trade_dur)
