...
class NFINextMultiOffsetAndHO2(IStrategy):
    ...
    order_types = {'entry': 'limit', 'exit': 'limit', 'trailing_stop_loss': 'limit', 'stoploss': 'market', 'stoploss_on_exchange': False}
    #############################################################
    #############
    ...
    entry_cti_8 = DecimalParameter(-0.99, -0.5, default=-0.9, space='entry', decimals=2, optimize=False, load=True)
    entry_rsi_8 = DecimalParameter(20.0, 50.0, default=30.0, space='entry', decimals=1, optimize=False, load=True)
    entry_rsi_1h_8 = DecimalParameter(40.0, 66.0, default=54.0, space='entry', decimals=1, optimize=False, load=True)
    entry_volume_8 = DecimalParameter(0.6, 6.0, default=1.2, space='entry', decimals=1, optimize=False, load=True)
    ...
    entry_chop_min_19 = DecimalParameter(20.0, 60.0, default=22.1, space='entry', decimals=1, optimize=False, load=True)
    ...
    exit_custom_profit_under_profit_1 = DecimalParameter(0.0, 0.01, default=0.0, space='exit', optimize=False, load=True)
    ...
    def load_hold_trades_config(self):
        ...
        if not hold_trades_config_file.is_file():
            ...
            if not hold_trades_config_file.is_file():
                log.warning("The 'hold-trades.json' file was not found. Looked in '%s' and '%s'. HOLD support disabled.", strat_file_path.resolve().parent, strat_file_path.absolute().parent)
                return
    ...
    def exit_under_min(self, current_profit: float, last_candle) -> tuple:
        if last_candle['moderi_96'] == False:
            # Downtrend
            if (current_profit > self.exit_custom_profit_under_profit_1.value) & (last_candle['close'] < last_candle['ema_200']) & ((last_candle['ema_200'] - last_candle['close']) / last_candle['close'] < self.exit_custom_profit_under_rel_1.value) & (last_candle['rsi'] > last_candle['rsi_1h'] + self.exit_custom_profit_under_rsi_diff_1.value):
    ...
    def exit_quick_mode(self, current_profit: float, max_profit: float, last_candle, entry_signal_candle) -> tuple:
        if entry_signal_candle['entry_condition_32'] == 1 or entry_signal_candle['entry_condition_33'] == 1 or entry_signal_candle['entry_condition_34'] == 1:
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        # Quick exit mode
        if not entry_signal.empty:
            exit, signal_name = self.exit_quick_mode(current_profit, max_profit, last_candle, entry_signal_candle)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for index in self.entry_protection_params:
            ...
            item_entry_protection_list.append(not_empty_volume)
            entry_protection_list.append(item_entry_protection_list)
        # Buy Condition #1
        dataframe.loc[:, 'entry_condition_1'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_1_enable']:
            ...
            dataframe.loc[item_entry, 'entry_condition_1'] = 1
            conditions.append(item_entry)
        # Buy Condition #2
        dataframe.loc[:, 'entry_condition_2'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_2_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_2'] = 1
            conditions.append(item_entry)
        # Buy Condition #3
        dataframe.loc[:, 'entry_condition_3'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_3_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_3'] = 1
            conditions.append(item_entry)
        # Buy Condition #4
        dataframe.loc[:, 'entry_condition_4'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_4_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_4'] = 1
            conditions.append(item_entry)
        # Buy Condition #5
        dataframe.loc[:, 'entry_condition_5'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_5_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_5'] = 1
            conditions.append(item_entry)
        # Buy Condition #6
        dataframe.loc[:, 'entry_condition_6'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_6_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_6'] = 1
            conditions.append(item_entry)
        # Buy Condition #7
        dataframe.loc[:, 'entry_condition_7'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_7_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_7'] = 1
            conditions.append(item_entry)
        # Buy Condition #8
        dataframe.loc[:, 'entry_condition_8'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_8_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_8'] = 1
            conditions.append(item_entry)
        # Buy Condition #9
        dataframe.loc[:, 'entry_condition_9'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_9_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_9'] = 1
            conditions.append(item_entry)
        # Buy Condition #10
        dataframe.loc[:, 'entry_condition_10'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_10_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_10'] = 1
            conditions.append(item_entry)
        # Buy Condition #11
        dataframe.loc[:, 'entry_condition_11'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_11_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_11'] = 1
            conditions.append(item_entry)
        # Buy Condition #12
        dataframe.loc[:, 'entry_condition_12'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_12_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_12'] = 1
            conditions.append(item_entry)
        # Buy Condition #13
        dataframe.loc[:, 'entry_condition_13'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_13_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_13'] = 1
            conditions.append(item_entry)
        # Buy Condition #14
        dataframe.loc[:, 'entry_condition_14'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_14_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_14'] = 1
            conditions.append(item_entry)
        # Buy Condition #15
        dataframe.loc[:, 'entry_condition_15'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_15_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_15'] = 1
            conditions.append(item_entry)
        # Buy Condition #16
        dataframe.loc[:, 'entry_condition_16'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_16_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_16'] = 1
            conditions.append(item_entry)
        # Buy Condition #17
        dataframe.loc[:, 'entry_condition_17'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_17_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_17'] = 1
            conditions.append(item_entry)
        # Buy Condition #18
        dataframe.loc[:, 'entry_condition_18'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_18_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_18'] = 1
            conditions.append(item_entry)
        # Buy Condition #19
        dataframe.loc[:, 'entry_condition_19'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_19_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_19'] = 1
            conditions.append(item_entry)
        # Buy Condition #20
        dataframe.loc[:, 'entry_condition_20'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_20_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_20'] = 1
            conditions.append(item_entry)
        # Buy Condition #21
        dataframe.loc[:, 'entry_condition_21'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_21_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_21'] = 1
            conditions.append(item_entry)
        # Buy Condition #22
        dataframe.loc[:, 'entry_condition_22'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_22_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_22'] = 1
            conditions.append(item_entry)
        # Buy Condition #23
        dataframe.loc[:, 'entry_condition_23'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_23_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_23'] = 1
            conditions.append(item_entry)
        # Buy Condition #24
        dataframe.loc[:, 'entry_condition_24'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_24_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_24'] = 1
            conditions.append(item_entry)
        # Buy Condition #25
        dataframe.loc[:, 'entry_condition_25'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_25_enable']:
            ...
            item_entry_logic.append(dataframe['cti'] < -0.6)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_25'] = 1
            conditions.append(item_entry)
        # Buy Condition #26
        dataframe.loc[:, 'entry_condition_26'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_26_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_26'] = 1
            conditions.append(item_entry)
        # Buy Condition #27
        dataframe.loc[:, 'entry_condition_27'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_27_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_27'] = 1
            conditions.append(item_entry)
        # Buy Condition #28
        dataframe.loc[:, 'entry_condition_28'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_28_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_28'] = 1
            conditions.append(item_entry)
        # Buy Condition #29
        dataframe.loc[:, 'entry_condition_29'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_29_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_29'] = 1
            conditions.append(item_entry)
        # Buy Condition #30
        dataframe.loc[:, 'entry_condition_30'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_30_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_30'] = 1
            conditions.append(item_entry)
        # Buy Condition #31
        dataframe.loc[:, 'entry_condition_31'] = 0
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_31_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_31'] = 1
            conditions.append(item_entry)
        # Buy Condition #32
        dataframe.loc[:, 'entry_condition_32'] = 0
        # Quick mode entry
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_32_enable']:
            ...
            item_entry_logic.append(dataframe['rsi_4'] < 46.0)
            item_entry_logic.append(dataframe['ema_20_1h'] > dataframe['ema_25_1h'])
            item_entry_logic.append((dataframe['open'] - dataframe['close']) / dataframe['close'] < 0.005)
            item_entry_logic.append(dataframe['close'] < dataframe['sma_15'] * 0.946)
            item_entry_logic.append((dataframe['open'] < dataframe['ema_20_1h']) & (dataframe['low'] < dataframe['ema_20_1h']) | (dataframe['open'] > dataframe['ema_20_1h']) & (dataframe['low'] > dataframe['ema_20_1h']))
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_32'] = 1
            conditions.append(item_entry)
        # Buy Condition #33
        dataframe.loc[:, 'entry_condition_33'] = 0
        # Quick mode entry
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_33_enable']:
            ...
            item_entry_logic.append(dataframe['cti'] < -0.8)
            item_entry_logic.append((dataframe['open'] - dataframe['close']) / dataframe['close'] < 0.005)
            item_entry_logic.append(dataframe['close'] < dataframe['ema_13'] * 0.954)
            item_entry_logic.append(dataframe['ewo'] > 11.0)
            item_entry_logic.append(dataframe['rsi'] < 36.0)
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_33'] = 1
            conditions.append(item_entry)
        # Buy Condition #34
        dataframe.loc[:, 'entry_condition_34'] = 0
        # Quick mode entry
        # -----------------------------------------------------------------------------------------
        if self.entry_params['entry_condition_34_enable']:
            ...
            item_entry_logic.append(dataframe['volume'] < dataframe['volume_mean_4'] * 1.1)
            item_entry_logic.append((dataframe['open'] - dataframe['close']) / dataframe['close'] < 0.002)
            item_entry_logic.append(dataframe['close'] < dataframe['ema_13'] * 0.968)
            item_entry_logic.append(dataframe['ewo'] < -20.0)
            item_entry_logic.append(dataframe['volume'] > 0)
            item_entry = reduce(lambda x, y: x & y, item_entry_logic)
            dataframe.loc[item_entry, 'entry_condition_34'] = 1
            conditions.append(item_entry)
        if conditions:
