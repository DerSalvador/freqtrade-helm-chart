...
class BB_RPB_TSL_SMA_Tranz(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[is_VWAP, 'enter_tag'] += 'is_VWAP '
        ...
        dataframe.loc[is_nasos_1, 'enter_tag'] += 'is_nasos_1 '
        dataframe.loc[is_clucHA, 'enter_tag'] += 'is_clucHA '  #     ---
        conditions.append(is_clucHA)
        dataframe.loc[is_nasos_2, 'enter_tag'] += 'is_nasos_2 '  #     ---
        conditions.append(is_nasos_2)
        conditions.append(is_hma)
        dataframe.loc[is_hma, 'enter_tag'] += 'is_hma '
        conditions.append(is_zema)
        dataframe.loc[is_zema, 'enter_tag'] += 'is_zema '
        conditions.append(is_trima)  #     -
        dataframe.loc[is_trima, 'enter_tag'] += 'is_trima '
        dataframe.loc[is_fama, 'enter_tag'] += 'is_fama '  #     ---
...
class UziChanTB2(BB_RPB_TSL_SMA_Tranz):
    ...
    init_trailing_entry_dict = {'trailing_entry_order_started': False, 'trailing_entry_order_uplimit': 0, 'start_trailing_price': 0, 'enter_tag': None, 'start_trailing_time': None, 'offset': 0, 'allow_trailing': False}
    ...
    def trailing_exit_offset(self, dataframe, pair: str, current_price: float):
        ...
        if trailing_duration.total_seconds() > self.trailing_expire_seconds:
            if current_trailing_exit_profit_ratio > 0 and last_candle['exit_long'] != 0:
            ...
            if self.trailing_exit_order_enabled and self.config['runmode'].value in ('live', 'dry_run'):
                ...
                if len(dataframe) >= 1:
                    ...
                    if trailing_exit['allow_exit_trailing']:
                        if not trailing_exit['trailing_exit_order_started'] and last_candle['exit_long'] != 0:
