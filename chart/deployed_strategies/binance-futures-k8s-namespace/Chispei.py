...
class Chispei(IStrategy):
    ...
    minimal_roi = {'5127': 0, '1836': 0.676, '2599': 0.079, '120': 0.1, '60': 0.1, '30': 0.05, '20': 0.05, '0': 0.04}
    stoploss = -0.32336
    timeframe = '4h'
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['mom'] < 15) & (dataframe['fastMA'] > dataframe['slowMA']), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['mom'] < 80) & (dataframe['fastMA'] < dataframe['slowMA']), 'exit_long'] = 1
        return dataframe