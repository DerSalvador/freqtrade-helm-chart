...
class PumpDetector(IStrategy):
    ...
    plot_config = {'main_plot': {}, 'subplots': {'XSA': {'j': {'color': 'blue'}, 'k': {'color': 'orange'}}}}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # Make sure Volume is not 0
        dataframe.loc[qtpylib.crossed_above(dataframe['j'], 0) & (dataframe['volume'] > 0), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # Make sure Volume is not 0
        dataframe.loc[(qtpylib.crossed_above(dataframe['j'], 90) | (qtpylib.crossed_below(dataframe['j'], dataframe['k']) & dataframe['j'] > 50)) & (dataframe['volume'] > 0), 'exit'] = 1
        return dataframe