...
class SMAOPv1_TTF(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['volume'] > 0) & (dataframe['rsi'] < self.rsi_entry.value))
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(qtpylib.crossed_above(dataframe['ttf'], self.ttf_upperTrigger.value) & (dataframe['volume'] > 0))
        if conditions:
...
def ttf(df, ttf_length):
    ...
    entryPower = high.rolling(ttf_length).max() - low.shift(ttf_length).fillna(99999).rolling(ttf_length).min()
    exitPower = high.shift(ttf_length).fillna(0).rolling(ttf_length).max() - low.rolling(ttf_length).min()
    ttf = 200 * (entryPower - exitPower) / (entryPower + exitPower)
    return ttf