...
class KC_BB(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['kc_lowerband_28_1'] < dataframe['bb_upperband2']) & (dataframe['kc_lowerband_28_1'] > dataframe['bb_lowerband2']) & (dataframe['kc_bb_delta'] < 1.5) & (dataframe['kc_bb_delta'] > 0.928) & (dataframe['r_14'] < -80) & (dataframe['r_14'] > -90) & (dataframe['ha_closedelta'] > dataframe['ha_close'] * 0.008), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['fisher'] > 0.39075) & dataframe['ha_high'].le(dataframe['ha_high'].shift(1)) & dataframe['ha_high'].shift(1).le(dataframe['ha_high'].shift(2)) & dataframe['ha_close'].le(dataframe['ha_close'].shift(1)) & (dataframe['ema_fast'] > dataframe['ha_close']) & (dataframe['ha_close'] * 0.99754 > dataframe['bb_middleband2']) & (dataframe['volume'] > 0), 'exit'] = 1
        return dataframe
# Williams %R
