...
class BB_RPB_TSL_SMA_Tranz_TB_MOD(IStrategy):
    ...
    minimal_roi = {'0': 0.2, '30': 0.1, '60': 0.05, '90': 0.03, '120': 0.02, '150': 0.01, '180': 0.005}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_nfix_39 = (dataframe['ema_200'] > dataframe['ema_200'].shift(12) * 1.01) & (dataframe['ema_200'] > dataframe['ema_200'].shift(48) * 1.07) & (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & dataframe['bb_lowerband2_40'].shift().gt(0) & dataframe['bb_delta_cluc'].gt(dataframe['close'] * 0.056) & dataframe['closedelta'].gt(dataframe['close'] * 0.01) & dataframe['tail'].lt(dataframe['bb_delta_cluc'] * 0.5) & dataframe['close'].lt(dataframe['bb_lowerband2_40'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['close'] > dataframe['ema_13'] * 0.912)
        ...
        is_additional_check = (dataframe['roc_1h'] < self.entry_roc_1h.value) & (dataframe['bb_width_1h'] < self.entry_bb_width_1h.value)  # Make sure Volume is not 0
        is_btc_safe = (pct_change(dataframe['btc_1d'], dataframe['btc_5m']).fillna(0) > self.entry_btc_safe_1d.value) & (dataframe['volume'] > 0)
        is_nasos_1 = (dataframe['rsi_fast'] < self.rsi_fast_entry.value) & (dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset_2.value) & (dataframe['EWO'] > self.ewo_high_2.value) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset.value) & (dataframe['rsi'] < 25)
        is_nasos_2 = (dataframe['rsi_fast2'] < self.rsi_fast_entry.value) & (dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO2'] < self.ewo_low.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset.value)
        ## Additional Check
        ...
        dataframe.loc[is_ewo2, 'entry_tag'] += 'is_ewo_2 '
