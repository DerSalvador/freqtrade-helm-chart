...
class ClucHAnix_BB_RPB_TraNz(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_nfi7_37 = (dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['close'] < dataframe['sma_75'] * 0.98) & (dataframe['EWO'] > 9.8) & (dataframe['rsi'] < 56.0) & (dataframe['cti'] < -0.7) & dataframe['safe_dump_50_1h']  # Make sure Volume is not 0
        ...
        lambo1 = bool(self.lambo1_enabled.value) & (dataframe['close'] < dataframe['ema_14'] * self.lambo1_ema_14_factor.value) & (dataframe['rsi_4'] < int(self.lambo1_rsi_4_limit.value)) & (dataframe['rsi_14'] < int(self.lambo1_rsi_14_limit.value)) & (dataframe['cti'] < -0.5) & (dataframe['recent_pump'] == False)
        dataframe.loc[lambo1, 'enter_tag'] += 'lambo1_'
        conditions.append(lambo1)
        lambo2 = bool(self.lambo2_enabled.value) & (dataframe['close'] < dataframe['ema_14'] * self.lambo2_ema_14_factor.value) & (dataframe['rsi_4'] < int(self.lambo2_rsi_4_limit.value)) & (dataframe['rsi_14'] < int(self.lambo2_rsi_14_limit.value)) & (dataframe['cti'] < -0.5) & (dataframe['recent_pump'] == False)
        dataframe.loc[lambo2, 'enter_tag'] += 'lambo2_'
        conditions.append(lambo2)
        local_uptrend = bool(self.local_trend_enabled.value) & (dataframe['ema_26'] > dataframe['ema_14']) & (dataframe['ema_26'] - dataframe['ema_14'] > dataframe['open'] * self.local_trend_ema_diff.value) & (dataframe['ema_26'].shift() - dataframe['ema_14'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb_lowerband2'] * self.local_trend_bb_factor.value) & (dataframe['closedelta'] > dataframe['close'] * self.local_trend_closedelta.value / 1000) & (dataframe['recent_pump'] == False)
        dataframe.loc[local_uptrend, 'enter_tag'] += 'local_uptrend_'
        conditions.append(local_uptrend)
        nfi_32 = bool(self.nfi32_enabled.value) & (dataframe['rsi_20'] < dataframe['rsi_20'].shift(1)) & (dataframe['rsi_4'] < self.nfi32_rsi_4.value) & (dataframe['rsi_14'] > self.nfi32_rsi_14.value) & (dataframe['close'] < dataframe['sma_15'] * self.nfi32_sma_factor.value) & (dataframe['cti'] < self.nfi32_cti_limit.value) & (dataframe['recent_pump'] == False)
        dataframe.loc[nfi_32, 'enter_tag'] += 'nfi_32_'
        conditions.append(nfi_32)
        ewo_1 = bool(self.ewo_1_enabled.value) & (dataframe['rsi_4'] < self.ewo_1_rsi_4.value) & (dataframe['close'] < dataframe[f'ma_entry_{self.ewo_candles_entry.value}'] * self.ewo_low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi_14'] < self.ewo_1_rsi_14.value) & (dataframe['close'] < dataframe[f'ma_exit_{self.ewo_candles_exit.value}'] * self.ewo_high_offset.value) & (dataframe['recent_pump'] == False)
        dataframe.loc[ewo_1, 'enter_tag'] += 'ewo1_'
        conditions.append(ewo_1)
        ewo_low = bool(self.ewo_low_enabled.value) & (dataframe['rsi_4'] < self.ewo_low_rsi_4.value) & (dataframe['close'] < dataframe[f'ma_entry_{self.ewo_candles_entry.value}'] * self.ewo_low_offset.value) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['close'] < dataframe[f'ma_exit_{self.ewo_candles_exit.value}'] * self.ewo_high_offset.value) & (dataframe['recent_pump'] == False)
        dataframe.loc[ewo_low, 'enter_tag'] += 'ewo_low_'
        conditions.append(ewo_low)
        cofi = bool(self.cofi_enabled.value) & (dataframe['open'] < dataframe['ema_8'] * self.cofi_ema.value) & qtpylib.crossed_above(dataframe['fastk'], dataframe['fastd']) & (dataframe['fastk'] < self.cofi_fastk.value) & (dataframe['fastd'] < self.cofi_fastd.value) & (dataframe['adx'] > self.cofi_adx.value) & (dataframe['EWO'] > self.cofi_ewo_high.value) & (dataframe['recent_pump'] == False)
        dataframe.loc[cofi, 'enter_tag'] += 'cofi_'
        conditions.append(cofi)
        clucHA = bool(self.clucha_enabled.value) & dataframe['rocr_1h'].gt(self.clucha_rocr_1h.value) & (dataframe['lower'].shift().gt(0) & dataframe['bbdelta'].gt(dataframe['ha_close'] * self.clucha_bbdelta_close.value) & dataframe['ha_closedelta'].gt(dataframe['ha_close'] * self.clucha_closedelta_close.value) & dataframe['tail'].lt(dataframe['bbdelta'] * self.clucha_bbdelta_tail.value) & dataframe['ha_close'].lt(dataframe['lower'].shift()) & dataframe['ha_close'].le(dataframe['ha_close'].shift()) & (dataframe['recent_pump'] == False) | (dataframe['ha_close'] < dataframe['ema_slow']) & (dataframe['ha_close'] < self.clucha_close_bblower.value * dataframe['bb_lowerband']) & (dataframe['recent_pump'] == False))
