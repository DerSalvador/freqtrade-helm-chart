...
class NowoIchimoku1hV1(IStrategy):
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df['entry_allowed'] = -df['is_cloud_green']
        for i in range(101, len(df)):
            df.loc[i, 'entry_allowed'] = df.loc[i - 1, 'entry_allowed']
            if df.loc[i - 1, 'entry']:
                df.loc[i, 'entry_allowed'] = False
            elif not df.loc[i, 'is_cloud_green']:
                df[i, 'entry_allowed'] = True
        df.loc[df['entry_allowed'] & df['should_entry'], 'entry'] = 1
        return df
