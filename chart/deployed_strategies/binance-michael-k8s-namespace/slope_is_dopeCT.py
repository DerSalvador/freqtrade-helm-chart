...
class slope_is_dopeCT(IStrategy):
    ...
    # hyperopt params
    slope_length = IntParameter(5, 30, default=11, space="entry", optimize=True)
    stoploss_length = IntParameter(5, 15, default=10, space="exit", optimize=True)
    rsi_length = IntParameter(5, 14, default=14, space="entry",optimize=True)
    rsi_entry = IntParameter(30, 60, default=55, space="entry", optimize=True)
    fslope_entry = IntParameter(-5, 5, default=0, space="entry", optimize=True)
    sslope_entry = IntParameter(-5, 5, default=0, space="entry", optimize=True)
    fslope_exit = IntParameter(-5, 5, default=0, space="exit", optimize=True)
    ...
    tsl_target3 = DecimalParameter(low=0.10, high=0.15, default=0.15, decimals=2,  space='exit', optimize=True, load=True)
    ts3 = DecimalParameter(low=0.025, high=0.04, default=0.035, decimals=3,  space='exit', optimize=True, load=True)
    tsl_target2 = DecimalParameter(low=0.08, high=0.10, default=0.1, decimals=3, space='exit', optimize=True, load=True)
    ...
    tsl_target0 = DecimalParameter(low=0.03, high=0.05, default=0.03, decimals=3, space='exit', optimize=True, load=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                ...
                (dataframe['fast_slope'] > self.fslope_entry.value) &
                # Only trade when the slow slope is above 0
                (dataframe['slow_slope'] > self.sslope_entry.value) &
                # Only entry when the close price is higher than the 3day average of ten periods ago
                # (dataframe['close'] > dataframe['entryMA'].shift(+(slope_length))) &
                # Or only entry when the close price is higher than the close price of 3 days ago (this is a choice)
                ...
                (dataframe[f'rsi_{self.rsi_length.value}'] > self.rsi_entry.value) 
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                ...
                (dataframe['fast_slope'] < self.fslope_exit.value)
