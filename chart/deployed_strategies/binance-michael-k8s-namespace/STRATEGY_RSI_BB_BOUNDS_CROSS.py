...
class STRATEGY_RSI_BB_BOUNDS_CROSS(IStrategy):
    ...
    #     '0': {},
    # },
    plot_config = {'main_plot': {'tema': {}, 'sar': {'color': 'white'}, 'bb_ub': {'color': 'green'}, 'bb_lb': {'color': 'green'}, 'bb_lb_smoothed': {'color': 'red'}, 'rsi_ub': {'color': 'black'}, 'rsi_lb': {'color': 'black'}, 'rsi_lb_smoothed': {'color': 'orange'}}, 'subplots': {'bb_rsi_count': {'ub_bb_over_rsi_trend': {}, 'lb_bb_under_rsi_trend': {}}}}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[dataframe['lb_bb_under_rsi_trend'] & (dataframe['bb_lb'] < dataframe['rsi_lb']) & (dataframe['bb_lb'].shift(1) < dataframe['rsi_lb'].shift(1)) & (dataframe['ema2'] > dataframe['ema2'].shift(1)), 'entry'] = 1
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # if ema2 is falling
        dataframe.loc[dataframe['ema2'] < dataframe['ema2'].shift(1), 'exit'] = 1
        return dataframe