...
from freqtrade.strategy import BooleanParameter, timeframe_to_minutes, merge_informative_pair, DecimalParameter, IntParameter, CategoricalParameter
...
class ichiV1_Marius(IStrategy):
    ...
    ##
    ##
    # "entry_fan_magnitude_shift_value": 3,
    ...
    entry_params = {'max_slip': 0.668, 'antipump_threshold': 0.265, 'antipump_threshold_2': 0.133, 'entry_btc_safe_1d': -0.236, 'entry_btc_safe': -213, 'entry_threshold': 0.012, 'pump_limit': 1000, 'pump_pause_duration': 192, 'pump_period': 14, 'pump_recorver_price': 1.1, 'entry_trend_above_senkou_level': 1, 'entry_trend_bullish_level': 6, 'tesla_enabled': True, 'entry_min_fan_magnitude_gain': 1.0022}
    exit_params = {'ProfitLoss1': 0.005, 'ProfitLoss2': 0.021, 'ProfitMargin1': 0.018, 'ProfitMargin2': 0.051, 'pHSL': -0.08, 'exit_trend_indicator': 'trend_close_2h'}
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float, min_stake: float, max_stake: float, **kwargs):
        ...
        if 0 < count_of_entrys <= self.max_dca_orders:
            try:
        ...
        return None
    # Pump protection
    pump_period = IntParameter(5, 24, default=entry_params['pump_period'], space='entry', optimize=False)
    pump_limit = IntParameter(100, 10000, default=entry_params['pump_limit'], space='entry', optimize=True)
    pump_recorver_price = DecimalParameter(1.0, 1.3, default=entry_params['pump_recorver_price'], space='entry', optimize=True)
    pump_pause_duration = IntParameter(6, 500, default=entry_params['pump_pause_duration'], space='entry', optimize=True)
    ##################################################################    
    ## Slippage params
    is_optimize_slip = False
    max_slip = DecimalParameter(0.33, 0.8, default=0.33, decimals=3, optimize=is_optimize_slip, load=True)
    entry_btc_safe = IntParameter(-300, 50, default=entry_params['entry_btc_safe'], optimize=True)
    ...
    antipump_threshold_2 = DecimalParameter(0, 0.4, default=entry_params['antipump_threshold_2'], space='entry', optimize=True)
    entry_min_fan_magnitude_gain = DecimalParameter(70, 90, default=entry_params['entry_min_fan_magnitude_gain'], space='entry', optimize=False, load=True)  # Multi Offset
    entry_threshold = DecimalParameter(0.003, 0.012, default=entry_params['entry_threshold'], optimize=True)
    #######################################################################
    # ROI table:
    ...
    window_entry = IntParameter(60, 1000, default=500, space='entry', optimize=True)
    bandwidth_entry = IntParameter(2, 15, default=8, space='entry', optimize=True)
    mult_entry = DecimalParameter(0.5, 20.0, default=3, space='entry', optimize=True)
    ...
    plot_config = {'main_plot': {'senkou_a': {'color': 'green', 'fill_to': 'senkou_b', 'fill_label': 'Ichimoku Cloud', 'fill_color': 'rgba(255,76,46,0.2)'}, 'senkou_b': {}}, 'subplots': {'fan_magnitude': {'fan_magnitude': {}}, 'fan_magnitude_gain': {'fan_magnitude_gain': {}}}}
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float, time_in_force: str, exit_reason: str, current_time: datetime, **kwargs) -> bool:
        ...
        if 'tesla_' in trade.entry_tag and current_profit > 0.01:
            return True
        if trade.entry_tag == 'telsa_':
            if exit_reason in ['exit_signal'] or exit_reason in ['roi'] or exit_reason in ['trailing_stop_loss']:
                return False
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['entry_ok'] = np.where(dataframe['pump_dump_alert'], False, True)
        return dataframe
    ## Confirm Entry
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_protection = (pct_change(dataframe['btc_1d'], dataframe['btc_5m']).fillna(0) > self.entry_btc_safe_1d.value) & (dataframe['pump_strength_2'] < self.antipump_threshold_2.value) & dataframe['entry_ok'] & (dataframe['volume'] > 0)
        if self.entry_params['tesla_enabled'] >= True:
            tesla = (dataframe['rsi'] > dataframe['rsi_1h']) & (dataframe['trend_close_8h'] > dataframe['trend_close_6h']) & (dataframe['trend_close_15m'] > dataframe['trend_close_30m']) & (dataframe['trend_open_5m'] > dataframe['trend_open_15m']) & (dataframe['trend_close_1h'] > dataframe['ema55']) & (dataframe['ema21'] > dataframe['trend_close_4h']) & (dataframe['trend_open_1h'] > dataframe['trend_open_2h']) & (dataframe['mfi'] < 70) & (dataframe['fan_magnitude_gain'] >= self.entry_min_fan_magnitude_gain.value) & (dataframe['fan_magnitude'] > 0.99)
            conditions.append(tesla)
            dataframe.loc[tesla, 'entry_tag'] += 'tesla_'
        if conditions:
            dataframe.loc[is_protection & reduce(lambda x, y: x & y, conditions), 'entry'] = 1
        return dataframe
