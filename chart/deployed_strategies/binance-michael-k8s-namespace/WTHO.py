...
class WTHO(IStrategy):
    ...
    # indicators 
    entry_esa_length = IntParameter(3, 20, default=10, optimize=True)
    entry_d_length = IntParameter(3, 20, default=10, optimize=True)
    entry_wt1_length = IntParameter(15, 50, default=21, optimize=True)
    entry_wt2_length = IntParameter(3, 14, default=4, optimize=True)
    ...
    # values
    # entry
    wt_os = IntParameter(-55, 1 , default=-50, space="entry", optimize=True)
    wt_os_sma = IntParameter(-30, 50 , default=0, space="entry", optimize=True)
    entry_wt_pc = IntParameter(-30, 10 , default=0, space="entry", optimize=True)
    entry_mfi = IntParameter(0, 60 , default=50, space="entry", optimize=True)
    entry_mfi_slope = IntParameter(-20, 20 , default=0, space="entry", optimize=True)
    entry_sma_pc = IntParameter(-10, 10 , default=0, space="entry", optimize=True)

    # exit
    wt_ob = IntParameter(20, 55 , default=50, space="exit", optimize=True)
    wt_ob_sma = IntParameter(-20, 55 , default=0, space="exit", optimize=True)
    exit_wt_pc = IntParameter(-10, 30 , default=0, space="exit", optimize=True)
    exit_mfi = IntParameter(40, 100 , default=70, space="exit", optimize=True)
    exit_mfi_slope = IntParameter(-20, 20 , default=0, space="exit", optimize=True)
    exit_sma_pc = IntParameter(-10, 10 , default=0, space="exit", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for vale in self.entry_esa_length.range:
            dataframe[f'esa_{vale}'] = ta.EMA(ap, timeperiod = vale)
       
        # find ema length for d       
        for vald in self.entry_d_length.range:
        ...
        # if dataframe[f'sma_{self.sma_length.value}pc'] > self.entry_sma_pc.value:
        #     dataframe['entry_os'] = self.wt_os_sma.value
        # else:
        #     dataframe['entry_os']= self.wt_os.value 
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # dataframe.loc[
        #     (
        #         (dataframe[f'sma_{self.sma_length.value}pc'] >= self.entry_sma_pc.value) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] <= self.wt_os.value ) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] > dataframe[f'wave_t2_{self.entry_wt2_length.value}']) &
        #         (dataframe['wave_t1_pc'] >= self.entry_wt_pc.value) &
        #         (dataframe[f'mfi_sma{self.mfi_sma_length.value}'] <= self.entry_mfi.value ) &
        #         (dataframe['mfi_pc'] >= self.entry_mfi_slope.value ) &
        #         (dataframe['volume'] > 0)

        #     ),
        #     ['entry', 'entry_tag']] = (1, 'Bear')

        # dataframe.loc[
        #     (
        #         (dataframe[f'sma_{self.sma_length.value}pc'] > self.entry_sma_pc.value) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] <= self.wt_os_sma.value ) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] > self.wt_os.value ) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] > dataframe[f'wave_t2_{self.entry_wt2_length.value}']) &
        #         (dataframe['wave_t1_pc'] >= self.entry_wt_pc.value) &
        #         (dataframe[f'mfi_sma{self.mfi_sma_length.value}'] <= self.entry_mfi.value ) &
        ...
        #     ),
        #     ['entry', 'entry_tag']] = (1, 'Bull')



        # conditions.append(dataframe[f'wave_t1_{self.entry_wt1_length.value}'] >= self.wt_os.value )
        conditions.append(dataframe[f'wave_t1_{self.entry_wt1_length.value}'] > dataframe[f'wave_t2_{self.entry_wt2_length.value}'])
        conditions.append(dataframe['wave_t1_pc'] >= self.entry_wt_pc.value)

        # Money Flow Index
        conditions.append(dataframe[f'mfi_sma{self.mfi_sma_length.value}'] <= self.entry_mfi.value )
        conditions.append(dataframe['mfi_pc'] >= self.entry_mfi_slope.value )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # dataframe.loc[
        #     (
        #         (dataframe[f'sma_{self.sma_length.value}pc'] <= self.exit_sma_pc.value) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] >= self.wt_ob.value ) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] < dataframe[f'wave_t2_{self.entry_wt2_length.value}']) &
        #         (dataframe['wave_t1_pc'] <= self.exit_wt_pc.value) &
        #         (dataframe[f'mfi_sma{self.mfi_sma_length.value}'] >= self.exit_mfi.value ) &
        #         (dataframe['mfi_pc'] <= self.exit_mfi_slope.value ) &
        #         (dataframe['volume'] > 0)
        ...
        #         (dataframe[f'sma_{self.sma_length.value}pc'] < self.exit_sma_pc.value) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] <= self.wt_ob.value ) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] >= self.wt_ob_sma.value ) &
        #         (dataframe[f'wave_t1_{self.entry_wt1_length.value}'] < dataframe[f'wave_t2_{self.entry_wt2_length.value}']) &
        #         (dataframe['wave_t1_pc'] <= self.exit_wt_pc.value) &
        #         (dataframe[f'mfi_sma{self.mfi_sma_length.value}'] >= self.exit_mfi.value ) &
        #         (dataframe['mfi_pc'] <= self.exit_mfi_slope.value ) &
        ...
        # #Wave Trend
        # if (dataframe['sma_pc'] < self.exit_sma_pc.value) and (dataframe[f'wave_t1_{self.exit_wt1_length.value}'] < self.wt_ob.value ):
        #     conditions.append(dataframe[f'wave_t1_{self.self_wt1_length.value}'] >= self.wt_ob_sma.value )
        # else:
        # conditions.append(dataframe[f'wave_t1_{self.entry_wt1_length.value}'] <= self.wt_ob.value)

        conditions.append(dataframe[f'wave_t2_{self.entry_wt2_length.value}'] <= dataframe[f'wave_t1_{self.entry_wt1_length.value}'])
        conditions.append(dataframe['wave_t1_pc'] <= self.exit_wt_pc.value)

        # Money Flow Index
        conditions.append(dataframe[f'mfi_sma{self.mfi_sma_length.value}'] >= self.exit_mfi.value )
        conditions.append(dataframe['mfi_pc'] <= self.exit_mfi_slope.value )
