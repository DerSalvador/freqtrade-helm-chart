...
class Ichess(IStrategy):
    ...
    # Opt Timeframe
    timeframe = '1d'
    entry_fast_timeperiod = IntParameter(2, 50, default=9, space='entry')
    entry_slow_timeperiod = IntParameter(2, 50, default=10, space='entry')
    exit_fast_timeperiod = IntParameter(2, 50, default=15, space='exit')
    exit_slow_timeperiod = IntParameter(2, 50, default=16, space='exit')
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        def calcTkCross(tenkan, kijun, tenkan1, kijun1, senkou_span_a, senkou_span_b):
            if tenkan > kijun and tenkan1 <= kijun1:
                intersect = (tenkan1 * (kijun - kijun1) - kijun1 * (tenkan - tenkan1)) / (kijun - kijun1 - (tenkan - tenkan1))
        ...
        df['tkCrossScore'] = np.vectorize(calcTkCross)(df['tenkan'], df['kijun'], df['tenkan1'], df['kijun1'], df['senkou_span_a'], df['senkou_span_b'])
        # // == Price and Kijun Sen (standard line) Cross ==

        def calcPkCross(ha_close, kijun, ha_close1, kijun1, senkou_span_a, senkou_span_b):
            if ha_close > kijun and ha_close1 <= kijun1:
                intersect = (ha_close1 * (kijun - kijun1) - kijun1 * (ha_close - ha_close1)) / (kijun - kijun1 - (ha_close - ha_close1))
                if intersect > senkou_span_a and intersect > senkou_span_b:
                    return 2
                elif intersect < senkou_span_a and intersect < senkou_span_b:
                    return 0.5
                ...
                return 0
        df['pkCrossScore'] = np.vectorize(calcPkCross)(df['ha_close'], df['kijun'], df['ha_close1'], df['kijun1'], df['senkou_span_a'], df['senkou_span_b'])
        # // == Kumo Breakouts ==

        def calcKumoBreakout(ha_close, senkou_span_a, ha_close1, senkou_span_a1, senkou_span_b, senkou_span_b1):
            if ha_close > senkou_span_a and ha_close1 <= senkou_span_a1 and (senkou_span_a > senkou_span_b) or (ha_close > senkou_span_b and ha_close1 <= senkou_span_b1 and (senkou_span_a < senkou_span_b)):
                ...
                return 0
        df['kumoBreakoutScore'] = np.vectorize(calcKumoBreakout)(df['ha_close'], df['senkou_span_a'], df['ha_close1'], df['senkou_span_a1'], df['senkou_span_b'], df['senkou_span_b1'])
        # // == Senkou Span Cross ==

        def calcSenkouCross(senkou_leading_a, senkou_leading_b, senkou_leading_a1, senkou_leading_b1, ha_close, senkou_span_a, senkou_span_b):
            if senkou_leading_a > senkou_leading_b and senkou_leading_a1 <= senkou_leading_b1:
                ...
                return 0
        df['senkouCrossScore'] = np.vectorize(calcSenkouCross)(df['senkou_leading_a'], df['senkou_leading_b'], df['senkou_leading_a1'], df['senkou_leading_b1'], df['ha_close'], df['senkou_span_a'], df['senkou_span_b'])
        # // == Chikou Span Cross ==

        def calcChikouCross(ha_close, chikou_span, ha_close1, chikou_span1, senkou_span_a, senkou_span_b):
            if ha_close > chikou_span and ha_close1 <= chikou_span1:
                intersect = (ha_close1 * (chikou_span - chikou_span1) - chikou_span1 * (ha_close - ha_close1)) / (chikou_span - chikou_span1 - (ha_close - ha_close1))
                if intersect > senkou_span_a and intersect > senkou_span_b:
                    return 2
                elif intersect < senkou_span_a and intersect < senkou_span_b:
                    return 0.5
                ...
                return 0
        df['chikouCrossScore'] = np.vectorize(calcChikouCross)(df['ha_close'], df['chikou_span'], df['ha_close1'], df['chikou_span1'], df['senkou_span_a'], df['senkou_span_b'])
        # // == price relative to cloud ==

        def calcPricePlacement(ha_close, senkou_span_a, senkou_span_b):
            if ha_close > senkou_span_a and ha_close > senkou_span_b:
                return 2
            elif ha_close < senkou_span_a and ha_close < senkou_span_b:
                return -2
                ...
                return 0
        df['pricePlacementScore'] = np.vectorize(calcPricePlacement)(df['ha_close'], df['senkou_span_a'], df['senkou_span_b'])
        # // == lag line releative to cloud ==

        def calcChikouPlacement(ha_close, senkou_leading_a, senkou_leading_b):
            if ha_close > senkou_leading_a and ha_close > senkou_leading_b:
                ...
                return 0
        df['chikouPlacementScore'] = np.vectorize(calcChikouPlacement)(df['ha_close'], df['senkou_leading_a'], df['senkou_leading_b'])
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # //
        dataframe['Ichimoku_Score'] = (df['tkCrossScore'] + df['pkCrossScore'] + df['kumoBreakoutScore'] + df['senkouCrossScore'] + df['chikouCrossScore']).rolling(1).sum().cumsum()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[qtpylib.crossed_above(ta.SMA(dataframe['Ichimoku_Score'], self.entry_fast_timeperiod.value), ta.SMA(dataframe['Ichimoku_Score'], self.entry_slow_timeperiod.value)), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # (dataframe['Ichimoku_Score']<0)|
        dataframe.loc[qtpylib.crossed_below(ta.SMA(dataframe['Ichimoku_Score'], self.exit_fast_timeperiod.value), ta.SMA(dataframe['Ichimoku_Score'], self.exit_slow_timeperiod.value)), 'exit'] = 1
        return dataframe