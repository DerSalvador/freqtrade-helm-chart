...
class BB_RPB_TSL_c7c477d_20211030(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[is_cofi_checked, 'entry_tag'] += 'cofi '
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # Make sure Volume is not 0
        dataframe.loc[((dataframe['btc_diff'] < self.exit_btc_safe.value) | (dataframe['close'] < dataframe['ema_200'] * self.exit_ema.value) & (dataframe['cmf'] < self.exit_cmf.value) & ((dataframe['ema_200'] - dataframe['close']) / dataframe['close'] < self.exit_ema_close_delta.value) & (dataframe['rsi'] > dataframe['rsi'].shift(1))) & (dataframe['volume'] > 0), 'exit'] = 1
        return dataframe