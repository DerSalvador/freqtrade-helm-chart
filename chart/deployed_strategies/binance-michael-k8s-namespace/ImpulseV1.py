...
class ImpulseV1(IStrategy):
    ...
    # entry optizimation
    max_epa = CategoricalParameter([0, 1, 2], default=0, space='entry', optimize=True)
    # protections
    cooldown_lookback = IntParameter(2, 48, default=5, space='protection', optimize=True)
    stop_duration = IntParameter(12, 120, default=72, space='protection', optimize=True)
    use_stop_protection = BooleanParameter(default=True, space='protection', optimize=True)
    # indicators
    ma_length = IntParameter(8, 30, default=15, space='entry', optimize=True)
    ...
    tsl_target2 = DecimalParameter(low=0.06, high=0.1, default=0.1, decimals=3, space='exit', optimize=True, load=True)
    ts2 = DecimalParameter(low=0.015, high=0.03, default=0.02, decimals=3, space='exit', optimize=True, load=True)
    tsl_target1 = DecimalParameter(low=0.03, high=0.06, default=0.06, decimals=3, space='exit', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.01, high=0.016, default=0.013, decimals=3, space='exit', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.02, high=0.03, default=0.03, decimals=3, space='exit', optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for ma_len in self.ma_length.range:
            ...
            dataframe[f'md_{ma_len}'] = np.where(dataframe[f'mi_{ma_len}'] > dataframe[f'hi_{ma_len}'], dataframe[f'mi_{ma_len}'] - dataframe[f'hi_{ma_len}'], np.where(dataframe[f'mi_{ma_len}'] < dataframe[f'lo_{ma_len}'], dataframe[f'mi_{ma_len}'] - dataframe[f'lo_{ma_len}'], 0))
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:  # Bullish 0 Cross - Lime Color on TV
        # Make sure Volume is not 0
        df.loc[(df[f'sh_{self.ma_length.value}'] > df[f'sh_{self.ma_length.value}'].shift(1)) & (df['HLC3'] > df[f'hi_{self.ma_length.value}']) & qtpylib.crossed_above(df[f'sh_{self.ma_length.value}'], 0) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Impulse Bullish Buy')  # Bearish 0 Cross - Green Color on TV
        # Make sure Volume is not 0
        df.loc[(df[f'sh_{self.ma_length.value}'] > df[f'sh_{self.ma_length.value}'].shift(1)) & (df['HLC3'] < df[f'hi_{self.ma_length.value}']) & (df['HLC3'] > df[f'mi_{self.ma_length.value}']) & (df['HLC3'].shift(1) < df[f'hi_{self.ma_length.value}'].shift(1)) & (df['HLC3'].shift(1) > df[f'mi_{self.ma_length.value}'].shift(1)) & qtpylib.crossed_above(df[f'sh_{self.ma_length.value}'], 0) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Impulse Bearish Buy')
        return df
    ### EXIT CONDITIONS ###

    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:  # Bullish Exit - Orange Color on TV
        # (qtpylib.crossed_below(df[f'sh_{self.ma_length.value}'], 0)) &
        # Make sure Volume is not 0
        df.loc[(df[f'sh_{self.ma_length.value}'] > df[f'sh_{self.ma_length.value}'].shift(1)) & (df['HLC3'] < df[f'mi_{self.ma_length.value}']) & (df['HLC3'] > df[f'lo_{self.ma_length.value}']) & (df['HLC3'].shift(1) < df[f'mi_{self.ma_length.value}'].shift(1)) & (df['HLC3'].shift(1) > df[f'lo_{self.ma_length.value}'].shift(1)) & (df['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'Impulse Bullish Sell')  # Bearish Exit - Red Color on TV
        # Make sure Volume is not 0
        df.loc[(df[f'sh_{self.ma_length.value}'] > df[f'sh_{self.ma_length.value}'].shift(1)) & (df['HLC3'] < df[f'lo_{self.ma_length.value}']) & (df['30_SMA'] > df['200_SMA']) & (df['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'Impulse Bearish Sell')
        return df
# # 1HR
# 2023-02-07 19:18:33,386 - freqtrade.optimize.backtesting - INFO - Running backtesting for Strategy ImpulseV1
# 2023-02-07 19:18:33,387 - freqtrade.strategy.hyper - INFO - No params for entry found, using default values.
# 2023-02-07 19:18:33,387 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): entry_rsi = 25
...
# 2023-02-07 19:18:33,388 - freqtrade.strategy.hyper - INFO - No params for exit found, using default values.
# 2023-02-07 19:18:33,388 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): exit_rsi = 55
...
# 2023-02-07 19:20:48,997 - freqtrade.strategy.hyper - INFO - No params for entry found, using default values.
# 2023-02-07 19:20:48,998 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): entry_rsi = 25
...
# 2023-02-07 19:20:48,998 - freqtrade.strategy.hyper - INFO - No params for exit found, using default values.
# 2023-02-07 19:20:48,999 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): exit_rsi = 55
...
# 2023-02-07 19:24:10,531 - freqtrade.strategy.hyper - INFO - No params for entry found, using default values.
# 2023-02-07 19:24:10,532 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): entry_rsi = 25
...
# 2023-02-07 19:24:10,533 - freqtrade.strategy.hyper - INFO - No params for exit found, using default values.
# 2023-02-07 19:24:10,533 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): exit_rsi = 55
