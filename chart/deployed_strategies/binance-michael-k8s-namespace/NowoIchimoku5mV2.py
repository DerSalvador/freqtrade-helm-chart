...
def merge_informative_pair(dataframe: pd.DataFrame, informative: pd.DataFrame, timeframe: str, timeframe_inf: str, ffill: bool=True) -> pd.DataFrame:
    """
   ...
   """
    ...
    dataframe = pd.merge(dataframe, informative, left_on='date', right_on=f'date_merge_{timeframe_inf}', how='left')
...
class NowoIchimoku5mV2(IStrategy):
    ...
    def populate_indicators(self, df_5m: DataFrame, metadata: dict) -> DataFrame:
        ...
        stochrsi = (df_1h['rsi'] - df_1h['rsi'].rolling(lengthStoch).min()) / (df_1h['rsi'].rolling(lengthStoch).max() - df_1h['rsi'].rolling(lengthStoch).min())
        ...
        df.rename(columns=lambda s: s.replace('_{}'.format(self.informative_timeframe), '') if not s in skip_columns else s, inplace=True)
        return df
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        for i in range(1, len(df)):
            df.loc[i, 'entry_allowed'] = df.at[i - 1, 'entry_allowed']
            if df.at[i - 1, 'entry']:
                df.loc[i, 'entry_allowed'] = False
            if not df.at[i, 'is_cloud_green']:
                df.loc[i, 'entry_allowed'] = True
            df.loc[i, 'entry'] = df.at[i, 'entry_allowed'] & df.at[i, 'should_entry']
        return df
