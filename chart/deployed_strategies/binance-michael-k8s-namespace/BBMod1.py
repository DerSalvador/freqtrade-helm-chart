...
class BBMod1(IStrategy):
    ...
    entry_clucha_close_bblower = DecimalParameter(0.0005, 0.02, default=0.00799, optimize=is_optimize_clucha)
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if hasattr(trade, 'entry_tag') and trade.entry_tag is not None:
            entry_tag = trade.entry_tag
        entry_tags = entry_tag.split()
        if current_profit >= 0.01 and len(entry_tags) == 1:
            for i in self.lower_trailing_list:
                if i in entry_tags:
                    return None
        if current_profit >= 0.019:
        ...
        if 0.019 > current_profit >= 0.0:
            if last_candle['cti'] > self.exit_cti_r_cti.value and last_candle['r_14'] > self.exit_cti_r_r.value:
                return f'exit_profit_cti_r_0_1( {entry_tag})'
            elif entry_tag in ['nfix_39 ']:  # handle nfix_39 entry condition
                if last_candle['fisher'] > 0.39075 and last_candle['ha_high'] <= previous_candle_1['ha_high'] and (previous_candle_1['ha_high'] <= previous_candle_2['ha_high']) and (last_candle['ha_close'] <= previous_candle_1['ha_close']) and (last_candle['ema_4'] > last_candle['ha_close']) and (last_candle['ha_close'] * 0.99754 > last_candle['bb_middleband2']):
                    return f'exit_scalp( {entry_tag})'
        # when loss is -,use exit signal.
        if current_profit < 0.0 and last_candle['close'] > last_candle['sma_9'] and (last_candle['close'] > last_candle['ema_24'] * self.high_offset_2.value) and (last_candle['rsi'] > 50) and (last_candle['rsi_fast'] > last_candle['rsi_slow']):
            return f'exit_offset( {entry_tag})'
        if current_profit < 0.0 and last_candle['close'] > last_candle['ema_49'] * 1.006:
            return f'exit_offset2( {entry_tag})'
    ############################################################################
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        #     )
        # use origin bb_rpb_tsl value
        is_break = (dataframe['bb_delta'] > self.entry_bb_delta.value) & (dataframe['bb_width'] > self.entry_bb_width.value) & (dataframe['closedelta'] > dataframe['close'] * 17.922 / 1000) & (dataframe['close'] < dataframe['bb_lowerband3'] * self.entry_bb_factor.value)  # from NFI next gen, credit goes to @iterativ
        is_local_uptrend = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.entry_ema_diff.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb_lowerband2'] * self.entry_bb_factor.value) & (dataframe['closedelta'] > dataframe['close'] * self.entry_closedelta.value / 1000)  # use origin bb_rpb_tsl value
        is_local_uptrend2 = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * 0.026) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb_lowerband2'] * self.entry_bb_factor.value) & (dataframe['closedelta'] > dataframe['close'] * 17.922 / 1000)
        ...
        #     )
        is_clucha = dataframe['rocr_1h'].gt(self.entry_clucha_rocr_1h.value) & dataframe['bb_lowerband2_40'].shift().gt(0) & dataframe['bb_delta_cluc'].gt(dataframe['ha_close'] * self.entry_clucha_bbdelta_close.value) & dataframe['ha_closedelta'].gt(dataframe['ha_close'] * self.entry_clucha_closedelta_close.value) & dataframe['tail'].lt(dataframe['bb_delta_cluc'] * self.entry_clucha_bbdelta_tail.value) & dataframe['ha_close'].lt(dataframe['bb_lowerband2_40'].shift()) & dataframe['ha_close'].le(dataframe['ha_close'].shift())
        is_clucha2 = dataframe['rocr_1h'].gt(self.entry_clucha_rocr_1h.value) & (dataframe['ha_close'] < dataframe['ema_slow']) & (dataframe['ha_close'] < self.entry_clucha_close_bblower.value * dataframe['bb_lowerband2_40'])  # Modified from cofi, credit goes to original author "slack user CofiBit"
        is_cofi = (dataframe['open'] < dataframe['ema_8'] * self.entry_ema_cofi.value) & qtpylib.crossed_above(dataframe['fastk'], dataframe['fastd']) & (dataframe['fastk'] < self.entry_fastk.value) & (dataframe['fastd'] < self.entry_fastd.value) & (dataframe['adx'] > self.entry_adx.value) & (dataframe['EWO'] > self.entry_ewo_high.value) & (dataframe['cti'] < self.entry_cofi_cti.value) & (dataframe['r_14'] < self.entry_cofi_r14.value)
        ...
        is_nfix_39 = (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & dataframe['bb_lowerband2_40'].shift().gt(0) & dataframe['bb_delta_cluc'].gt(dataframe['close'] * 0.056) & dataframe['closedelta'].gt(dataframe['close'] * 0.01) & dataframe['tail'].lt(dataframe['bb_delta_cluc'] * 0.5) & dataframe['close'].lt(dataframe['bb_lowerband2_40'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['close'] > dataframe['ema_13'] * self.entry_nfix_39_ema.value)
        ...
        is_vwap = (dataframe['close'] < dataframe['vwap_low']) & (dataframe['tcp_percent_4'] > 0.04) & (dataframe['cti'] < -0.8) & (dataframe['rsi'] < 35) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & (dataframe['volume'] > 0)
        # Additional Check
        ...
        dataframe.loc[is_clucha2, 'entry_tag'] += 'clucHA2 '
        ...
        dataframe.loc[is_local_uptrend2, 'entry_tag'] += 'local_uptrend2 '
        if conditions:
