...
class Schism3(IStrategy):
    ...
    entry_params = {'bounce-lookback': 8, 'bounce-price': 'min', 'down-inf-rsi': 37, 'down-mp': 60, 'down-rmi-fast': 28, 'down-rmi-slow': 35, 'up-inf-rsi': 59, 'xinf-stake-rmi': 70, 'xtf-fiat-rsi': 15, 'xtf-stake-rsi': 60}
    exit_params = {'rmi-high': 50, 'rmi-low': 10}
    # Pair Specific Buy/Sell Params
    entry_params_FOO = {}
    exit_params_FOO = {}
    minimal_roi = {'0': 0.05, '10': 0.025, '20': 0.015, '30': 0.01, '720': 0.005, '1440': 0}
    stoploss = -0.3
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['bounce-pending'] = np.where((dataframe[f'rsi_{self.inf_timeframe}'] >= params['down-inf-rsi']) & (dataframe['rmi-dn-trend'] == 1) & (dataframe['rmi-slow'] >= params['down-rmi-slow']) & (dataframe['rmi-fast'] <= params['down-rmi-fast']) & (dataframe['mp'] <= params['down-mp']), 1, 0)
        # Capture the price where we got the signal that a bounce would happen
        dataframe['bounce-price'] = np.where(dataframe['bounce-pending'] == 1, dataframe['close'], getattr(dataframe['close'].rolling(params['bounce-lookback'], min_periods=1), params['bounce-price'])())
        ...
        # when this entry signal is not present a exit can happen according to the defined ROI table
        if trade_data['active_trade']:
            ...
            conditions.append((dataframe[f'rsi_{self.inf_timeframe}'] >= params['up-inf-rsi']) & (dataframe['bounce-range'] == 1) & (dataframe['rmi-up-trend'] == 1) & (dataframe['close'] >= dataframe['bounce-price']))
            # If the stake is BTC or ETH apply additional conditions
            if self.config['stake_currency'] in ('BTC', 'ETH'):
                # default timeframe conditions
                conditions.append((dataframe[f"{self.config['stake_currency']}_rsi"] < params['xtf-stake-rsi']) | (dataframe[f'{self.custom_fiat}_rsi'] > params['xtf-fiat-rsi']))
        ...
        # Anything below here applies to persisting and new entry signal
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if trade_data['active_trade']:
            # give some wiggle room for the trade to go negative a little bit before exiting
            ...
            # if we have other trades, consider their profit and the # of free slots in the exit
            if trade_data['other_trades']:
                if trade_data['free_slots'] > 0:
                    ...
                    # if were out of slots, allow the biggest losing trade to exit regardless of avg profit
