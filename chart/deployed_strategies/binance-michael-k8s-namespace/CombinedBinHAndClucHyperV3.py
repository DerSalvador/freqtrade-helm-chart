...
class CombinedBinHAndClucHyperV3(IStrategy):
    ...
    entry_a_atr_window = IntParameter(10, 300, default=14)
    entry_a_bbdelta_rate = DecimalParameter(0.004, 0.02, default=0.016, decimals=3)
    entry_a_closedelta_rate = DecimalParameter(0.0, 0.02, default=0.0087, decimals=4)
    entry_a_tail_rate = DecimalParameter(0.12, 1, default=0.28, decimals=2)
    entry_a_min_exit_rate = DecimalParameter(1.004, 1.1, default=1.03, decimals=3)
    entry_a_atr_rate = DecimalParameter(0.0, 3.0, default=1, decimals=2)
    ...
    # Sell
    exit_bb_mid_slow_window = IntParameter(10, 100, default=91)
    exit_trailing_stop_positive_offset = DecimalParameter(0.01, 0.03, default=0.012, decimals=3)
    exit_trailing_stop_positive = 0.001
    # ----------------------------------------------------------------
    # Buy hyperspace params:
    entry_params = {'entry_a_closedelta_rate': 0.004, 'entry_a_time_window': 21, 'entry_a_atr_window': 14, 'entry_a_atr_rate': 0.26, 'entry_a_bbdelta_rate': 0.014, 'entry_a_min_exit_rate': 1.062, 'entry_a_tail_rate': 0.47, 'entry_b_close_rate': 0.979, 'entry_b_time_window': 20, 'entry_b_ema_slow': 50, 'entry_b_volume_mean_slow_num': 20, 'entry_b_volume_mean_slow_window': 30}
    # Sell hyperspace params:
    exit_params = {'exit_bb_mid_slow_window': 91, 'exit_trailing_stop_positive_offset': 0.014}
    # ROI table:
    minimal_roi = {'0': 3}
    # Stoploss:
    stoploss = -0.06
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for x in self.exit_bb_mid_slow_window.range if isinstance(self.exit_bb_mid_slow_window, ABC) else [self.exit_bb_mid_slow_window]:
            exit_bollinger = qtpylib.bollinger_bands(qtpylib.typical_price(dataframe), window=x, stds=2)
            dataframe[f'bb_typical_mid_{x}'] = exit_bollinger['mid']
        for x in self.entry_a_atr_window.range if isinstance(self.entry_a_atr_window, ABC) else [self.entry_a_atr_window]:
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        entry_a_atr_rate = self.entry_a_atr_rate.value if isinstance(self.entry_a_atr_rate, ABC) else self.entry_a_atr_rate
        entry_a_atr_window = self.entry_a_atr_window.value if isinstance(self.entry_a_atr_window, ABC) else self.entry_a_atr_window
        ...
        exit_bb_mid_slow_window = self.exit_bb_mid_slow_window.value if isinstance(self.exit_bb_mid_slow_window, ABC) else self.exit_bb_mid_slow_window  # strategy BinHV45
        # strategy ClucMay72018
        dataframe.loc[dataframe[f'lower_{entry_a_time_window}'].shift().gt(0) & dataframe[f'bbdelta_{entry_a_time_window}'].gt(dataframe['close'] * entry_a_bbdelta_rate) & dataframe[f'closedelta_{entry_a_time_window}'].gt(dataframe['close'] * entry_a_closedelta_rate) & dataframe[f'tail_{entry_a_time_window}'].lt(dataframe[f'bbdelta_{entry_a_time_window}'] * entry_a_tail_rate) & dataframe['close'].lt(dataframe[f'lower_{entry_a_time_window}'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & dataframe[f'bb_typical_mid_{exit_bb_mid_slow_window}'].gt(dataframe['close'] * (entry_a_min_exit_rate + dataframe[f'atr_rate_{entry_a_atr_window}'] * entry_a_atr_rate)) | (dataframe['close'] < dataframe[f'ema_slow_{entry_b_ema_slow}']) & (dataframe['close'] < entry_b_close_rate * dataframe[f'bb_typical_lower_{entry_b_time_window}']) & (dataframe['volume'] < dataframe[f'volume_mean_slow_{entry_b_volume_mean_slow_window}'].shift(1) * entry_b_volume_mean_slow_num), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        exit_bb_mid_slow_window = self.exit_bb_mid_slow_window.value if isinstance(self.exit_bb_mid_slow_window, ABC) else self.exit_bb_mid_slow_window
        dataframe.loc[dataframe['close'] > dataframe[f'bb_typical_mid_{exit_bb_mid_slow_window}'], 'exit'] = 1
        return dataframe