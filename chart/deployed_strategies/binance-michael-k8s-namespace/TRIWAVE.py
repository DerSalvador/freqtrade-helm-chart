...
class TRIWAVE(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[dataframe['200_SMAPC'] < self.sma200_entry_pc.value, 'sma_entry2'] = -1
        dataframe['200SMA_weight'] = (dataframe['sma_entry1'] + dataframe['sma_entry2']) / 2 * self.x4.value
        ...
        dataframe['auto_entry'] = dataframe[['rsi_weight', 'willr_weight', '200SMA_weight', 'from_weight']].sum(axis=1)
        ### SELLING ###
        ...
        dataframe.loc[dataframe['200_SMAPC'] < self.sma200_exit_pc.value, 'sma_exit2'] = 1
        dataframe['200SMA_weight_exit'] = (dataframe['sma_exit1'] + dataframe['sma_exit2']) / 2 * self.y4.value
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[qtpylib.crossed_above(dataframe['auto_entry_decision'], self.auto_entry.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'auto entry bullzzz')
        dataframe.loc[qtpylib.crossed_above(dataframe['auto_entry_decision'], self.auto_entry.value + self.auto_entry_bearzzz.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'auto entry bearzzz')
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['auto_exit_decision'] >= self.auto_exit.value) & (dataframe['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'auto_exit')
        return dataframe