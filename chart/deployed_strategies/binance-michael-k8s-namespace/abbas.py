...
protection_params = {'cooldown_stop_duration_candles': 1, 'lowprofit2_lookback_period_candles': 107, 'lowprofit2_required_profit': 0.026, 'lowprofit2_stop_duration_candles': 8, 'lowprofit2_trade_limit': 6, 'lowprofit_lookback_period_candles': 34, 'lowprofit_required_profit': 0.027, 'lowprofit_stop_duration_candles': 126, 'lowprofit_trade_limit': 36, 'maxdrawdown_lookback_period_candles': 36, 'maxdrawdown_max_allowed_drawdown': 0.34, 'maxdrawdown_stop_duration_candles': 54, 'maxdrawdown_trade_limit': 17, 'stoplossguard_lookback_period_candles': 3, 'stoplossguard_stop_duration_candles': 8, 'stoplossguard_trade_limit': 11}
# Buy hyperspace params:
entry_params = {'base_nb_candles_entry': 17, 'ewo_high': 2.182, 'ewo_high_2': -3.44, 'ewo_low': -10.26, 'low_offset': 1.066, 'low_offset_2': 0.961, 'rsi_entry': 68, 'min_profit': 1.03}
# Sell hyperspace params:
exit_params = {'base_nb_candles_exit': 9, 'high_offset': 1.01, 'high_offset_2': 1.233, 'high_offset_ema': 0.931}
...
class abbas(IStrategy):
    ...
    maxdrawdown_max_allowed_drawdown = DecimalParameter(0.05, 0.4, default=protection_params['maxdrawdown_max_allowed_drawdown'], space='protection', decimals=2, optimize=maxdrawdown_optimize)
    ...
    lowprofit_required_profit = DecimalParameter(0.0, 0.05, default=protection_params['lowprofit_required_profit'], space='protection', decimals=3, optimize=lowprofit_optimize)
    ...
    lowprofit2_required_profit = DecimalParameter(0, 0.03, default=protection_params['lowprofit2_required_profit'], space='protection', decimals=3, optimize=lowprofit2_optimize)
    ...
    def protections(self):
        ...
        prot.append({'method': 'LowProfitPairs', 'lookback_period_candles': self.lowprofit_lookback_period_candles.value, 'trade_limit': self.lowprofit_trade_limit.value, 'stop_duration_candles': self.lowprofit_stop_duration_candles.value, 'required_profit': self.lowprofit_required_profit.value})
        prot.append({'method': 'LowProfitPairs', 'lookback_period_candles': self.lowprofit2_lookback_period_candles.value, 'trade_limit': self.lowprofit2_trade_limit.value, 'stop_duration_candles': self.lowprofit2_stop_duration_candles.value, 'required_profit': self.lowprofit2_required_profit.value})
        return prot
    ...
    class HyperOpt:
        ...
        def stoploss_space():
            return [SKDecimal(-0.08, -0.03, decimals=3, name='stoploss')]
        # Define custom trailing space

        def trailing_space() -> List[Dimension]:
            return [Categorical([True, False], name='trailing_stop'), SKDecimal(0.0001, 0.001, decimals=5, name='trailing_stop_positive'), SKDecimal(0.008, 0.018, decimals=4, name='trailing_stop_positive_offset_p1'), Categorical([True, False], name='trailing_only_offset_is_reached')]
    # ROI table:
    minimal_roi = {'200': 0}
    # Stoploss:
    stoploss = -0.04
    ...
    high_offset_ema = DecimalParameter(0.9, 1.1, default=exit_params['high_offset_ema'], load=True, space='exit', decimals=3, optimize=smaoffset_optimize)
    base_nb_candles_entry = IntParameter(15, 30, default=entry_params['base_nb_candles_entry'], space='entry', optimize=smaoffset_optimize)
    base_nb_candles_exit = IntParameter(5, 30, default=exit_params['base_nb_candles_exit'], space='exit', optimize=smaoffset_optimize)
    low_offset = DecimalParameter(1.0, 1.1, default=entry_params['low_offset'], space='entry', decimals=3, optimize=smaoffset_optimize)
    low_offset_2 = DecimalParameter(0.94, 0.98, default=entry_params['low_offset_2'], space='entry', decimals=3, optimize=smaoffset_optimize)
    high_offset = DecimalParameter(1.0, 1.1, default=exit_params['high_offset'], space='exit', decimals=3, optimize=smaoffset_optimize)
    high_offset_2 = DecimalParameter(1.2, 1.5, default=exit_params['high_offset_2'], space='exit', decimals=3, optimize=smaoffset_optimize)
    # Protection
    fast_ewo = 50
    slow_ewo = 200
    protection_optimize = True
    ewo_low = DecimalParameter(-12.0, -8.0, default=entry_params['ewo_low'], space='entry', decimals=2, optimize=protection_optimize)
    ewo_high = DecimalParameter(1.0, 2.2, default=entry_params['ewo_high'], space='entry', decimals=3, optimize=protection_optimize)
    ewo_high_2 = DecimalParameter(-4.0, -2.0, default=entry_params['ewo_high_2'], space='entry', decimals=2, optimize=protection_optimize)
    rsi_entry = IntParameter(55, 85, default=entry_params['rsi_entry'], space='entry', optimize=protection_optimize)
    min_profit = DecimalParameter(0.7, 1.2, default=entry_params['min_profit'], space='entry', decimals=2, optimize=protection_optimize)
    ...
    plot_config = {'main_plot': {'bb_upperband28': {'color': '#bc281d', 'type': 'line'}, 'bb_midband28': {'color': 'orange', 'type': 'line'}, 'bb_lowerband28': {'color': '#792bbb', 'type': 'line'}}, 'subplots': {'RSI': {'rsi': {'color': 'yellow'}}}}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dont_entry_conditions.append(dataframe['close_1h'].rolling(24).max() < dataframe['close'] * self.min_profit.value)
