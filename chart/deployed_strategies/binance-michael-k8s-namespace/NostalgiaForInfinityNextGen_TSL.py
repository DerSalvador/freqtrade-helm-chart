...
class NostalgiaForInfinityNextGen_TSL(IStrategy):
    ...
    def exit_signals(self, current_profit: float, max_profit: float, max_loss: float, last_candle, previous_candle_1, previous_candle_2, previous_candle_3, previous_candle_4, previous_candle_5, trade: 'Trade', current_time: 'datetime', entry_tag) -> tuple:
        # Sell signal 1
    ...
    def exit_stoploss(self, current_profit: float, max_profit: float, max_loss: float, last_candle, previous_candle_1, trade: 'Trade', current_time: 'datetime') -> tuple:
        ...
        if -0.04 > current_profit > -0.07 and last_candle['ema_vwma_osc_32'] < 0.0 and (last_candle['ema_vwma_osc_64'] < 0.0) and (max_profit < 0.005) and (max_loss < 0.07) and last_candle['sma_200_dec_24'] and (last_candle['close'] < last_candle['ema_200']) and (last_candle['ema_25'] < last_candle['ema_50']) and (last_candle['cmf'] < -0.0) and (last_candle['r_480'] < -50.0) and (last_candle['btc_not_downtrend_1h'] == False):
            return (True, 'exit_stoploss_u_e_b_3')
        # Under EMA200, pair & BTC negative, low max rate, short trade duration
        if -0.04 > current_profit > -0.09 and max_profit < 0.005 and (max_loss < 0.09) and (last_candle['close'] < last_candle['ema_200']) and (last_candle['ema_25'] < last_candle['ema_50']) and (last_candle['ema_vwma_osc_32'] < 0.0) and (last_candle['ema_vwma_osc_64'] < 0.0) and (last_candle['ema_vwma_osc_96'] < 0.0) and (last_candle['cmf'] < -0.1) and (last_candle['r_480'] < -75.0) and (last_candle['close'] < last_candle['sup_level_1h']) and (current_time - timedelta(minutes=60) < trade.open_date_utc) and (last_candle['btc_not_downtrend_1h'] == False):
            return (True, 'exit_stoploss_u_e_b_4')
        # Under EMA200, pair & BTC negative, low max rate, short trade duration
        if -0.04 > current_profit > -0.09 and max_profit < 0.015 and (max_loss < 0.09) and (last_candle['close'] < last_candle['ema_200']) and (last_candle['ema_25'] < last_candle['ema_50']) and (last_candle['ema_vwma_osc_32'] < 0.0) and (last_candle['ema_vwma_osc_64'] < 0.0) and (last_candle['ema_vwma_osc_96'] < 0.0) and (last_candle['cmf'] < -0.2) and (last_candle['close'] < last_candle['sup_level_1h']) and (current_time - timedelta(minutes=180) < trade.open_date_utc) and (last_candle['btc_not_downtrend_1h'] == False):
            return (True, 'exit_stoploss_u_e_b_5')
        # Under EMA200, pair & BTC negative, low max rate, short trade duration
        if -0.04 > current_profit > -0.1 and max_profit < 0.015 and (last_candle['close'] < last_candle['ema_200']) and (last_candle['ema_25'] < last_candle['ema_50']) and (last_candle['ema_vwma_osc_32'] < 0.0) and (last_candle['ema_vwma_osc_64'] < 0.0) and (last_candle['ema_vwma_osc_96'] < 0.0) and (last_candle['cmf'] < -0.25) and (last_candle['r_480'] < -75.0) and (last_candle['close'] < last_candle['sup_level_1h']) and (current_time - timedelta(minutes=60) < trade.open_date_utc) and (last_candle['btc_not_downtrend_1h'] == False):
            return (True, 'exit_stoploss_u_e_b_6')
        return (False, None)
    ...
    def exit_r(self, current_profit: float, last_candle) -> tuple:
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        exit, signal_name = self.exit_signals(current_profit, max_profit, max_loss, last_candle, previous_candle_1, previous_candle_2, previous_candle_3, previous_candle_4, previous_candle_5, trade, current_time, entry_tag)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Stoplosses
        exit, signal_name = self.exit_stoploss(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        ...
        exit, signal_name = self.exit_r(current_profit, last_candle)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Trailing
        exit, signal_name = self.exit_trail(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        ...
        exit, signal_name = self.exit_pump_dec(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # The pair is pumped, trailing
        exit, signal_name = self.exit_pump_trail(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # The pair is pumped, stoploss
        exit, signal_name = self.exit_pump_stoploss(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Trade duration based exit logic
        exit, signal_name = self.exit_duration(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        return None
        ...
        for index in self.entry_protection_params:
            ...
            if self.entry_params[f'entry_condition_{index}_enable']:
                ...
                if index == 1:
                    ...
                    # Logic
                    item_entry_logic.append((dataframe['close'] - dataframe['open'].rolling(4).min()) / dataframe['open'].rolling(4).min() > 0.038)
                ...
                # Condition #3 - Semi swing. Local dip. Strong uptrend.
                    ...
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_25'] * 0.952)
                ...
                # Condition #4 - Semi swing. Local dip. Uptrend. Strict pump & dip protections.
                    ...
                    item_entry_logic.append(dataframe['closedelta'].gt(dataframe['close'] * 0.03))
                ...
                # Condition #10 - Semi swing. Local dip.
                    ...
                    item_entry_logic.append(dataframe['rsi_14'] < 25.5)
                    item_entry_logic.append(dataframe['crsi'] > 8.0)
                    item_entry_logic.append(dataframe['r_480_1h'] > -75.0)
                # Condition #11 - Semi swing. Deep local dip. Mild uptrend.
                elif index == 11:
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_20'] * 0.926)
                    item_entry_logic.append(dataframe['ewo_ema'] > 2.0)
                    item_entry_logic.append(dataframe['cti'] < -0.9)
                ...
                # Condition #14 - Semi swing. Mild uptrend. 1h uptrend. Local dip.
                    ...
                    # Logic
                    item_entry_logic.append(dataframe['ewo_ema'] > 2.0)
                    item_entry_logic.append(dataframe['close'].shift(1) < dataframe['ema_20'].shift(1) * 0.992)
                    item_entry_logic.append(dataframe['close'] > dataframe['close'].shift(1))
                    item_entry_logic.append(dataframe['cmf'].shift(1) < -0.48)
                    item_entry_logic.append(dataframe['r_14'].shift(1) < -97.0)
                    item_entry_logic.append(dataframe['ewo_ema_1h'] > 7.5)
                    item_entry_logic.append(dataframe['cti_1h'] < 0.0)
                # Condition #15 - Semi swing. Downtrend.Local dip.
                elif index == 15:
                    # Non-Standard protections (add below)
                    # Logic
                    item_entry_logic.append(dataframe['close'].shift(1) < dataframe['ema_20'].shift(1) * 0.996)
                ...
                # Condition #16 - Semi swing. Strong uptrend. 1h uptrend. Local dip. BTC not downtrend.
                    ...
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['ewo_ema'].shift(1) > 6.6)
                    item_entry_logic.append(dataframe['close'].shift(1) < dataframe['sma_30'].shift(1) * 0.988)
                    item_entry_logic.append(dataframe['close'].shift(1) < dataframe['bb20_2_low'].shift(1) * 0.982)
                    item_entry_logic.append(dataframe['rsi_14'].shift(1) < 32.0)
                    item_entry_logic.append(dataframe['r_14'].shift(1) < -94.0)
                    item_entry_logic.append(dataframe['ewo_ema_1h'] > 2.0)
                ...
                dataframe.loc[item_entry, 'entry_tag'] += f'{index} '
