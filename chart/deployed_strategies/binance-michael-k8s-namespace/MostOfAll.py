...
def MOST(dataframe, length=8, percent=2, MAtype=1):
    ...
    data['most'] = 0.0
    for i in range(length, len(data)):
        data['most'].iat[i] = data['final_ub'].iat[i] if data['most'].iat[i - 1] == data['final_ub'].iat[i - 1] and data['exma'].iat[i] <= data['final_ub'].iat[i] else data['final_lb'].iat[i] if data['most'].iat[i - 1] == data['final_ub'].iat[i - 1] and data['exma'].iat[i] > data['final_ub'].iat[i] else data['final_lb'].iat[i] if data['most'].iat[i - 1] == data['final_lb'].iat[i - 1] and data['exma'].iat[i] >= data['final_lb'].iat[i] else data['final_ub'].iat[i] if data['most'].iat[i - 1] == data['final_lb'].iat[i - 1] and data['exma'].iat[i] < data['final_lb'].iat[i] else 0.0
    # Mark the trend direction up/down
    data['trend'] = np.where(data['most'] > 0.0, np.where(data['exma'] < data['most'], 0, 1), np.NaN)
...
class MostOfAll(IStrategy):
    ...
    def plot_config(self):
        ...
        return {'main_plot': {'most': {'color': 'darkpurple'}, 'exma': {'color': 'green'}}, 'subplots': {'trend': {'trend': {'color': 'blue'}}}}
    # hard stoploss profit
    pHSL = DecimalParameter(-0.5, -0.04, default=-0.99, decimals=3, space='exit', load=True)
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(0.008, 0.02, default=0.011, decimals=3, space='exit', load=True)
    pSL_1 = DecimalParameter(0.008, 0.02, default=0.009, decimals=3, space='exit', load=True)
    # profit threshold 2, SL_2 is used
    pPF_2 = DecimalParameter(0.04, 0.1, default=0.04, decimals=3, space='exit', load=True)
    pSL_2 = DecimalParameter(0.02, 0.07, default=0.02, decimals=3, space='exit', load=True)
    # Custom stoploss
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        If the bullish fractal is active and below the teeth of the gator -> entry
        :param dataframe: DataFrame
        :return: DataFrame with entry column
        """
        conditions = []
        conditions.append(qtpylib.crossed_above(dataframe['most'], dataframe['exma']) & (dataframe['volume'] > 0))
        if conditions:
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        If the bearish fractal is active and above the teeth of the gator -> exit
        :param dataframe: DataFrame
        :return: DataFrame with entry column
        """
        conditions = []
        conditions.append(qtpylib.crossed_above(dataframe['exma'], dataframe['most']) & (dataframe['volume'] > 0))
        if conditions:
