...
class pcb20(IStrategy):
    ...
    timeframe = '6h'
    # hyperopt params
    exit_rsil = DecimalParameter(45, 90, default=70)
    exit_rsiu = DecimalParameter(65, 100, default=80)
    exit_wavetrend = DecimalParameter(0, 5, default=0)
    entry_wavetrend = DecimalParameter(-10, -1, default=0)
    entry_rsil = DecimalParameter(0, 40, default=40)
    entry_rsiu = DecimalParameter(40, 65, default=50)

    @property
    def plot_config(self):
        return {'main_plot': {}, 'subplots': {'obv': {'OBV': {'color': '#1b61ab', 'type': 'line'}, 'OBVSlope': {'color': '#f18b7a', 'type': 'line'}}, 'wavetrend': {'wave_t1': {'color': '#1b61ab', 'type': 'line'}, 'wave_t2': {'color': '#f18b7a', 'type': 'line'}}}}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['wave_t1'] >= dataframe['wave_t2']) & (dataframe['rsi_ma'] <= self.entry_rsiu.value) & (dataframe['rsi_ma'] >= self.entry_rsil.value) & (dataframe['rsi_ma_slope'] > 0), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # (dataframe['wave_t1'] <= dataframe['wave_t2'])
        dataframe.loc[(dataframe['wave_t1_pc'] <= self.exit_wavetrend.value) & (dataframe['rsi_ma'] >= self.exit_rsil.value) & (dataframe['rsi_ma'] <= self.exit_rsiu.value) & (dataframe['rsi_ma_slope'] <= 0), 'exit_long'] = 1
        return dataframe
