...
class e6v34(IStrategy):
    INTERFACE_VERSION = 3
    bwill = RealParameter(-25, -15, default=-20, space='entry')
    swill = RealParameter(-55, -35, default=-50, space='exit')
    # bwill = -20 #entry williams r
    # swill = -50 #exit williams r
    # ROI table:
    minimal_roi = {'0': 0.555, '300': 0.35, '500': 0.247, '1200': 0.0936, '2100': 0.04, '4000': 0}
    # Stoploss:
    stoploss = -0.54
    # Trailing stop:
    trailing_stop = True
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['willr'] < self.swill.value) & (dataframe['willr'].shift(1) > self.swill.value), 'exit'] = 1
        return dataframe