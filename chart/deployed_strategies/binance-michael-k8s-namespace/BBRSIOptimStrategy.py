...
class BBRSIOptimStrategy(IStrategy):
    ...
    minimal_roi = {'0': 0.323, '107': 0.097, '150': 0.019, '238': 0}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['rsi'] > 12) & (dataframe['close'] < dataframe['bb_lowerband_2sd']), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['rsi'] > 96) & (dataframe['close'] > dataframe['bb_lowerband_1sd']), 'exit'] = 1
        return dataframe