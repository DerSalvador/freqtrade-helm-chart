...
class NNTC(IStrategy):
    INTERFACE_VERSION = 3
    plot_config = {'main_plot': {'close': {'color': 'cornflowerblue'}}, 'subplots': {'Diff': {'%train_buy': {'color': 'mediumaquamarine'}, 'predict_buy': {'color': 'cornflowerblue'}, '%train_sell': {'color': 'lightsalmon'}, 'predict_sell': {'color': 'brown'}}}}
    # Do *not* hyperopt for the roi and stoploss spaces (unless you turn off custom stoploss)
    # ROI table:
    minimal_roi = {'0': 0.15, '20160': 0.09, '40160': 0}
    # Stoploss:
    stoploss = -0.99
    # Trailing stop:
    trailing_stop = False
    ...
    timeframe = '5m'
    exit_on_current_profit = 0.4  # 30% profit
    inf_timeframe = '5m'
    use_custom_stoploss = False
    # Recommended
    use_entry_signal = True
    entry_profit_only = True
    ...
    def save_debug_data(self, future_df: DataFrame):
        ...
        dbg_list = ['full_dwt', 'train_buy', 'train_sell', 'future_gain', 'future_min', 'future_max', 'future_profit_min', 'future_profit_max', 'future_loss_min', 'future_loss_max', 'loss_threshold']
        if len(dbg_list) > 0:
    ...
    def train_models(self, curr_pair, dataframe: DataFrame, buys, sells):
        ...
        if self.trinary_classifier is None or self.model_per_pair:
            self.trinary_classifier, name = NNTClassifier.create_classifier(self.classifier_type, self.curr_pair, num_features, self.seq_len)
        ...
        print(f'    holds:{num_holds:.0f} ({hpct:.2f}%) ' + f'buys:{num_buys:.0f} ({bpct:.2f}%) sells:{num_sells:.0f} ({spct:.2f}%)')
        # quick check
        ...
        if self.dbg_verbose:
            ...
            print('    training samples: ', train_size, ' #buys:', num_buys, ' ({:.2f}'.format(buy_pct), '%)', ' #sells:', num_sells, ' ({:.2f}'.format(100.0 * (num_sells / train_size)), '%)')
        ...
        if self.dbg_test_classifier:
            if not clf is None:
                ...
                print(f'    Testing Classifier: {clf_name}, signals:{self.training_signals.get_signal_name()}, ', f'pair: {curr_pair}')
    ...
    def get_trinary_classifier(self, tensor, results, test_tensor, test_labels):
        ...
        if self.dp.runmode.value in 'backtest':
            ...
            if self.trinary_classifier:
                clf = self.fit_classifier(self.trinary_classifier, name, '', tensor, labels, test_tensor, test_labels)
            else:
                num_features = np.shape(tensor)[2]
                clf, name = NNTClassifier.create_classifier(self.classifier_type, self.curr_pair, num_features, self.seq_len)
                # set the model name
                # category, model_name = self.get_model_identifiers(self.curr_pair, name)
    ...
    classifier_list = [NNTClassifier.ClassifierType.MLP, NNTClassifier.ClassifierType.Ensemble, NNTClassifier.ClassifierType.LSTM, NNTClassifier.ClassifierType.Multihead, NNTClassifier.ClassifierType.Transformer]
    ...
    def get_model_identifiers(self, pair, clf_name, tag=''):
        ...
        model_name = 'NNTC_' + ds + self.training_signals.get_signal_name() + '_' + clf_name
        category = 'NNTC_' + self.training_signals.get_signal_name() + '_' + clf_name
    ...
    def find_best_classifier(self, tensor, results, tag=''):
        ...
        for clf_id in self.classifier_list:
            clf, name = NNTClassifier.create_classifier(self.classifier_type, self.curr_pair, num_features, self.seq_len, tag=tag)
            # set the model name
            # category, model_name = self.get_model_identifiers(self.curr_pair, name)
            # clf.set_model_name(category, model_name)
            self.trinary_classifier.set_model_path(self.get_model_path(self.curr_pair, name))
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # )
        predict_cond = dataframe['predict_buy'] > 0.5
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(dataframe['volume'] > 0)
        # MFI
        conditions.append(dataframe['mfi'] > 90.0)
        # Fisher/Williams in sell region
        conditions.append(dataframe['fisher_wr'] >= 0.9)
        # model triggers
        # predict_cond = (
        #     qtpylib.crossed_above(dataframe['predict_sell'], 0.5)
        # )
        predict_cond = dataframe['predict_sell'] > 0.9
        conditions.append(predict_cond)
        # add strategy-specific conditions (from subclass)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if current_profit > self.exit_on_current_profit:
            if last_candle['fisher_wr'] > 0.95:
                return 'take_profit'
        # Mod: strong sell signal, in profit
        if current_profit > self.exit_on_current_profit and last_candle['fisher_wr'] > 0.95:
            return 'fwr_high'
        ...
        if current_profit > self.exit_on_current_profit and last_candle['exit_long'] > 0:
            return 'exit_signal'
        return None
#######################