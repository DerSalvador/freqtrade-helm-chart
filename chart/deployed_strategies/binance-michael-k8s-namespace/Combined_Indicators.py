...
class Combined_Indicators(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # strategy BinHV45
        # strategy ClucMay72018
        dataframe.loc[dataframe['lower'].shift().gt(0) & dataframe['bbdelta'].gt(dataframe['close'] * 0.00703) & dataframe['closedelta'].gt(dataframe['close'] * 0.01476) & dataframe['tail'].lt(dataframe['bbdelta'] * 0.03632) & dataframe['close'].lt(dataframe['lower'].shift()) & dataframe['close'].le(dataframe['close'].shift()) | (dataframe['close'] < dataframe['ema_slow']) & (dataframe['close'] < 0.98863 * dataframe['bb_lowerband']) & (dataframe['volume'] < dataframe['volume_mean_slow'].shift(1) * 34), 'entry'] = 1
        return dataframe
