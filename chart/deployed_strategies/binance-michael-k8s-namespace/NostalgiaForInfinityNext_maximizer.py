...
class NostalgiaForInfinityNext_maximizer(IStrategy):
    ...
    entry_ewo_22 = 5.8
    entry_rsi_22 = 36.0
    entry_bb_offset_23 = 0.985
    entry_ewo_23 = 6.2
    entry_rsi_23 = 32.4
    entry_rsi_1h_23 = 70.0
    entry_24_rsi_max = 50.0
    entry_24_rsi_1h_min = 66.9
    entry_25_ma_offset = 0.922
    entry_25_rsi_4 = 38.0
    entry_25_cti = -0.76
    entry_26_zema_low_offset = 0.9
    entry_26_cti = -0.9
    entry_26_r = -80.0
    entry_26_r_1h = -80.0
    entry_26_volume = 2.0
    ...
    exit_custom_rsi_under_bull_5 = 45.0
    exit_custom_profit_bull_6 = 0.07
    exit_custom_rsi_under_bull_6 = 48.0
    ...
    exit_custom_profit_under_profit_max_1 = 0.01
    ...
    def exit_under_min(self, current_profit: float, last_candle) -> tuple:
        ...
        #        else:
        # Uptrend
        if current_profit >= self.exit_custom_profit_under_profit_2 and last_candle['close'] < last_candle['ema_200'] and ((last_candle['ema_200'] - last_candle['close']) / last_candle['close'] < self.exit_custom_profit_under_rel_2) and (last_candle['rsi_14'] > last_candle['rsi_14_1h'] + self.exit_custom_profit_under_rsi_diff_2):
    ...
    def mark_profit_target(self, pair: str, trade: 'Trade', current_rate: float, current_profit: float, max_profit: float, last_candle, previous_candle_1) -> tuple:
        if self.profit_target_1_enable:
            if last_candle['moderi_96'] == False and self.exit_custom_profit_under_profit_max_1 > current_profit >= self.exit_custom_profit_under_profit_min_1 and (last_candle['close'] < last_candle['ema_200']) and ((last_candle['ema_200'] - last_candle['close']) / last_candle['close'] < self.exit_custom_profit_under_rel_1) and (last_candle['rsi_14'] > last_candle['rsi_14_1h'] + self.exit_custom_profit_under_rsi_diff_1):
                self._set_profit_target(pair, current_rate, 'mark_profit_target_01')

    def exit_profit_target(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, max_profit: float, last_candle, previous_candle_1) -> tuple:
        ...
        if self.profit_target_1_enable:
            ...
            if last_candle['zlema_1_exit'] < last_candle['zlema_4_exit'] and previous_candle_1['zlema_1_exit'] > previous_candle_1['zlema_4_exit'] and (last_candle['hrsi_exit'] > 90):
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if all((c in ['32', '33', '34', '35', '36', '37', '38', '39', '40'] for c in entry_tags)):
            ...
            if exit and signal_name is not None:
                return signal_name + ' ( ' + entry_tag + ')'
        # Ichi Trade management
        if all((c in ['39'] for c in entry_tags)):
            exit, signal_name = self.exit_ichi(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
            if exit and signal_name is not None:
                return signal_name + ' ( ' + entry_tag + ')'
        ...
        if exit and signal_name is not None:
            return signal_name + ' ( ' + entry_tag + ')'
        # Stoplosses
        exit, signal_name = self.exit_stoploss(current_profit, last_candle, previous_candle_1)
        if exit and signal_name is not None:
            return signal_name + ' ( ' + entry_tag + ')'
        ...
        if exit and signal_name is not None:
            return signal_name + ' ( ' + entry_tag + ')'
        # Profit Target Signal
        exit, signal_name = self.exit_profit_target(pair, trade, current_time, current_rate, current_profit, max_profit, last_candle, previous_candle_1)
        if exit and signal_name is not None:
            return signal_name + ' ( ' + entry_tag + ')'
        ...
        if self.exit_condition_1_enable and last_candle['rsi_14'] > self.exit_rsi_bb_1 and (last_candle['close'] > last_candle['bb20_2_upp']) and (previous_candle_1['close'] > previous_candle_1['bb20_2_upp']) and (previous_candle_2['close'] > previous_candle_2['bb20_2_upp']) and (previous_candle_3['close'] > previous_candle_3['bb20_2_upp']) and (previous_candle_4['close'] > previous_candle_4['bb20_2_upp']) and (previous_candle_5['close'] > previous_candle_5['bb20_2_upp']):
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.0:
                    return 'exit_signal_1_1_1' + ' ( ' + entry_tag + ')'
            elif current_profit > 0.0:
                return 'exit_signal_1_2_1' + ' ( ' + entry_tag + ')'
            elif max_loss > 0.25:
                return 'exit_signal_1_2_2' + ' ( ' + entry_tag + ')'
        # Sell signal 2
            ...
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.0:
                    return 'exit_signal_2_1_1' + ' ( ' + entry_tag + ')'
            elif current_profit > 0.0:
                return 'exit_signal_2_2_1' + ' ( ' + entry_tag + ')'
            elif max_loss > 0.25:
                return 'exit_signal_2_2_2' + ' ( ' + entry_tag + ')'
        # Sell signal 4
            ...
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.0:
                    return 'exit_signal_4_1_1' + ' ( ' + entry_tag + ')'
            elif current_profit > 0.0:
                return 'exit_signal_4_2_1' + ' ( ' + entry_tag + ')'
            elif max_loss > 0.25:
                return 'exit_signal_4_2_2' + ' ( ' + entry_tag + ')'
        # Sell signal 6
            ...
            if current_profit > 0.0:
                return 'exit_signal_6_1' + ' ( ' + entry_tag + ')'
            elif max_loss > 0.25:
                return 'exit_signal_6_2' + ' ( ' + entry_tag + ')'
        # Sell signal 7
        elif self.exit_condition_7_enable and last_candle['rsi_14_1h'] > self.exit_rsi_1h_7 and last_candle['crossed_below_ema_12_26']:
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.0:
                    return 'exit_signal_7_1_1' + ' ( ' + entry_tag + ')'
            elif current_profit > 0.0:
                return 'exit_signal_7_2_1' + ' ( ' + entry_tag + ')'
            elif max_loss > 0.25:
                return 'exit_signal_7_2_2' + ' ( ' + entry_tag + ')'
        # Sell signal 8
        elif self.exit_condition_8_enable and last_candle['close'] > last_candle['bb20_2_upp_1h'] * self.exit_bb_relative_8:
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.0:
                    return 'exit_signal_8_1_1' + ' ( ' + entry_tag + ')'
            elif current_profit > 0.0:
                return 'exit_signal_8_2_1' + ' ( ' + entry_tag + ')'
            elif max_loss > 0.25:
                return 'exit_signal_8_2_2' + ' ( ' + entry_tag + ')'
        return None
        ...
        for index in self.entry_protection_params:
            ...
            if self.entry_params['entry_condition_' + str(index) + '_enable']:
                ...
                # Condition #23
                    ...
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['bb20_2_low'] * self.entry_bb_offset_23)
                    item_entry_logic.append(dataframe['ewo'] > self.entry_ewo_23)
                    item_entry_logic.append(dataframe['rsi_14'] < self.entry_rsi_23)
                    item_entry_logic.append(dataframe['rsi_14_1h'] < self.entry_rsi_1h_23)
                # Condition #24
                elif index == 24:
                    # Non-Standard protections
                    # Logic
                ...
                dataframe.loc[item_entry, 'entry_tag'] += str(index) + ' '
    ...
    def confirm_trade_exit(self, pair: str, trade: 'Trade', order_type: str, amount: float, rate: float, time_in_force: str, exit_reason: str, **kwargs) -> bool:
        ...
        if self._should_hold_trade(trade, rate, exit_reason):
            return False
        # if self._should_catch_profit_target(pair, trade, rate, exit_reason):
        #     self._set_profit_target(pair, trade, rate, exit_reason)
        #     return False
        self._remove_profit_target(pair)
        return True
    # def _should_catch_profit_target(self, pair: str, trade: "Trade", rate: float, exit_reason: str) -> bool:
    #     if exit_reason == "force_exit":
    #         return False
    #     # ADD LOGIC HERE
    #     if exit_reason in ['profit_target_01']:
    #         return True
    #     return False

    def _set_profit_target(self, pair: str, rate: float, exit_reason: str):
        self.custom_info[pair] = {'rate': rate, 'exit_reason': exit_reason}
...
DATA_NFI_TARGET_PROFIT_BY_PAIR_PATH = os.path.join(USER_DATA_DIR, 'data-nfi-profit_target_by_pair.json')
