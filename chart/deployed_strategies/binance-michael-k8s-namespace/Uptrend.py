...
class Uptrend(IStrategy):
    INTERFACE_VERSION = 3
    entry_params = {'entry_rsi_uplimit': 50}
    entry_rsi_uplimit = IntParameter(50, 90, default=entry_params['entry_rsi_uplimit'], optimize=False, space='entry')
    # Minimal ROI designed for the strategy.
    # This attribute will be overridden if the config file contains "minimal_roi".
    minimal_roi = {'60': 0.1, '30': 0.02, '0': 0.04}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # uptrend
        # Make sure Volume is not 0
        dataframe.loc[(dataframe['rsi'] < 80) & (dataframe['mama'] > dataframe['fama']) & (dataframe['mama_diff_ratio'] > 0.04) & (dataframe['volume'] > 0), 'entry'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # Make sure Volume is not 0
        dataframe.loc[(dataframe['mama_diff_ratio'] < 0.01) & (dataframe['volume'] > 0), 'exit'] = 1
        return dataframe
import random
...
class SuperBuy(Uptrend):
    """
	Idea is to build random entry signales from populate_indicators, with luck we'll get a good entry signal
	"""
    generator = IntParameter(0, 100000000000, default=99295874569, optimize=True, space='entry')  # generate unique matrix of conditions for your dataframe
    operators_used_to_compare_between_columns = IntParameter(0, 3, default=3, optimize=True, space='entry')  # number of conditions you will keep to build entry signal
    operators_used_to_with_best_point = IntParameter(0, 3, default=1, optimize=True, space='entry')  # number of conditions you will keep to build entry signal
    condition_selector = IntParameter(0, 100, default=50, optimize=True, space='entry')  # how to select the desired conditions beteween all conditions generated (seed random)
    best_entry_point = None
    best_entry_point_dict = dict()
    bad_entry_point_dict = dict()
    all_points_dict = dict()
    entry_signal_already_printed = False
    columns = []
    ...
    top_index_criteria = {'min_close_hh_ratio': 0.08, 'max_candles_to_get_ratio': 8, 'candles_after_dip_to_entry': 0, 'select_parameter_if_in_more_than_x_percent_of_best_points': 97, 'select_prameter_if_prop_is_x_percent_higher_in_best_points': 10}
    ...
    def find_best_entry_point(self, dataframe: DataFrame, metadata: dict):
        ...
        filtered_df['shifted_index'] = filtered_df.index - lookahead_candles + self.top_index_criteria['candles_after_dip_to_entry']
    ...
    def common_points_for_every_best_entry(self, dataframe: DataFrame, metadata: dict, columns: list) -> list:
        ...
        if current_pair not in self.best_entry_point_dict:
            self.best_entry_point_dict[current_pair], self.bad_entry_point_dict[current_pair] = self.find_best_entry_point(dataframe, metadata)
        ...
        for pair in full_pairlist:
            ...
            if pair not in self.best_entry_point_dict and (not current_df.empty):
        ...
        for pair in full_pairlist:
            ...
            if not pair in self.best_entry_point_dict:
                continue
            if all_best_points is None:
                all_best_points = self.best_entry_point_dict[pair]
                all_bad_points = self.bad_entry_point_dict[pair]
                all_points = self.all_points_dict[pair]
            else:
                all_best_points = all_best_points.append(self.best_entry_point_dict[pair])
                all_bad_points = all_bad_points.append(self.bad_entry_point_dict[pair])
        ...
        for column in columns:
            ...
            count = count[df_mask]
            if not count.empty and (not column in ['entry', 'entry_tag']):
                ...
                print(column)
                print(df_all)  # the part of best points should be at least 3% for the value
                # proportion of value is X* more important in best points than in bad points
                # minimum number of the value (because we don't want close=1.121213243482902183 as result)
                values = df_all.query(f'part_of_best_points > 3 & part_of_best_points_percent > 13 & best_points > 12').index.tolist()  # & df_all["part_of_best_points_percent"] > 10)]
                print(values)
                for elt in values:
                    best_indicators.append(f"dataframe['{column}'] == {elt}")
            if column in ['entry', 'entry_tag']:
                print(column)
                print(all_best_points[column].value_counts())
            elif not count.empty:
                res.append({'column': column, 'value': count.index[0], 'ratio_for_best': count.iloc[0] / all_best_points.shape[0], 'ratio_for_all': all_points_values_count[count.index[0]] / all_points.shape[0], 'ratio_diff': count.iloc[0] / all_best_points.shape[0] - all_points_values_count[count.index[0]] / all_points.shape[0]})
        ...
        print('Suggested entry signal:')
        print('( # main entry signals found')
    ...
    def generate_superentry_signal(self, dataframe: DataFrame, metadata: dict) -> list:
        # every indicators names
        columns = list(dataframe.columns)
        columns.remove('date')
        columns.remove('exit')
        columns.remove('entry')
        columns.remove('entry_tag')
        ...
        if len(self.columns_to_compare_to_best_point) == 0 and len(self.columns_to_compare_to_volume) == 0 and (len(self.columns_to_compare_to_price) == 0):
            ...
            if self.best_entry_point is not None:
                for column in self.columns_to_compare_to_best_point:
                    if str(self.best_entry_point[column]) == 'nan':
        ...
        # generate matrix of all operators for all combinations of columns and create entry conditions
        index = 0
        for left_elt in self.columns_to_compare_to_price:
            for right_elt in self.columns_to_compare_to_price:
                ...
                entry_conds.append("(dataframe['" + left_elt + "'] " + self.operators[int(generator)] + " dataframe['" + right_elt + "'])")
        for left_elt in self.columns_to_compare_to_volume:
            for right_elt in self.columns_to_compare_to_volume:
                ...
                # print("(dataframe['" + left_elt + "'] " + self.operators[int(generator)] + " dataframe['" + right_elt + "'])")
                entry_conds.append("(dataframe['" + left_elt + "'] " + self.operators[int(generator)] + " dataframe['" + right_elt + "'])")
        entry_conds_best_point = []
        # generate entry conditions with best entry point
        for column in self.columns_to_compare_to_best_point:
            if self.best_entry_point is None:
                continue
            if index > len(generators):
            ...
            if int(generator) not in self.operators:
                # pass if no operator is selected
                continue
            # print("(dataframe['" + column + "'] " + self.operators[int(generator)] + " best_entry_point['" + column + "']))")
            # print(eval("best_entry_point['" + column + "']"))
            entry_conds_best_point.append("(dataframe['" + column + "'] " + self.operators[int(generator)] + ' ' + str(self.best_entry_point[column]) + ')')
        # select a few entry conditions
        random.seed(self.condition_selector.value)
        try:
            entry_conds = random.sample(entry_conds, self.operators_used_to_compare_between_columns.value)
        except ValueError:
            print('not enough conditions to compare between columns')
            # Sample larger than population or is negative
            pass
        try:
            entry_conds += random.sample(entry_conds_best_point, self.operators_used_to_with_best_point.value)
        except ValueError as e:
            if self.config['runmode'].value != 'hyperopt':
                print('not enough conditions to compare with best point')
            # Sample larger than population or is negative
            pass
        if self.config['runmode'].value in ('backtest', 'hyperopt') and self.entry_signal_already_printed != entry_conds:
            print(entry_conds)
            self.entry_signal_already_printed = entry_conds
        try:
            entry_conds = [eval(entry_cond, globals(), {'dataframe': dataframe, 'best_entry_point': self.best_entry_point}) for entry_cond in entry_conds]
        except:
            return []
        return entry_conds

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        entry_conds = self.generate_superentry_signal(dataframe, metadata)
        if self.config['runmode'].value in 'backtest':  # backtest, we want to check must common entry tags...
            dataframe = super().populate_entry_trend(dataframe, metadata)  # get entry tags
            self.common_points_for_every_best_entry(dataframe, metadata, self.columns + ['entry', 'entry_tag'])
        elif self.config['runmode'].value in 'hyperopt':  # hyperopt, we want to test new entry signals
            # main entry signals found
            # protections
            # (dataframe['ema_50_lin'] < dataframe['ema_26_lin']) |
            # (dataframe['sma_15'] > dataframe['ema_50_1h']) |
            # (dataframe['ema_slow'] <= dataframe['sup1']) |
            is_additional_check = (dataframe['not_res1_1h'] == True) & ((dataframe['rsi_fast_lower_20'] == 0) & (dataframe['rsi_fast_lower_30'] == 0) & (dataframe['r_14_lower_minus_80'] == 0) & (dataframe['r_32_lower_minus_80'] == 0) & (dataframe['r_96_lower_minus_80'] == 0)) & (dataframe['res1'] >= dataframe['bb_upperband2_1h'])
            if entry_conds:
                dataframe.loc[is_additional_check & reduce(lambda x, y: x & y, entry_conds), 'entry'] = 1
