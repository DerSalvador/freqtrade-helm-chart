...
class Stinkfist(IStrategy):
    ...
    timeframe = '5m'
    inf_timeframe = '1h'
    entry_params = {'inf-pct-adr': 0.8, 'mp': 30}
    exit_params = {}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if trade_data['active_trade']:
            ...
            conditions.append((dataframe['close'] <= dataframe[f'3d_low_{self.inf_timeframe}'] + params['inf-pct-adr'] * dataframe[f'adr_{self.inf_timeframe}']) & (dataframe['mp'] < params['mp']) & (dataframe['streak-roc'] > dataframe['pcc-lowerband']) & (dataframe['mac'] == 1))
...
class Stinkfist_BTC(Stinkfist):
    timeframe = '5m'
    inf_timeframe = '1h'
    entry_params = {'inf-pct-adr': 0.91556, 'mp': 66}
...
class Stinkfist_ETH(Stinkfist):
    timeframe = '5m'
    inf_timeframe = '1h'
    entry_params = {'inf-pct-adr': 0.81628, 'mp': 40}
    trailing_stop = True
