...
class wavetrend_rsi(IStrategy):
    ...
    minimal_roi = {'12000': 0.1, '600': 0.15, '300': 0.2, '180': 0.3, '120': 0.4, '60': 0.45, '0': 0.5}
    ### Hyperoptable parameters ###
    # entry optizimation
    max_epa = CategoricalParameter([-1, 0, 1, 3, 5, 10], default=3, space='entry', optimize=True)
    dcal2 = DecimalParameter(-5, -1, default=-2.5, space='entry', optimize=True)
    dcal3 = DecimalParameter(-8, -3, default=-5, space='entry', optimize=True)
    dcal4 = DecimalParameter(-15, -5, default=-10, space='entry', optimize=True)
    # protections
    cooldown_lookback = IntParameter(2, 48, default=5, space='protection', optimize=True)
    stop_duration = IntParameter(12, 200, default=5, space='protection', optimize=True)
    use_stop_protection = BooleanParameter(default=True, space='protection', optimize=True)
    # trading
    entry_rsi = IntParameter(low=15, high=30, default=25, space='entry', optimize=True, load=True)
    exit_rsi = IntParameter(low=50, high=70, default=55, space='exit', optimize=True, load=True)
    ### entry opt. ###
    ...
    def protections(self):
        ...
        if self.use_stop_protection.value:
            prot.append({'method': 'StoplossGuard', 'lookback_period_candles': 24 * 3, 'trade_limit': 8, 'stop_duration_candles': self.stop_duration.value, 'only_per_pair': False})
        return prot
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[(df['rsi'] > self.entry_rsi.value) & (df['rsi'] < 60) & (df['rsi'] > df['rsi_ma']) & (df['wave_t1'] > df['wave_t1'].shift(1)) & qtpylib.crossed_above(df['wave_t1'], df['wave_t2']) & (df['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'WT/RSI')
        return df
    ### EXIT CONDITIONS ###
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Guard: Wave 1 is raising
        # Make sure Volume is not 0
        df.loc[(df['rsi'] > self.exit_rsi.value) & (df['wave_t1'] < df['wave_t1'].shift(1)) & qtpylib.crossed_above(df['wave_t2'], df['wave_t1']) & (df['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'WT/RSI')
        return df