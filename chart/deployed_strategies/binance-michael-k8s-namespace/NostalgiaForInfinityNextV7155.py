...
class NostalgiaForInfinityNextV7155(IStrategy):
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        exit, signal_name = self.exit_r_5(current_profit, last_candle)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Williams %R based exit 6, plus  RSI, CTI, CCI
        exit, signal_name = self.exit_r_6(current_profit, last_candle)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Pivot points based exits
        exit, signal_name = self.exit_pivot(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Profit Target Signal
        ...
        if self.target_profit_cache is not None and pair in self.target_profit_cache.data:
            ...
            exit, signal_name = self.exit_profit_target(pair, trade, current_time, current_rate, current_profit, last_candle, previous_candle_1, previous_rate, previous_exit_reason, previous_time_profit_reached)
            if exit and signal_name is not None:
                return f'{signal_name} ( {entry_tag})'
        ...
        if self.exit_condition_1_enable and last_candle['rsi_14'] > self.exit_rsi_bb_1 and (last_candle['close'] > last_candle['bb20_2_upp']) and (previous_candle_1['close'] > previous_candle_1['bb20_2_upp']) and (previous_candle_2['close'] > previous_candle_2['bb20_2_upp']) and (previous_candle_3['close'] > previous_candle_3['bb20_2_upp']) and (previous_candle_4['close'] > previous_candle_4['bb20_2_upp']) and (previous_candle_5['close'] > previous_candle_5['bb20_2_upp']):
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.01:
                    return f'exit_signal_1_1_1 ( {entry_tag})'
            elif current_profit > 0.01:
                return f'exit_signal_1_2_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_1_2_2 ( {entry_tag})'
        # Sell signal 2
            ...
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.01:
                    return f'exit_signal_2_1_1 ( {entry_tag})'
            elif current_profit > 0.01:
                return f'exit_signal_2_2_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_2_2_2 ( {entry_tag})'
        # Sell signal 3
        elif self.exit_condition_3_enable and last_candle['rsi_14'] > self.exit_rsi_main_3:
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.01:
                    return f'exit_signal_3_1_1 ( {entry_tag})'
            elif current_profit > 0.01:
                return f'exit_signal_3_2_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_3_2_2 ( {entry_tag})'
        # Sell signal 4
            ...
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.01:
                    return f'exit_signal_4_1_1 ( {entry_tag})'
            elif current_profit > 0.01:
                return f'exit_signal_4_2_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_4_2_2 ( {entry_tag})'
        # Sell signal 6
            ...
            if current_profit > 0.01:
                return f'exit_signal_6_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_6_2 ( {entry_tag})'
        # Sell signal 7
        elif self.exit_condition_7_enable and last_candle['rsi_14_1h'] > self.exit_rsi_1h_7 and last_candle['crossed_below_ema_12_26']:
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.01:
                    return f'exit_signal_7_1_1 ( {entry_tag})'
            elif current_profit > 0.01:
                return f'exit_signal_7_2_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_7_2_2 ( {entry_tag})'
        # Sell signal 8
        elif self.exit_condition_8_enable and last_candle['close'] > last_candle['bb20_2_upp_1h'] * self.exit_bb_relative_8:
            if last_candle['close'] > last_candle['ema_200']:
                if current_profit > 0.01:
                    return f'exit_signal_8_1_1 ( {entry_tag})'
            elif current_profit > 0.01:
                return f'exit_signal_8_2_1 ( {entry_tag})'
            elif max_loss > 0.5:
                return f'exit_signal_8_2_2 ( {entry_tag})'
        return None
