...
class NFIX_BB_RPB_c7c477d_20211030(IStrategy):
    ...
    def exit_long_mode(self, current_profit: float, max_profit: float, max_loss: float, last_candle, previous_candle_1, previous_candle_2, previous_candle_3, previous_candle_4, previous_candle_5, trade: 'Trade', current_time: 'datetime', entry_tag) -> tuple:
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        # Long mode
        if all((c in ['31', '32', '33', '34'] for c in entry_tags)):
            exit, signal_name = self.exit_long_mode(current_profit, max_profit, max_loss, last_candle, previous_candle_1, previous_candle_2, previous_candle_3, previous_candle_4, previous_candle_5, trade, current_time, entry_tag)
            if exit and signal_name is not None:
                return f'{signal_name} ( {entry_tag})'
            # Skip remaining exit logic for long mode
            return None
        # Quick exit mode
        if all((c in ['empty', '38', '39'] for c in entry_tags)):
            exit, signal_name = self.exit_quick_mode(current_profit, max_profit, last_candle, previous_candle_1)
            if exit and signal_name is not None:
                return f'{signal_name} ( {entry_tag})'
        # Original exit signals
        exit, signal_name = self.exit_signals(current_profit, max_profit, max_loss, last_candle, previous_candle_1, previous_candle_2, previous_candle_3, previous_candle_4, previous_candle_5, trade, current_time, entry_tag)
        ...
        exit, signal_name = self.exit_r(current_profit, max_profit, max_loss, last_candle, previous_candle_1, trade, current_time)
        if exit and signal_name is not None:
            return f'{signal_name} ( {entry_tag})'
        # Trailing
        ...
        for index in self.entry_protection_params:
            ...
            if self.entry_params[f'entry_condition_{index}_enable']:
                ...
                # Condition #5 - Semi swing. Local dip. Uptrend.
                    ...
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['sma_75'] * 0.942)
                    item_entry_logic.append(dataframe['ewo'] > 3.8)
                    item_entry_logic.append(dataframe['cti'] < -0.9)
                    item_entry_logic.append(dataframe['cci'] < -120.0)
                ...
                # Condition #11 - Semi swing. Local dip.
                    ...
                    item_entry_logic.append(dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100)
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_20'] * 0.938)  # 0.958 0.938
                    item_entry_logic.append(dataframe['rsi_14'] < 20.0)  # 28.0 20.0
                    item_entry_logic.append(dataframe['crsi_1h'] > 14.0)
                # Condition #12 - Semi swing. Local deeper dip. Uptrend.
                elif index == 12:
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_20'] * 0.935)
                ...
                # Condition #26 - (nfi 32)
                    ...
                    item_entry_logic.append(dataframe['close'] < dataframe['sma_15'] * 0.942)
                ...
                # Condition #27 - Semi swing. Local deep. Uptrend.
                    ...
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['sma_75'] * 0.934)  # 0.984
                ...
                # Condition #33 - Long mode. Local dip. Uptrend.
                    ...
                    # Non-Standard protections
                    # Logic
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_16'] * 0.942)
                ...
                # ewo
                    ...
                    # Logic
                    item_entry_logic.append(dataframe['rsi_4'] < 45)
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_8'] * 0.942)
                    item_entry_logic.append(dataframe['ewo'] > -5.585)
                    item_entry_logic.append(dataframe['close'] < dataframe['ema_16'] * 1.084)
                    item_entry_logic.append(dataframe['rsi_14'] < 35)
                # BB safe dump
                elif index == 37:
                    # Logic
