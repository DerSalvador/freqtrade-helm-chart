...
class BB_RPB_3c(IStrategy):
    ...
    entry_params = {
        ...
        "entry_fastk": 22,
        "entry_ema_cofi": 0.98,
        "entry_ewo_high": 4.179,
        ##
        "entry_clucha_bbdelta_close": 0.01965,
        "entry_clucha_bbdelta_tail": 0.95089,
        "entry_clucha_closedelta_close": 0.00556,
        "entry_clucha_rocr_1h": 0.54904,
        ##
        ...
        ##
        "entry_vwap_closedelta": 19.108,
        "entry_vwap_cti": -0.022,
        "entry_vwap_width": 0.392,
        ##
        "entry_lambo2_ema": 0.983,
        "entry_lambo2_rsi14": 44,
        "entry_lambo2_rsi4": 44,
        ##
        "entry_V_bb_width": 0.067,
        "entry_V_cti": -0.672,
        "entry_V_mfi": 38.796,
        "entry_V_r14": -53.601,
    }
    ...
    entry_vwap_width = DecimalParameter(0.05, 10.0, default=0.80 , optimize = is_optimize_vwap)
    ...
    pHSL = DecimalParameter(-0.350, -0.040, default=-0.08, decimals=3, space='exit', load=True, optimize=True)
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(0.008, 0.020, default=0.016, decimals=3, space='exit', load=True, optimize=False)
    pSL_1 = DecimalParameter(0.008, 0.020, default=0.011, decimals=3, space='exit', load=True, optimize=False)

    # profit threshold 2, SL_2 is used
    pPF_2 = DecimalParameter(0.040, 0.100, default=0.080, decimals=3, space='exit', load=True, optimize=False)
    pSL_2 = DecimalParameter(0.020, 0.070, default=0.040, decimals=3, space='exit', load=True, optimize=False)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_cofi = (
                (dataframe['open'] < dataframe['ema_8'] * self.entry_ema_cofi.value) &
                (qtpylib.crossed_above(dataframe['fastk'], dataframe['fastd'])) &
                (dataframe['fastk'] < self.entry_fastk.value) &
                (dataframe['fastd'] < self.entry_fastd.value) &
                (dataframe['adx'] > self.entry_adx.value) &
                (dataframe['EWO'] > self.entry_ewo_high.value)
            )
        ...
        is_vwap = (
                ...
                (dataframe['vwap_width'] > self.entry_vwap_width.value) &
                (dataframe['closedelta'] > dataframe['close'] * self.entry_vwap_closedelta.value / 1000 ) &
                (dataframe['cti'] < self.entry_vwap_cti.value)
            )

        is_lambo_2 = (
                (dataframe['close'] < dataframe['ema_14'] * self.entry_lambo2_ema.value) &
                (dataframe['rsi_fast'] < self.entry_lambo2_rsi4.value) &
                (dataframe['rsi'] < self.entry_lambo2_rsi14.value)
            )

        is_V = (
                (dataframe['bb_width'] > self.entry_V_bb_width.value) &
                (dataframe['cti'] < self.entry_V_cti.value) &
                (dataframe['r_14'] < self.entry_V_r14.value) &
                (dataframe['mfi'] < self.entry_V_mfi.value)
            )
        ...
        dataframe.loc[is_clucHA, 'entry_tag'] += 'cluc '
        ...
        dataframe.loc[is_lambo_2, 'entry_tag'] += 'lambo2 '
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(
            (                   
                ...
                (dataframe['close'] > (dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset_2.value)) &
                (dataframe['rsi']>50)&
                (dataframe['volume'] > 0)&                
