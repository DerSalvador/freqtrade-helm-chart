kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-mssm-01 exec -it pod/freqtrade-bot-mssm-01-858b999f6c-pf26r -c freqtrade -- cat /extra_strategies/DWT_Predict2Hedged.py
...
class DWT_Predict2Hedged(IStrategy):
    ...
    # Required
    startup_candle_count = 128  # must be power of 2
    win_size = 14
    ...
    profit_threshold = DecimalParameter(0.01, 0.08, default=0.07, decimals=3, space='sell', load=True, optimize=True)
    use_profit_threshold = CategoricalParameter([True, False], default=False, space='sell', load=True, optimize=True)
    ...
    def hedge(self, pair, direction):
        ...
        # log.info("Entering Hedge Modus")
        # current_timestamp = datetime.now()
        # tag = f'ForcedHedging {pair} at {current_timestamp} with leverage {self.hedging_leverage}'
        # tag += f'amount {self.hedging_stake_amount}' 
        # tag += f'direction {direction} on bot {self.hedging_url}'
        # log.info(f"Entering hedging: {tag}")
        # payload = {
        #     "pair": pair,
        #     "side": direction,
        #     "ordertype": "market",
        #     "stakeamount": self.hedging_stake_amount,
        #     "entry_tag": tag,
        #     "leverage": self.hedging_leverage
        # }
        # headers = {
        ...
        # try:
        #     response = requests.post(self.hedging_url, json=payload, headers=headers, auth=auth)
        #     response.raise_for_status()  # Check for HTTP errors
        #     if response.status_code == 200:
        #         log.info("Hedging successful. Response:")
        #         log.info(response.json())
        #     else:
        #         log.info(f"Request returned status code: {response.status_code}")
        # except requests.RequestException as e:
        #     log.error(f"An error occurred during hedging: {e}")
        # log.info("Leaving Hedge Modus")
        
    @staticmethod
    def setRPCManager(rpc: RPCManager):
        DWT_Predict2Hedged.rpc = rpc
    ...
    def sendMessageToTelegram(msg: str):
        ...
        if DWT_Predict2Hedged.rpc is not None: 
            DWT_Predict2Hedged.rpc.send_msg(msg)
        else:
    ...
    def bot_start(self, **kwargs) -> None:
        ...
        if self.config['dersalvador']['hedging'] is not None:
            ...
            log.info(msg)
            DWT_Predict2Hedged.sendMessageToTelegram(msg)
        else:
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        log.info(f"populate_indicators:" + str(dir(self.config)))
    ...
    # is called every 5 seconds, https://www.freqtrade.io/en/stable/strategy-callbacks/#custom-exit-signal
    def custom_exit(self, pair: str, trade: Trade, current_time: 'datetime', current_rate: float,
                    current_profit: float, **kwargs):
        # Keep for testing Strategy Improvements, is called every 5 seconds, add code and breakpoint here
        ...
        DWT_Predict2Hedged.existing_position_on_exchange = positionFetcher.get_futures_position_information(symbol)
        # dsHedging.hedge_me(self, trade, pair)
        # dsHedging.hedge_me(self, trade, pair, DWT_Predict2Hedged.existing_position_on_exchange)
        
        if not self.use_custom_stoploss:
            return None

        # New logic: Evaluate indicators for hedging when profit trend is negative
        if current_profit < 0:
            # Example indicator: if RSI is below 30, indicating oversold, consider hedging
            if last_candle['rsi'] < 30:
                log.info(f"Hedging: RSI oversold")
                dsHedging.hedge_me(self, trade, pair, DWT_Predict2Hedged.existing_position_on_exchange)
            ...
            if last_candle['macd'] < last_candle['macdsignal']:
                log.info(f"Hedging: MACD bearish")
                dsHedging.hedge_me(self, trade, pair, DWT_Predict2Hedged.existing_position_on_exchange)
                log.info(f"Leaving custom_exit for {pair}, current_profit: {current_profit}, current_rate: {current_rate}, timestamp: {current_time}")
                return 'macd_bearish'
        ...
        if (current_profit > 0.05) and (last_candle['fisher_wr'] > 0.95):
            log.info(f"Hedging: fwr_overbought")
            log.info(f"Leaving custom_exit for {pair}, current_profit: {current_profit}, current_rate: {current_rate}, timestamp: {current_time}")
            return 'fwr_overbought'

        # Above 1%, sell if Fisher/Williams in sell range
        if current_profit > 0.05:
            if last_candle['fisher_wr'] > 1.8:
                return 'take_profit'
        ...
        if (current_profit > 0.05) and (last_candle['exit_long'] > 0):
            log.info(f"Leaving custom_exit for {pair}, current_profit: {current_profit}, current_rate: {current_rate}, timestamp: {current_time}")
            return 'exit_signal'
        else:
            dsHedging.hedge_me(self, trade, pair, DWT_Predict2Hedged.existing_position_on_exchange)
