kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-mssm-02 exec -it pod/freqtrade-bot-mssm-02-688b7954b7-s4785 -c freqtrade -- cat /extra_strategies/NNPredict_Wavenet.py
import operator
...
from NNPredictor_Wavenet import NNPredictor_Wavenet

"""
####################################################################################
Predict_Wavenet2 - uses a simplified Wavenet neural network to try and predict the future stock price
...
class NNPredict_Wavenet(NNPredict):
    ...
    def get_classifier(self, pair, seq_len: int, num_features: int):
        return NNPredictor_Wavenet(pair, seq_len, num_features)
