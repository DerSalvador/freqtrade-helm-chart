kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-mssm-02 exec -it pod/freqtrade-bot-mssm-02-688b7954b7-s4785 -c freqtrade -- cat /extra_strategies/NNPredict_dTransformer.py
import operator
...
from NNPredictor_dTransformer import NNPredictor_dTransformer

"""
####################################################################################
Predict_dTransformer - uses a darts Transformer neural network to try and predict the future stock price
...
class NNPredict_dTransformer(NNPredict):
    plot_config = {
        'main_plot': {
    ...
    def get_classifier(self, pair, seq_len: int, num_features: int):
        use_gpu = True # gpu issues with this model
        predictor = NNPredictor_dTransformer(pair, seq_len, num_features, use_gpu=use_gpu)
