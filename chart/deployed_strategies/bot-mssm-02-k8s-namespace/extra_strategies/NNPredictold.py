kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-mssm-02 exec -it pod/freqtrade-bot-mssm-02-6458f9f56b-q6g2s -c freqtrade -- cat /extra_strategies/NNPredictold.py
# import operator
# import tracemalloc
...
class NNPredict(IStrategy):
    ...
    def add_model_batch_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        if num_left > 0:
            ...
            if use_dataframes:
                chunk = df_norm.iloc[-num_left:]
            else:
                chunk = df_tensor[-num_left:]
            # preds = self.get_predictions(chunk).reshape(-1)
            preds = self.get_predictions(chunk)
            ...
            tmp = self.norm_array(dataframe['gain'].iloc[-num_left:].to_numpy())
            preds = self.denorm_array(preds)
