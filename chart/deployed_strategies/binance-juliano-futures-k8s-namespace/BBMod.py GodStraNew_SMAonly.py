kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n binance-juliano-futures exec -it pod/freqtrade-binance-juliano-futures-6f9c6d5f65-gmgmq -c freqtrade -- cat /freqtrade/user_data/strategies/BBMod.py GodStraNew_SMAonly.py
...
class BBMod(IStrategy):
    ...
    exit_fisher = DecimalParameter(0.1, 0.5, default=0.38414, space='exit', optimize=True)
    exit_bbmiddle_close = DecimalParameter(0.97, 1.1, default=1.07634, space='exit', optimize=True)
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if current_profit >= 0.019:
            return None
        if current_profit < 0.019 and last_candle['close'] > last_candle['sma_9'] and (last_candle['close'] > last_candle['ema_24'] * self.high_offset_2.value) and (last_candle['rsi'] > 50) and (last_candle['rsi_fast'] > last_candle['rsi_slow']):
            if last_candle['close'] > last_candle['bb_middleband2']:
                if trade.id not in TMP_HOLD:
                    TMP_HOLD.append(trade.id)
                    return None
        # start cross under bb mid to exit
        for i in TMP_HOLD:
            if trade.id == i and last_candle['close'] < last_candle['bb_middleband2']:
                TMP_HOLD.remove(i)
                return 'exit_drop_bb_mid'
    ############################################################################
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        dataframe.loc[:, 'enter_tag'] = ''  # from NFI next gen, credit goes to @iterativ
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float, min_stake: float, max_stake: float, **kwargs):
        ...
        if 0 < count_of_entrys <= self.max_reentry_orders:
            try:
                ...
                return None
        return Nonecat: GodStraNew_SMAonly.py: No such file or directory
