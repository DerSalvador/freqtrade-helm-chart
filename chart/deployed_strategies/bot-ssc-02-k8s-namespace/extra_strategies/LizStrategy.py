kubectl --context=gke_vaulted-gift-406223_europe-west1-b_private-cluster-3 -n bot-ssc-02 exec -it pod/freqtrade-bot-ssc-02-dcb7fc589-9qw8q -c freqtrade -- cat /extra_strategies/LizStrategy.py
...
class LizStrategy(IStrategy):
    ...
    def resetDatabase(self):
        try:
            ...
            data_dir = self.config['database_path']
    ...
    def adjustLinearRegressionThreshold(self):
        ...
        if winrate < self.linear_regression_winrate_threshold: 
    ...
    def checkTrends(self, pair, trade, current_time, current_rate, current_profit, pair_dataframe, market_aggregrated_df, aggregate_df, existing_position_on_exchange, infix=""):
        ...
        if trend_pair == Constants.LINEAR_DECREASING and trend_market == Constants.LINEAR_DECREASING and trend_pair_aggregated == Constants.LINEAR_DECREASING:            
            ...
            self.writeDataframeToFile(pair, market_aggregrated_df, hedging_direction + "Market_LINEAR_DECREASING", slope_market)  
            self.writeDataframeToFile(pair, aggregate_df, hedging_direction + "Pair_Aggregated_LINEAR_DECREASING", slope_pair_aggregated)  
