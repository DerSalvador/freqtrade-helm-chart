...
class twinturboV8(IStrategy):
    ...
    # entry optizimation
    max_epa = CategoricalParameter([2, 3], default=0, space='entry', optimize=True)
    # protections
    ...
    low_offset = DecimalParameter(0.95, 0.99, default=0.97, decimals=2, space='entry', optimize=True)
    high_offset = DecimalParameter(0.95, 1.1, default=1.0, decimals=2, space='exit', optimize=True)
    high_offset_2 = DecimalParameter(0.99, 1.5, default=1.3, decimals=2, space='exit', optimize=True)
    # Protection
    fast_ewo = 50
    slow_ewo = 200
    ewo_low = IntParameter(-5, 0, default=-4, space='entry', optimize=True)
    ewo_high = IntParameter(0, 2, default=0, space='entry', optimize=True)
    ewo_high_safe = IntParameter(2, 8, default=8, space='entry', optimize=True)
    ewo_high_uL = IntParameter(15, 25, default=20, space='entry', optimize=True)
    ...
    dca1 = DecimalParameter(low=0.01, high=0.05, decimals=2, default=0.02, space='entry', optimize=True, load=True)
    dca2 = DecimalParameter(low=0.03, high=0.08, decimals=2, default=0.04, space='entry', optimize=True, load=True)
    dca3 = DecimalParameter(low=0.05, high=0.12, decimals=2, default=0.06, space='entry', optimize=True, load=True)
    #trailing stop loss optimiziation
    tsl_target5 = DecimalParameter(low=0.3, high=0.4, decimals=1, default=0.3, space='exit', optimize=True, load=True)
    ...
    tsl_target3 = DecimalParameter(low=0.12, high=0.18, default=0.15, space='exit', optimize=True, load=True)
    ...
    tsl_target1 = DecimalParameter(low=0.04, high=0.07, default=0.06, space='exit', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.01, high=0.016, default=0.013, space='exit', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.02, high=0.05, default=0.04, space='exit', optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # HMA-BUY SQUEEZE
        dataframe['HMA_SQZ'] = (dataframe['hma_50'] - dataframe[f'ma_entry_{self.base_nb_candles_entry.value}']) / dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * 100
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] < self.ewo_high_uL.value) & (dataframe['EWO'] > self.ewo_high_safe.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'].shift(1) > dataframe['EWO'].shift(2)) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['rsi_fast'] < 35) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Safe < EWO2 & RSI Fast')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] < self.ewo_high_uL.value) & (dataframe['EWO'] > self.ewo_high_safe.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'].shift(1) > dataframe['EWO'].shift(2)) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Safe < EWO2')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] < self.ewo_high_uL.value) & (dataframe['EWO'] > self.ewo_high_safe.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['rsi_fast'] < 35) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Safe < EWO1 & RSI Fast')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] < self.ewo_high_uL.value) & (dataframe['EWO'] > self.ewo_high_safe.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Safe < EWO1')
        # dataframe.loc[
        #     (
        ...
        #         (dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] > dataframe[f'ma_exit_{self.base_nb_candles_exit.value}']) &
        ...
        #         (dataframe['EWO'] > self.estop.value) &
        #         (dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] < dataframe[f'ma_exit_{self.base_nb_candles_exit.value}']) &
        #         (dataframe['hma_50'] > dataframe[f'ma_exit_{self.base_nb_candles_exit.value}']) &
        #         (dataframe['HMA_SQZ'] < 0.2) &
        #         (dataframe['rsi'] < self.rsi_entry.value) &
        #         (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        #         (dataframe['volume'] > 0)
        ...
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['EWO'] < self.ewo_high_safe.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'].shift(1) > dataframe['EWO'].shift(2)) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Safe > EWO2> High')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'].shift(1) > dataframe['EWO'].shift(2)) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['rsi_fast'] < 35) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'EWO2 < low & RSI Fast')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'].shift(1) > dataframe['EWO'].shift(2)) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'EWO2 < low')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['EWO'] < self.ewo_high_safe.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['rsi'] < self.rsi_entry.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'Safe > EWO1 > High')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['rsi_fast'] < 35) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'EWO1 < low & RSI Fast')
        # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        dataframe.loc[(dataframe['close'] < dataframe[f'ma_entry_{self.base_nb_candles_entry.value}'] * self.low_offset.value) & (dataframe['EWO'] > dataframe['EWO'].shift(1)) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'EWO1 < low')
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['close'] > dataframe['hma_50']) & (dataframe['close'] > dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset_2.value) & (dataframe['rsi'] > 50) & (dataframe['volume'] > 0) & (dataframe['rsi_fast'] > dataframe['rsi_slow']), ['exit_long', 'exit_tag']] = (1, 'Close > Offset Hi')
        dataframe.loc[(dataframe['close'] < dataframe['hma_50']) & (dataframe['close'] > dataframe[f'ma_exit_{self.base_nb_candles_exit.value}'] * self.high_offset.value) & (dataframe['volume'] > 0) & (dataframe['rsi_fast'] > dataframe['rsi_slow']), ['exit_long', 'exit_tag']] = (1, 'Close < Offset Lo')
        # dataframe.loc[
        #     (
        ...
        dataframe.loc[(dataframe['BTC_EWO_Fast_1h'] <= self.estop.value) & (dataframe['volume'] > 0), ['exit_long', 'exit_tag']] = (1, 'fucking bearzzz')
        return dataframe
# 2023-02-28 18:02:48,341 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): base_nb_candles_entry = 59
...
# 2023-02-28 18:02:48,341 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): rsi_entry = 68
# 2023-02-28 18:02:48,342 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): slow_1h = 30
# 2023-02-28 18:02:48,342 - freqtrade.strategy.hyper - INFO - No params for exit found, using default values.
# 2023-02-28 18:02:48,342 - freqtrade.strategy.hyper - INFO - Strategy Parameter(default): base_nb_candles_exit = 49
