# Hour Strategy
# In this strategy we try to find the best hours to entry and exit in a day.(in hourly timeframe)
...
class HourBasedStrategy(IStrategy):
    ...
    # entry_params = {"entry_hour_max": 18,"entry_hour_min": 7,}
    # exit_params = {"exit_hour_max": 9,"exit_hour_min": 21,}
    ...
    # entry_params = {"entry_hour_max": 21,"entry_hour_min": 6,}
    # exit_params = {"exit_hour_max": 6,"exit_hour_min": 4,}
    ...
    # entry_params = {"entry_hour_max": 23,"entry_hour_min": 4,}
    # exit_params = {"exit_hour_max": 23,"exit_hour_min": 3,}
    ...
    # entry_params = {"entry_hour_max": 22,"entry_hour_min": 9,}
    # exit_params = {"exit_hour_max": 1,"exit_hour_min": 7,}
    ...
    entry_params = {'entry_hour_max': 24, 'entry_hour_min': 4}
    # Sell hyperspace params:
    exit_params = {'exit_hour_max': 21, 'exit_hour_min': 22}
    # ROI table:
    ...
    # Optimal timeframe
    timeframe = '1h'
    entry_hour_min = IntParameter(0, 24, default=1, space='entry')
    entry_hour_max = IntParameter(0, 24, default=0, space='entry')
    exit_hour_min = IntParameter(0, 24, default=1, space='exit')
    exit_hour_max = IntParameter(0, 24, default=0, space='exit')
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        min, max = (self.entry_hour_min.value, self.entry_hour_max.value)
        dataframe.loc[dataframe['hour'].between(min, max), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        min, max = (self.exit_hour_min.value, self.exit_hour_max.value)
        dataframe.loc[dataframe['hour'].between(min, max), 'enter_long'] = 1
        return dataframe