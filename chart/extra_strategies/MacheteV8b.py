...
class MacheteV8b(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        #(dataframe['vfi'] < 0.0) &
        #(dataframe['volume'] > 0)
        dataframe.loc[((self.get_entry_signal_awesome_macd(dataframe) == True) | (self.get_entry_signal_adx_momentum(dataframe) == True) | (self.get_entry_signal_adx_smas(dataframe) == True) | (self.get_entry_signal_asdts_rockwelltrading(dataframe) == True) | (self.get_entry_signal_averages_strategy(dataframe) == True) | (self.get_entry_signal_fisher_hull(dataframe) == True) | (self.get_entry_signal_gettin_moist(dataframe) == True) | (self.get_entry_signal_hlhb(dataframe) == True) | (self.get_entry_signal_macd_strategy_crossed(dataframe) == True) | (self.get_entry_signal_macd_strategy(dataframe) == True) | (self.get_entry_signal_pmax(dataframe) == True) | (self.get_entry_signal_quickie(dataframe) == True) | (self.get_entry_signal_scalp(dataframe) == True) | (self.get_entry_signal_simple(dataframe) == True) | (self.get_entry_signal_strategy001(dataframe) == True) | (self.get_entry_signal_technical_example_strategy(dataframe) == True) | (self.get_entry_signal_tema_rsi_strategy(dataframe) == True)) & (dataframe['sslUp_inf'] > dataframe['sslDown_inf']) & (dataframe['up_trend_inf'] > 0) & (dataframe['go_long_inf'] > 0), 'enter_long'] = 1
        return dataframe
    ...
    def get_entry_signal_pmax(self, dataframe: DataFrame):
        signal = (self.entry_should_use_get_entry_signal_pmax.value == True) & qtpylib.crossed_above(dataframe['ZLEMA'], dataframe['pm_10_3_10_9'])
        return signal
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  #&
        ...
        #(dataframe['vfi'] < 0.0) &
        #(dataframe['volume'] > 0)
        dataframe.loc[qtpylib.crossed_above(dataframe['sslDown_inf'], dataframe['sslUp_inf']) & (qtpylib.crossed_below(dataframe['tenkan_sen_inf'], dataframe['kijun_sen_inf']) | qtpylib.crossed_below(dataframe['close_inf'], dataframe['kijun_sen_inf'])), 'exit_long'] = 1
        return dataframe
    #
    # Custom Stoploss
    #
