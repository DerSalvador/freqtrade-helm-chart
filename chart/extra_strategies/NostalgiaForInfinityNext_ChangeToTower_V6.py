...
class NostalgiaForInfinityNext_ChangeToTower_V6(IStrategy):
    ...
    def informative_1h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_1h['seq_entry'] = informative_1h['close'] < informative_1h['close'].shift(4)
        informative_1h['seq_entry'] = informative_1h['seq_entry'] * (informative_1h['seq_entry'].groupby((informative_1h['seq_entry'] != informative_1h['seq_entry'].shift()).cumsum()).cumcount() + 1)
        # count consecutive closes “higher” than the close 4 bars prior.
        informative_1h['seq_exit'] = informative_1h['close'] > informative_1h['close'].shift(4)
        informative_1h['seq_exit'] = informative_1h['seq_exit'] * (informative_1h['seq_exit'].groupby((informative_1h['seq_exit'] != informative_1h['seq_exit'].shift()).cumsum()).cumcount() + 1)
        for index, row in informative_1h.iterrows():
            ...
            seq_b = row['seq_entry']
            if seq_b == 8:
                informative_1h.loc[index, 'exceed_low'] = (row['low'] < informative_1h.loc[index - 2, 'low']) | (row['low'] < informative_1h.loc[index - 1, 'low'])
            if seq_b > 8:
                informative_1h.loc[index, 'exceed_low'] = (row['low'] < informative_1h.loc[index - 3 - (seq_b - 9), 'low']) | (row['low'] < informative_1h.loc[index - 2 - (seq_b - 9), 'low'])
                if seq_b == 9:
            ...
            seq_s = row['seq_exit']
            if seq_s == 8:
                informative_1h.loc[index, 'exceed_high'] = (row['high'] > informative_1h.loc[index - 2, 'high']) | (row['high'] > informative_1h.loc[index - 1, 'high'])
            if seq_s > 8:
                informative_1h.loc[index, 'exceed_high'] = (row['high'] > informative_1h.loc[index - 3 - (seq_s - 9), 'high']) | (row['high'] > informative_1h.loc[index - 2 - (seq_s - 9), 'high'])
                if seq_s == 9:
        ...
        for index in self.entry_protection_params:
            ...
            if self.entry_params['entry_condition_' + str(index) + '_enable']:
                ...
                # Condition #40 - ZLEMA X entry
                    ...
                    item_entry_logic.append(dataframe['exceed_low_1h'])
                    item_entry_logic.append(dataframe['seq_entry_1h'] > 8)
