...
class CCIStrategy(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        """
        # insurance
        dataframe.loc[(dataframe['cci_one'] < -100) & (dataframe['cci_two'] < -100) & (dataframe['cmf'] < -0.1) & (dataframe['mfi'] < 25) & (dataframe['resample_medium'] > dataframe['resample_short']) & (dataframe['resample_long'] < dataframe['close']), 'enter_long'] = 1
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        :return: DataFrame with entry column
        """
        dataframe.loc[(dataframe['cci_one'] > 100) & (dataframe['cci_two'] > 100) & (dataframe['cmf'] > 0.3) & (dataframe['resample_sma'] < dataframe['resample_medium']) & (dataframe['resample_medium'] < dataframe['resample_short']), 'exit_long'] = 1
        return dataframe
