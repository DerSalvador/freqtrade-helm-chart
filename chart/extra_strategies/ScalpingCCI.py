...
class ScalpingCCI(IStrategy):
    ...
    minimal_roi = {'0': 0.02, '10': 0.05, '20': 0.04, '60': 0.3, '120': 0.2}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['close'] > dataframe['sma']) & qtpylib.crossed_above(dataframe['macd'], dataframe['macdsignal']) & (dataframe['close'] > dataframe['pivot']) & (dataframe['close'] > dataframe['bc']) & (dataframe['close'] > dataframe['tc']) & ((dataframe['pivot'] > dataframe['pivot'].shift(97)) & (dataframe['bc'] > dataframe['bc'].shift(97)) & (dataframe['close'] > dataframe['tc'].shift(97))), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['open'] >= 0.98 * dataframe['high_daily']) | qtpylib.crossed_below(dataframe['macd'], dataframe['macdsignal']), 'exit_long'] = 1
        return dataframe