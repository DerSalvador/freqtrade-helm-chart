...
class NostalgiaForInfinityX4(IStrategy):
  ...
  # Based on the the first entry (regardless of reentries)
  stop_threshold = 0.5
  ...
  reentry_mode_thresholds = [-0.06, -0.08, -0.10]
  ...
  entry_params = {
    ...
    "entry_condition_104_enable": False,
    "entry_condition_105_enable": False,
  }
  ...
  def custom_stake_amount(
    ...
    **kwargs,
    ...
    if self.position_adjustment_enable == True:
      ...
      # Reentry mode
      if all(c in self.long_reentry_mode_tags for c in enter_tags):
        return proposed_stake * self.reentry_mode_stake_multiplier
  ...
  def adjust_trade_position(
    ...
    **kwargs,
    ...
    # Reentry mode
    if all(c in self.long_reentry_mode_tags for c in enter_tags):
      return self.reentry_adjust_trade_position(
        trade,
        current_time,
        current_rate,
        current_profit,
  ...
  def grind_adjust_trade_position(
    ...
    **kwargs,
    ...
    if (self.grinding_enable) and (trade.open_date_utc.replace(tzinfo=None) >= datetime(2022, 8, 1) or is_backtest):
      ...
      slice_profit_exit = (
        ((exit_rate - filled_exits[-1].average) / filled_exits[-1].average) if count_of_exits > 0 else 0.0
      )
      ...
      if current_grind_mode == 0:
        ...
        if count_of_entries > 1:
          ...
          for exit_order in filled_exits:
            if (
              exit_order.remaining * exit_rate / (self.futures_mode_leverage if self.is_futures_mode else 1.0)
            ) < min_stake:
          ...
          for order in reversed(filled_orders):
            if order.ft_order_side == "entry":
              ...
              if (
                order.remaining * exit_rate / (self.futures_mode_leverage if self.is_futures_mode else 1.0)
              ) < min_stake:
                num_exists += 1
            # patial fills on exits
            if (num_entries == num_exists) and (order.ft_order_side == "exit"):
              exit_amount = order.remaining * exit_rate / (self.futures_mode_leverage if self.is_futures_mode else 1.0)
              ...
              if exit_amount > min_stake:
                ...
                if (trade.stake_amount - exit_amount) > min_stake:
                  if grind_profit > 0.01:
                    self.dp.send_msg(
                      f"Grinding exit (remaining) [{trade.pair}] | Rate: {exit_rate} | Stake amount: {exit_amount} | Coin amount: {order.remaining} | Profit (stake): {profit_stake} | Profit: {(profit_ratio * 100.0):.2f}% | Grind profit: {(grind_profit * 100.0):.2f}%"
                    )
                    return -exit_amount
                  else:
              ...
              if grind_profit > self.grinding_profit_threshold:
                exit_amount = (
                  entry_order.filled * exit_rate / (self.futures_mode_leverage if self.is_futures_mode else 1.0)
                )
                ...
                if exit_amount > min_stake:
                  self.dp.send_msg(
                    f"Grinding exit [{trade.pair}] | Rate: {exit_rate} | Stake amount: {exit_amount}| Coin amount: {entry_order.filled} | Profit (stake): {profit_stake} | Profit: {(profit_ratio * 100.0):.2f}% | Grind profit: {(grind_profit * 100.0):.2f}%"
                  )
                  return -exit_amount
              # elif (
              ...
              #         )
              # ):
              #     exit_amount = entry_order.filled * exit_rate * 0.999
              #     if ((current_stake_amount - exit_amount) < (min_stake * 1.7)):
              #         exit_amount = (trade.amount * exit_rate) - (min_stake * 1.7)
              #     if (exit_amount > min_stake):
              #         self.dp.send_msg(f"Grinding stop exit [{trade.pair}] | Rate: {exit_rate} | Stake amount: {exit_amount}| Coin amount: {entry_order.filled} | Profit (stake): {profit_stake} | Profit: {(profit_ratio * 100.0):.2f}% | Grind profit: {(grind_profit * 100.0):.2f}%")
              #         return -exit_amount
              break
      ...
      # mode 1
        ...
        # Sell
          ...
          if grind_profit > self.grinding_mode_1_profit_threshold:
            ...
            and (len(filled_orders) >= 2 and filled_orders[1].ft_order_side == "exit")
            # temporary
            and (
  ...
  def reentry_adjust_trade_position(
    self,
    trade: Trade,
    current_time: datetime,
    current_rate: float,
    ...
    **kwargs,
    ...
    reentry_mode_sub_thresholds = self.reentry_mode_thresholds
    partial_exit = False
    sub_grind_count = 0
    total_amount = 0.0
    total_cost = 0.0
  ...
  def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
    ...
    for entry_enable in self.entry_params:
      ...
      if self.entry_params[f"{entry_enable}"]:
        ...
        item_entry_logic.append(dataframe["protections_global"] == True)
